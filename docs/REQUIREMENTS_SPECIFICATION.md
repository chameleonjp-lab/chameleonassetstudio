...
4. v1.0.0 ではスマホの軽編集、確認、テンプレート適用、書き出しを優先する。2D Pro では画面を作業単位に分けつつ、作成から再読み込みまでの全工程へ到達できるようにする。
...
Phase 1〜17 の v1.0.0 要件と、そこから先の 2D Pro 完成条件を混同しない。将来の2D完成条件は `docs/future/2D_COMPLETE_PRODUCT_SPEC.md` を正本とし、データ、対象別出力、端末・復旧、実装順は同文書の「10.1 優先順位」に従う。旧 Phase 文書と3D文書は、今後の着手順の上位文書ではない。
...
| 区分 | 画面幅目安 | 主な用途 |
|---|---:|---|
| スマホ縦 | 375px 以上 | v1.0.0 は確認・軽編集・書き出し。2D Pro は作成から再読み込みまでの全工程へ到達する。 |
| スマホ横 | 667px 以上 | v1.0.0 は軽いキャンバス編集。2D Pro はキャンバス編集、検査、書き出しを完走する。 |
...
## 16. v1.0.0 完成条件と 2D Pro

次の一覧は v1.0.0 の基本完成条件である。2D Pro の完成判定は、`docs/future/2D_COMPLETE_PRODUCT_SPEC.md` と `docs/future/2D_COMPLETION_ROADMAP.md` の 2D Pro Gate を優先する。
