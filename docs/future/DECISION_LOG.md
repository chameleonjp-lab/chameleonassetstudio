...
- D3: implemented / CI-passed / independently-verified / merged（PR #206、CI Run #610）
- D4はADR-2026-07-29-030時点ではpending。現在の契約はADR-2026-07-29-031を正本とする。
- B2 numeric budget / warning / hard cap: pending
...
- D3はPR #206 final head `ac84b8c2d6141f6353c3e07dbb2dbfae9a2f5c98`、merge `3081495a979d10176a05eb2907e7cede55cc8c9a`でmainへ反映した。
...
- D3は`implemented / CI-passed / independently-verified / merged`とする。schema、保存形式、export、dependencyはPR #206で変更していない。

## ADR-2026-07-29-031: D3をcloseoutし、D4 frame alignmentをA1+B1+C1で固定する

### 状態

- accepted（2026-07-29 人間承認、A1+B1+C1）
...
- `Frame.layerStates`はLayerごとのoptionalな上書きであり、stateまたは`transform`がない場合は、現行previewがAsset本体のLayer状態を利用する。
- 不足するstateや`transform`をalignment時に自動生成すると、既存のoptional data境界を変え、対象Frame以外の基礎Layer状態まで暗黙に固定する。
- `Animation.frameIds`は同じFrame IDを複数回参照でき、複数Animationが同じFrameを共有できる。Frame IDを編集すると、そのFrameを参照する全出現へ反映される。
- `LayerTransform.position`は、キャンバス左上を`(0, 0)`、右を`x+`、下を`y+`とするアセット座標のpx値である。
- Frame全体の位置関係を保って動かすには、対象Frameの全LayerStateへ同じdeltaを適用し、rotation、scale、visibility、opacityを変えない必要がある。
- 透明外周や画像内容から自動位置を求める方法は、意図的なジャンプ、攻撃、反動などを誤って打ち消す可能性がある。
...
- Layer単位・全Frame一括移動、canvas drag、自動整列、透明外周解析、画像内容比較、rotation / scale / opacity / visibility調整。
- Frame複製、共有Frameの自動分離、基準Frameの永続変更、D2 onion skin設定の変更。
- schema、version、migration、IndexedDB layout、`asset.json`、`.casproj`、export ZIP、dependency。
- B2の資源上限、Group 12完了判定、今回のdocs-only PRでの製品コード変更、Ready化、merge、Pages配信。
