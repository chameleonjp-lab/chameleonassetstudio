...
## ADR-2026-07-11-013: 2D-3-GAMEDATA-01: ゲームデータ意味検査を advisory UI として実装
...
> 注記: 本実装（PR #58）は ADR-2026-07-11-012 の停止方針の決定と並行して作成された。同 ADR の「再監査まで追加の 2D-3 本実装を停止する」方針に照らし、本 PR も PR #53 / #55 と同じく provisional / ahead-of-gate として扱い、マージ可否と再監査の時期は人間判断に委ねる。

`asset.json` の JSON Schema は型・必須項目という「形」しか検証しない。実際の制作では、フレームが存在しないレイヤーを参照したまま残る、当たり判定や名前が重複する、character なのに body 判定やアンカーが 1 つもない、といった「形は正しいが意味が壊れている／薄い」状態が起こり得るが、これに気づく手段がなかった。work package `2D-3-GAMEDATA-01` として、既存の `Asset` / `Project` / `Layer` / `Animation` 型、JSON Schema、`.casproj` / export ZIP 形式、座標系を一切変更せずに、意味検証を行う advisory な検査 UI を追加した。

### 決定

- `src/core/model/gameDataInspection.ts` に `inspectAsset(asset): InspectionFinding[]` を純関数として追加した。asset を deep-read するだけで一切変更せず、severity（`error` / `warning` / `info`）降順にソートした所見配列を返す。検査内容は次の通り。
  - `error`（参照整合性・id 重複）: `frame.layerStates[].layerId` / `animation.frameIds[]` / `layer.textureId` / `part.layerIds[]` が実在しない ID を参照している（`reference.*`）。当たり判定・アンカー・アニメーション・フレームの `id` が重複している（`*.duplicateId`）。
  - `warning`（見分け・書き出しキー衝突）: 当たり判定・フレームの `name` が重複している（`*.duplicateName`。atlas 書き出しは frame.name をキーに使うため実害があり得る）。`animation.frameIds` が空でアニメーションとして再生できない（`animation.empty`）。
  - `info`（character 向けの気づき）: `assetType === 'character'` なのに用途 `body` の当たり判定が 1 つもない（`collider.characterBodyMissing`）。同じく anchors が 1 つもない（`anchor.characterAnchorsEmpty`）。origin が canvasSize の範囲外（`origin.outOfBounds`、境界ちょうどは範囲内扱い）。
  - 型 aware（assetType で分岐する検査）は現時点で character 向けの 2 件のみに留めた。background / tile / gimmick / item など他の `AssetType` 固有の意味検査は本 work package のスコープ外。
- `src/features/editor/InspectionPanel.tsx` を追加し、`EditorScreen.tsx` の「プロパティ」欄（`selectedAsset` があるときだけ）に「ゲームデータ検査」区画として配置した。`<section aria-label="ゲームデータ検査">`（アクセシブル名を持つため role="region" として露出する）の中に、件数サマリ（0 件なら「問題は見つかりませんでした。」、それ以外は「エラー N・警告 N・情報 N」）、`<ul role="list" aria-label="検査結果">` による所見一覧（severity バッジ・category・message）を表示する。`target.kind` が `collider` または `animation` の所見にだけ「対象を選択」ボタンを出し、押すと `onSelectCollider` / `onSelectAnimation`（`EditorScreen.tsx` 側は既存の `selectedColliderId` / `selectedAnimationId` state をそのまま再利用）経由で当たり判定パネル・アニメーションパネルの選択状態と同期する。`frame` を対象に持つ所見（`reference.frameLayerMissing` 等）にはジャンプ用のフレーム選択 UI が既存に存在しないため、本 work package ではボタンを出さずメッセージ表示のみにした。件数サマリの `<p>` は当初 `role="status"` を付けていたが、画面上部の自動保存インジケーター（同じく `role="status"`、「保存済み」）と衝突し `getByRole('status')` を使う既存 E2E（`assettypes.spec.ts` / `canvas.spec.ts` / `layers.spec.ts`）が strict mode violation で失敗したため、`role="status"` を外した（サマリは毎レンダー静的テキストでライブリージョンとして自動アナウンスする必要性が薄く、実害の大きい ARIA role 衝突を避ける方を優先した）。E2E からサマリを参照する場合は `getByRole('region', { name: 'ゲームデータ検査' })` でスコープしたうえでテキストを直接参照する。
- **自動修正は一切行わない**。`inspectAsset` は読み取り専用で、UI 側もボタンは「対象を選択」だけで、値を書き換えるアクションを一切持たない。ユーザーが対象へジャンプしたうえで、当たり判定パネル / アニメーションパネルから手動で直す前提にした。
- **export / save の経路はゲートしない**。`inspectAsset` は `InspectionPanel` からしか呼ばれず、保存（autosave / `saveProjectBundle`）や書き出し（PNG / asset.json / ZIP / `.casproj`）の処理には一切組み込んでいない。エラーがあっても保存・書き出しは既存どおり成功する。
- **JSON Schema には組み込まない**。`InspectionFinding` 等の型は `src/core/model` 配下に置くが asset.json の一部ではなく、`.casproj` / export ZIP のいずれにも出力しない。UI 表示専用の派生データという位置づけを保つ。
- `src/core/model/gameDataInspection.test.ts`（18 件）で各 code の severity / category / target とソート順、非破壊性（deep-equal）、finding id の一意性を固定した。`e2e/inspection.spec.ts` を追加し、character 新規作成（2D-2-CREATE-01 のフォーム経由）で anchors 空の info が出ること・アンカー追加で消えること、矩形判定を追加して名前重複の warning が出ること・「対象を選択」で GameDataPanel 側の選択（`aria-pressed`）と同期することを確認した。character の新規作成テンプレートは既定で当たり判定「body」を 1 つ付けるため、character 新規作成直後は `collider.characterBodyMissing` が出ない実挙動に合わせてテストを組んでいる。

### しないこと

- 所見の自動修正・ワンクリック解決アクション。
- character 以外の `AssetType`（background / tile / gimmick / item 等）向けの型 aware な意味検査の追加。
- 検査結果を asset.json / `.casproj` / export ZIP のいずれかに保存すること。
- export / save を検査結果でブロックする・確認ダイアログを挟むこと。
- `frame` を対象に持つ所見のジャンプ UI（フレーム選択の仕組み自体の追加）。
- `Asset` / `Project` / `Layer` / `Animation` 型、JSON Schema、`.casproj` 形式、export ZIP 構成、座標系、既存テストの期待値の変更。

### 影響する文書

- `docs/USER_GUIDE.md`（8 章: 「ゲームデータ検査」節を追加）

### 未確定事項

- character 以外の `AssetType` に型 aware な検査（例: tile の tileSize 未設定、item の score/rarity 未設定など）を広げるかどうか。
- `frame` 対象の所見からフレーム一覧・タイムラインへジャンプする UI を将来追加するかどうか。
- 所見件数が多い場合の折りたたみ・フィルタ UI（severity 別の絞り込みなど）を追加するかどうか。
- 検査結果を将来 export 前チェックリスト（ブロックしない警告表示）として書き出しパネル側にも要約表示するかどうか。
