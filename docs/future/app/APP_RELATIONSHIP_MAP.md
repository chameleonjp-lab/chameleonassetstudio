...
| `APP_3D_EDITION_SPEC.md` *(予定)* | A3(3D), PF4(3D), P5 | A4, A2 | 一部（`features/F-3D-01`/`03`〜`07`/`10`〜`15` で入口・確認面・`T-DONE-3`/`4`/`6`/`7`/`8` を具体化。**取り込み → 検品 → 整備 → 軽量化 → 記録 → 書き出しの一続きが通った**。残るは `02`（多形式）・`08`/`09`（骨格・VRM。`APP-O-04` 判断待ち）とエディション文書の統合） |
