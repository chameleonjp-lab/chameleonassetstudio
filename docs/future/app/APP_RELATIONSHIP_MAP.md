...
| F-MOD-05 | プロファイル（推奨セットの束ね） | `features/F-MOD-05-profiles.md` | 詳 | —（提示と決定を分ける。「おすすめ」が全部入りにならない） |
| F-MOD-06 | 文脈連動の「この機能が要る」提示 | `features/F-MOD-06-contextual-suggestion.md` | 詳 | —（気づけない／うるさい、の間に細い道を通す） |
| F-UX-01 | 一貫デザインシステム | `features/F-UX-01-design-system.md` | 詳 | —（揃えるものと揃えないものを分ける。語彙は共有、実装は共有しない） |
| F-UX-02 | アクセシビリティ | `features/F-UX-02-accessibility.md` | 詳 | —（描画面を情報の唯一の出どころにしない。設計の制約であって後付けの補助ではない） |
| F-UX-03 | エラー・制限メッセージ | `features/F-UX-03-error-messages.md` | 詳 | —（中身が揃っても伝わらなければ意味がない。警告疲れを防ぐ） |
| F-UX-04 | 対象端末の方針 | `features/F-UX-04-device-policy.md` | 詳 | —（単に画面が開くことを合格にしない。編集能力と到達能力を分ける） |
| F-UX-05 | 初回起動・最小オンボーディング | `features/F-UX-05-onboarding.md` | 詳 | —（軽く起動することと、すぐ何かできることの両立） |
| F-3D-08 | 骨格検査 & 人型対応付け | `features/F-3D-08-skeleton-humanoid-mapping.md` | 詳 | —（標準語彙に乗る。推定は候補まで・確定は利用者。付与は `APP-O-04` 判断待ち） |
| F-3D-09 | 人型アバター形式の検出 & 利用条件表示 | `features/F-3D-09-vrm-detection-license.md` | 詳 | —（素通しで成立。利用条件は記録と表示まで、機械が権利を決めない） |
...
| F-3D-13 | 3D ゲームデータ + metadata | `features/F-3D-13-game-data-metadata.md` | 詳 | —（器。利用者申告の利用条件を正とし機械が権利を決めない） |
| F-3D-14 | 3D 検品 / inspection report | `features/F-3D-14-inspection-report.md` | 詳 | —（記録。どのバイト列への検査かを必ず紐づけ、追記型） |
| F-3D-15 | 3D 書き出し preset & import notes | `features/F-3D-15-export-presets.md` | 詳 | —（既定では手を加えず出す。再現可能な出力・対応区分） |
...
| E255 | F-3D-05 | 制約 | F-3D-03 | 保存は model 空間に限り、表示変換を変えても保存値が不変である |
...
| E265 | F-3D-13 | 依存 | F-CORE-05 | 器と版・移行の規律に乗る（optional の積み増しだけで済ませない） |
| E266 | F-3D-13 | 尊重 | D3 | additive で、未対応環境でも正本が成立し往復で失われない |
| E267 | F-3D-13 | 制約 | F-3D-15 | 利用者申告の利用条件を正とし、未確認は未確認として出させる |
| E268 | F-3D-14 | 実現 | F-CORE-06 | 検査結果の記録形式（対象の特定値・前提・検査版を伴う）を与える |
| E269 | F-3D-14 | 依存 | F-3D-04 | 指摘の中身と派生への再検査結果を受け取る |
| E270 | F-3D-14 | 支える | F-3D-15 | 成果物へ同梱する検査記録を供給する |
| E271 | F-3D-15 | 実現 | F-CORE-07 | 対象別 preset と取り込み手順として書き出しフレームを 3D で具体化 |
| E272 | F-3D-15 | 尊重 | G3 | 説明を書いただけの対象を「確認済み」と表示しない |
...
| E281 | F-3D-08 | 依存 | F-3D-04 | 構造検査の枠の中で骨の階層と検査を扱う |
| E282 | F-3D-08 | 支える | F-3D-06 | 人型スロットを使ったアンカー配置の補助を提供する（自動配置ではない） |
| E283 | F-3D-08 | 尊重 | G5 | 対応付けは非破壊の付随情報で、モデルを変更しない |
| E284 | F-3D-08 | 制約 | F-3D-15 | 骨の制約値を検品・書き出し前検査・取り込み手順で同じ所在から参照させる |
| E285 | F-3D-09 | 尊重 | D3 | 拡張データを未知データとして保持し、素通しで成立する |
| E286 | F-3D-09 | 支える | F-3D-13 | 内包された利用条件を「参考」として転記する元を提供する |
| E287 | F-3D-09 | 尊重 | G5 | 素通しでバイト列を保持し、利用条件を書き換えた派生を作らない |
| E288 | F-3D-09 | 依存 | F-3D-04 | 拡張と必須要素の検査を構造検査の枠に載せる |
| E289 | F-UX-01 | 実現 | P5 | 別最適化しつつ同じ操作感で行き来できる作法を定める |
| E290 | F-UX-01 | 制約 | A4 | モジュール提供の UI も同じ作法・語彙・状態表示に従う |
| E291 | F-UX-01 | 依存 | F-CORE-04 | 編集操作を共通の履歴経路に載せ、画面ごとの取り消しを作らない |
| E292 | F-UX-02 | 制約 | F-2D-03 | 描画面を情報の唯一の出どころにしない（一覧・数値・文章でも辿れる） |
| E293 | F-UX-02 | 制約 | F-3D-03 | 同上（3D の表示面にも同じ制約を掛ける） |
| E294 | F-UX-02 | 支える | P1 | 使えない利用者を作らないことで「使われない理由」を一つ減らす |
| E295 | F-UX-02 | 依存 | F-UX-01 | 到達性の要件を共通の作法の上に載せる |
| E296 | F-UX-03 | 実現 | F-CORE-06 | 指摘の中身を、人が読んで動ける文章にする |
| E297 | F-UX-03 | 制約 | F-CORE-07 | 失敗の提示に理由と復旧手段を伴わせ、「失敗しました」で終わらせない |
| E298 | F-UX-03 | 支える | P1 | 「失敗が分からない」による離脱を防ぐ |
| E299 | F-UX-03 | 依存 | F-UX-02 | 失敗の提示が支援技術にも確実に届く |
| E300 | F-UX-04 | 制約 | F-UX-02 | 対象環境ごとに到達経路を確保し、片方だけ使える状態を作らない |
| E301 | F-UX-04 | 依存 | F-PLT-04 | 環境ごとに別々に測り、まとめて 1 つの合否にしない |
| E302 | F-UX-04 | 支える | P1 | 「自分の環境では実質使えない」による離脱を防ぐ |
| E303 | F-UX-05 | 実現 | P3 | install-on-demand の入口として「まず最小で始められる」を示す |
| E304 | F-UX-05 | 尊重 | P4 | 初回起動でも未導入機能の実体を読まない |
| E305 | F-UX-05 | 制約 | F-MOD-01 | 導入の勧誘で最初の画面を散らかさない |
| E306 | F-UX-05 | 依存 | F-CORE-08 | 最初の道筋を Dashboard の上に置く |
| E307 | F-UX-05 | 検証 | P1 | 起動の速さと「すぐ使える」が最初に評価される場所 |
| E308 | F-MOD-05 | 支える | P3 | 選択の負担を減らし、install-on-demand を体験として成立させる |
| E309 | F-MOD-05 | 制約 | F-MOD-02 | 適用は明示的な導入経路に乗り、束で入れたものを個別に撤去できる |
| E310 | F-MOD-05 | 尊重 | P4 | 束ねることで「とりあえず入れておく」を作らない |
| E311 | F-MOD-05 | 尊重 | G1 | 未検証のモジュールを束ねて検証を迂回させない |
| E312 | F-MOD-06 | 実現 | P3 | install-on-demand の「必要になった時に気づける」半分を担う |
| E313 | F-MOD-06 | 制約 | F-MOD-01 | 提示を文脈時とカタログ内に限り、常時の宣伝を作らない |
| E314 | F-MOD-06 | 依存 | F-UX-03 | 「できない」と「入れれば できる」の区別を、伝わる文章で示す |
| E315 | F-MOD-06 | 尊重 | D3 | 未導入の機能に触れてもプロジェクトデータが壊れない |
| E316 | F-MOD-06 | 支える | P1 | 「できないと誤解されて離脱する」を防ぐ |
