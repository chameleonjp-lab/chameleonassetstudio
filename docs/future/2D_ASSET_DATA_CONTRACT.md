...
> 境界はADR-0008 / 0009 / 0011、Group 12の実装契約はT1 / ADR-0021、派生exportはH1=E1で決定済み。T1 Slice AはPR #153でmainへ反映済みで、Atlas形式拡張とTimelineのevent編集は後続へ残す。

- `Frame.durationMs?: number`は有限かつ0より大きいms値とし、不在時は`1000 / animation.fps`を使う。
- Animation全体の実時間は`frameIds`の各出現の実効時間を合計する。同じFrameを再利用すれば同じoverrideが全出現へ適用され、違う時間が必要ならFrameを複製する。
- 既存`Animation.durationMs`はinformationalのまま。旧fps-only Assetへ値を補完せず、Asset 0.2.0、migrationなしを維持する。
- eventは対象Frameの表示開始時に発火し、同じFrameの各出現・loopの各周回で発火する。参照はFrame ID、dangling参照は意味検証エラーとする。
- event payloadはプリミティブ、プリミティブ配列、値がプリミティブの平坦objectまでとし、任意コード、秘密情報、URL自動読み込みを許可しない。
- 固定fpsしか表現できない派生exportへ、可変時間やeventを黙って均一化・削除しない。H1=E1により対象atlas / product ZIP / helpers / examplesを理由付きで拒否し、PNG / WebP、単体`asset.json`、`.casproj`は許可する。
...
> Frame単位だけを許可する境界はADR-0010 / 0011、canonicalな永続・fallback・検証・UI・変換・保存・export拒否の詳細はGroup 13 O1 Slice BのADR-0024で決定済みである。Slice BはPR #218で完了し、製品実装Slice Cは別のDraft PRで固定head検証を必須として実装し、mergeまでGroup 13完了へ数えない。
...
O1は2026-08-02に採用済みで、Slice B / PR #218がこの契約をmainへ固定した。現在のSlice Cはこの契約を実装中だが、CIと固定head独立監査が完了するまで実装済み・検証済みとは扱わない。
...
- danglingな`provenance[].textureId`と、正式recordからsource以外への参照は読み取り専用素材検査で示す。検査結果は保存せず、保存・autosave・`.casproj`・exportを新たに停止しない。
