...
マージ後基準: PR #146 merge `cb21ea4` / PR #147 final head `1ba671f7` / merge `24a089c` / PR #148 final head `0cfc1ea` / merge `fbdeb357` / PR #149 merge `536318f`
状態: `T1 Slice A merged / P1 Slice C implementation in review / R1 accepted / formal B0 deferred / H3 numeric budget pending`
...
merge後の人間判断はH1=E1、H2=L1、H3=M1である。E1 / L1の意味は§6、M1の再現手順は`2D_3_H3_MEASUREMENT_PROTOCOL.md`を正本とする。T1 Slice AはPR #153でmainへ反映済みである。今回の明示指示によりP1 Slice Cを本実装PRで検証し、H3の数値budget、R1、bake共通preflightは後続のまま維持する。
...
本実装PRはSlice Cだけを扱う。既存Part 1件の`layerIds`差し替えUI、H2=L1検証、read-only inspection、H2違反だけを割当前に止める狭いbake refusal、History・保存roundtripを追加する。T1のschema・scheduler・export契約を変更せず、有限値・座標・参照全体・資源上限をまとめるbake共通preflight、R1、数値上限、event編集、onion skin、frame alignmentは後続へ残す。
