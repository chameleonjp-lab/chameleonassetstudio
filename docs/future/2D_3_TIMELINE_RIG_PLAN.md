...
状態: `T1 Slice A implementation in review / R1 + P1 accepted / formal B0 deferred / H3 numeric budget pending`
...
merge後の人間判断はH1=E1、H2=L1、H3=M1である。E1 / L1の意味は§6、M1の再現手順は`2D_3_H3_MEASUREMENT_PROTOCOL.md`を正本とする。T1 Slice Aは本実装PRで検証し、H3の数値budgetとR1 / P1製品実装は未承認のまま維持する。
...
本実装PRはSlice Aだけを扱う。TimelineにはFrame duration入力、合計時間、保存済みeventの発火確認を追加するが、eventの作成・編集、onion skin、frame alignmentはSlice Dへ残す。
