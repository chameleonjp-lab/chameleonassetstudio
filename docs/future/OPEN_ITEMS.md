...
現在の正本: `2D_COMPLETION_ROADMAP.md`
旧計画・関連文書: `POST_PHASE17_IMPLEMENTATION_PLAN.md`

Phase 0〜17（+ 15.5）は実装済み・自動テスト通過済み。この文書は、**今すぐ実装しないが今後検討すべき項目**を一覧化する。今後の優先順は `2D_COMPLETION_ROADMAP.md` を正本とし、旧 Phase 22〜28 は 2D Pro Gate 待ちである。着手前には必要な人間確認を挟む（`docs/future/FABLELESS_DEVELOPMENT_GUIDE.md` 5 章）。
...
- 左右反転（通常反転 = `LayerTransform.scale.x` 符号反転、および左右反転コピー = アセット全体を反転した新規アセット生成 `flipCopyAsset` はいずれも実装済み。残: リグ編集データ `rigAnimations` / part `bindPose` / `rotationLimit` の反転は将来スライス。詳細は `docs/future/FLIP_DESIGN.md`）
- フレーム別の当たり判定編集
...
## 4. 3D Asset Preparation Mode（2D Pro Gate 待ち）

`docs/future/THREE_D_ASSET_PREPARATION_REQUIREMENTS.md` は旧計画として残す。2D Pro Gate を人間が承認した後、`2D_COMPLETION_ROADMAP.md` の `3D-0`〜`3D-6` として再開する。生成 AI の実接続は最後。
...
これらはいずれも現行の `asset.json` / `.casproj` / export ZIP / 座標系を壊さない範囲で、`2D_COMPLETION_ROADMAP.md` の順番と品質 gate に沿って進める。旧 Phase 文書は履歴参照としてだけ使う。
