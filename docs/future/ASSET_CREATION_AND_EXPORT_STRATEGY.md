...
文書種別: アセット作成・編集・修正・検品・書き出し方針（背景・旧方針）
基礎要件: `docs/REQUIREMENTS_SPECIFICATION.md`, `docs/IMPLEMENTATION_PLAN.md`
現在の2D完成仕様: `2D_COMPLETE_PRODUCT_SPEC.md`, `2D_EXPORT_COMPATIBILITY_MATRIX.md`, `2D_COMPLETION_ROADMAP.md`
旧計画・関連文書: `POST_PHASE17_REQUIREMENTS.md`, `POST_PHASE17_IMPLEMENTATION_PLAN.md`, `THREE_D_ASSET_PREPARATION_REQUIREMENTS.md`, `PRODUCT_DIRECTION_2D_TO_3D.md`, `DECISION_LOG.md`
...
この文書は、上記の判断の背景を Claude Code、Codex、Opus 4.8、人間レビューの間で共有するための記録である。実装者は、この文書を理由に既存 Phase 1〜17 の完了条件を勝手に広げてはいけない。2D Pro の実装へ進む場合は、`2D_COMPLETE_PRODUCT_SPEC.md` と `2D_COMPLETION_ROADMAP.md` を先に確認する。
...
| `docs/future/POST_PHASE17_IMPLEMENTATION_PLAN.md` | Phase 19〜21 は 2D 制作体験、書き出し品質、effect を優先する旧計画。今後の順番は `2D_COMPLETION_ROADMAP.md` を優先し、3D は 2D Pro Gate 後に `3D-0` から始める |
| `docs/future/THREE_D_ASSET_PREPARATION_REQUIREMENTS.md` | 3D は生成 AI から始めず、GLB / glTF の読み込み、表示、検品、軽量化、メタデータ、書き出しから始める |
| `docs/future/FABLELESS_DEVELOPMENT_GUIDE.md` | 開発モードとレビューの詳細は `docs/DEVELOPMENT_MODES.md` を正本とし、Codex Fallback Mode を含む運用ルールを補う |
...
| GLB / glTF を読み込む | 3D 素材を検品して整える | 2D Pro Gate 後の `3D-0` 以降 |
...
1. `2D_COMPLETE_PRODUCT_SPEC.md` と `2D_ASSET_DATA_CONTRACT.md` で、完成条件と座標・保存の契約を先に確認する。
2. `2D_COMPLETION_ROADMAP.md` の 2D-1〜2 に従い、新規作成、テンプレート、取り込み、修正、派生素材を段階的に設計する。
3. 2D-3 でゲーム用情報と型別の検査を整える。
4. 2D-4〜5 で Export Preset と対象別の手動 import 検証を一つずつ進める。
5. Unity / Godot / RPG Maker / Blender は、まず import-notes から始め、`2D_EXPORT_COMPATIBILITY_MATRIX.md` の条件を満たしたものだけを verified とする。
6. 3D は 2D Pro Gate を人間が承認した後に `3D-0` から始める。
