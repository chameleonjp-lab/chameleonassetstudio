...
この文書は判断候補を提示するdocs-only監査である。推奨組み合わせが人間判断でacceptedになるまで、型、schema、保存transaction、Family / Variant UI、batch実装を開始しない。
...
V1のfingerprintは表示名や`updatedAt`だけに依存せず、recipeが読むAsset構造と対象Blobの決定的hashから作る。fingerprint不一致を自動競合解消に使わず、更新可否と警告の根拠にだけ使う。
