...
Slice C実装基準main: `378d6117060dd5933cbabfecd3ed4edbeebc6c11`（PR #120 merge）
Slice D実装基準main: `2216f0c7e76718824e65009c5100b62a31717c42`（PR #121 merge）
...
Slice CはPR #121でmainへmerge済みである。次を同じ安全境界で実装し、merge後reviewは`BLOCKER 0 / MUST 0 / SHOULD 1 / NOTE 5`だった。SHOULD-Aとmobile NOTE-3はSlice Dへ持ち越した。

- Project内のFamily作成、manual member登録、linked mirror / 1 layer・1 edit Blob限定palette作成、detach、Family解除、variant Asset削除をEditorへ配線する。既存の独立copy / 独立左右反転copyはstandaloneのままと表示する。
...
- 準備中はtarget別progressと取消を表示する。取消要求は処理中targetを閉じた後に止まり、準備済み出力を破棄する。原子transaction開始後は取消せず、失敗、容量不足、CAS競合、transaction abortのいずれでも全targetとHistoryを無変更にする。
- 選択targetだけを`saveAssetBatchRevision`で1 transactionへ確定し、1 History entryのUndo / Redoにも同じAPIを使う。初回Blob変更はtargetごとの復旧点を作るが、同じ履歴の再生では復旧点を追加せず、3件上限から初回復旧点を押し出さない。
- `.casproj`退避導線と容量warningをpreview内に置く。source Blob、recipe対象外、除外target、standalone Asset、product exportを変更しない。
- Slice C reviewのSHOULD-Aとして、Family状態検査をProperties表示中の選択Familyだけへ限定した。NOTE-3として、手動調整上書き確認をiPhone SE級touch E2Eで44px以上と検証する。
- unit testで0 / 17件、重複Asset、直列取消、linked Family更新、preview除外、容量warning、snapshot非evictを確認する。browser E2Eでcanvasの除外・Undo / Redo・reload、2 Asset paletteのsource不変・snapshot、2 target transaction rollback、touch progress / 取消 / 横overflowなしを確認する。
- Project / Asset schema version、migration、IndexedDB version / store / index、`.casproj` ZIP配置、product export ZIP構成、dependenciesは変更しない。

各sliceは最新mainからbranchを作り、前sliceのmerge後に開始する。契約レーンがopenの間、別PRでFamily / Variantの将来形式を推測して実装しない。

## 6. 受け入れ条件

### contract / fixture（Slice A、Slice Cで回帰確認）
...
- **Slice B / D:** 0件、17件以上、欠落Asset、欠落Blob、validation失敗、容量不足、transaction abortを理由付きで拒否し、全targetを無変更にする。
- **Slice D:** previewで除外したtargetを変更しない。
- **Slice B / C / D:** 1回のUndoで対象Asset / Blob / Project要約が戻り、Redoで再適用する。途中失敗でHistoryを積まない。
- **Slice C / D:** source Blob、recipe / batch対象外、standalone Asset、export ZIP構成を変更しない。
- **Slice C / D:** reload後に確定状態が維持され、破壊的Blob変更はsnapshotから復旧できる。

### E2E / mobile

- **Slice C:** Family作成、linked / manual variant作成、stale表示、preview、Familyから外す操作、refresh、Undo / Redo、reloadを通す。
- **Slice D:** paletteとcanvas resizeの複数target batchで対象別preview、warning、取消、原子的失敗を通す。
- **Slice C:** touch contextとiPhone SE級viewportでFamily操作、改行不能な長い名前、warningへ到達でき、rootとProperties内部に横スクロールやhover依存がない。
- **Slice D:** touch contextとiPhone SE級viewportでtarget選択、progress、取消へ到達できる。
