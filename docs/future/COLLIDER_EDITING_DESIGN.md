...
今回の第2段階では、GameDataPanel の当たり判定一覧から選択中の rect / circle を上下左右ボタンで移動する操作を実装済み。スナップ ON では grid size 分、スナップ OFF では 1px 分だけ移動し、rect は x / y のみ、circle は x / y のみを更新する。

canvas 上での rect / circle ドラッグ移動、rect リサイズ、circle radius 変更、hover hit testing、resize handle、polygon / frame-specific / 3D collider は引き続き未実装。選択状態は保存データに入れず、asset.json / .casproj / export ZIP / schema / version も変更しない。
