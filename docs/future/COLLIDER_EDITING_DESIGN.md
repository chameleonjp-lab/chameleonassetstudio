...
Phase 19-C の第1段階は「表示・選択・凡例・パネル同期」までに限定した。GameDataPanel の一覧から rect / circle 判定を選択し、選択行とキャンバス上の判定を同期して強調表示する。用途色の凡例と一覧スワッチを追加し、sensor はキャンバス描画で破線、UI スワッチで縞表示にして色だけに依存しない。

Phase 19-C の第2段階では、選択中の rect / circle 判定を GameDataPanel の上下左右ボタンで移動する操作を実装済み。snap OFF は 1px、snap ON は現在の grid size 分を移動量とし、rect は x / y のみ、circle は x / y のみ更新する。visible: false の判定は移動対象外にする。

canvas 上での rect / circle ドラッグ移動、rect リサイズ、circle radius 変更、hover hit testing、resize handle、polygon / frame-specific / 3D collider は未実装として残す。選択状態は保存データに入れず、asset.json / .casproj / export ZIP / schema / version も変更しない。
