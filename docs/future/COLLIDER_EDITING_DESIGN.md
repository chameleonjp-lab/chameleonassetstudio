...
Group 13 O1 Slice B / ADR-0024は、次を正式契約とする。

- `Frame.colliderOverrides?`は順序に意味を持たない配列で、各entryは`colliderId`でAsset共通colliderを参照する。rect / circleのgeometryを保存する場合はshapeに一致する完全形とし、`visible`だけのentryも許可する。
- entryにないgeometryまたは`visible`はAsset共通値へfield単位でfallbackする。`id` / `name` / `purpose` / `shape`は共通値だけを使う。不在と空配列は共通値だけを使い、最後の解除時はfield自体を省略する。
...
過去の設計候補として、最初に入れる場合は`points`をアセット座標の絶対座標、最小3点、debug drawとraw data exportまでに限定する案がある。ただしP1を採用した現在は未採用であり、2D Pro Gateまで`unsupported`を維持する。再開には別の設計レビューと人間確認が必要である。
...
ADR-0015 / ADR-0024に従い、`Frame.colliderOverrides?`をoptional・additiveに追加するO1採用範囲ではAsset `0.2.0`を維持し、migrationを追加しない。Slice Bで正確な永続表現を固定し、TypeScript型とJSON Schema、field不在の旧data roundtrip、未知field保持、保存失敗時rollback、Atlas / product ZIPの理由付き拒否はSlice Cの同じ実装Gateで検証してmainへ反映した。
...
### 2026-07 実装範囲メモ（表示・選択・凡例）

Phase 19-C の最初の実装は「表示・選択・凡例・パネル同期」までに限定した。GameDataPanel の一覧から rect / circle 判定を選択し、選択行とキャンバス上の判定を同期して強調表示する。用途色の凡例と一覧スワッチを追加し、sensor はキャンバス描画で破線、UI スワッチで縞表示にして色だけに依存しない。

この段階では canvas 上での rect / circle ドラッグ移動、rect リサイズ、circle radius 変更、hover hit testing、resize handle、polygon / frame-specific / 3D collider は実装しなかった。選択状態は保存データに入れず、asset.json / .casproj / export ZIP / schema / version も変更していない。

### 2026-07 実装範囲メモ（判定ツールでのキャンバス直接編集）

続く実装 PR で、専用の「判定」ツールを追加し、rect / circle 判定をキャンバス上で選択・ドラッグ移動・リサイズできるようにした。

実装した内容:

- ツールバーに「判定」ツールを追加する（`CanvasTool` に `'collider'` を追加。既存 `select` ツールの挙動は変えない）。
- 本体ヒットテスト（`hitTestColliders`）: 選択中の判定を最優先、次に配列の後ろ（描画で上）から順に判定する。`visible: false` は対象外。
- ハンドルヒットテスト（`hitTestColliderHandle`）: 選択中の判定にのみ、画面座標で許容 10px（既存アンカードラッグと同じ基準）のハンドルを判定する。rect は四隅（`nw` / `ne` / `sw` / `se`）、circle は中心右 `(x + radius, y)` の 1 個（`radius`）。
- 移動（`moveColliderBy`）: ドラッグ開始時の座標を基準に world 座標の移動量を加える。rect は `x` / `y` のみ、circle は `x` / `y` のみを更新し、サイズは変えない。
- リサイズ（`resizeColliderRect`）: つかんだ隅を新しい位置へ動かし、対角の隅は固定する。最小サイズ 1px にクランプし、対角を越えて反転はしない。
- 半径変更（`resizeColliderRadius`）: 中心からポインタまでの距離を新しい半径にする。中心は変えない。最小半径は 1px。
- グリッド / スナップは既存の丸め規則（スナップ ON は grid size、OFF は整数）をそのまま使う。
- 更新は既存の `updateCollider`（`src/core/model/assetOps.ts`）を通し、`updatedAt` の touch を含む。Undo / Redo は既存の `onCommitAsset(before, next)` 経路にそのまま乗せ、ラベルは「判定移動」「判定リサイズ」「判定半径変更」を使う。
- 選択中判定のハンドル表示は判定ツール使用中のみ描画し、既存の選択 halo 描画の後に描く。

引き続き対象外（このメモの範囲外）:

- polygon 判定。
- フレーム別（frame-specific）判定。
- 3D collider との統合。
- ハンドルの hover 強調（選択前のマウスオーバー表示）。キャンバス上のハンドルは選択後にのみ表示し、hover 専用の見た目変化は追加していない。
- `asset.json` / `.casproj` / export ZIP / JSON Schema / TypeScript の `Collider` 型 / version の変更。選択状態・ドラッグ状態は引き続き UI state のみで、保存データには含まれない。
