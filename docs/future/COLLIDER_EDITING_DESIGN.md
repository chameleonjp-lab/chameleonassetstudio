...
今回は canvas 上での rect / circle ドラッグ移動、rect リサイズ、circle radius 変更、hover hit testing、resize handle、polygon / frame-specific / 3D collider は実装しない。選択状態は保存データに入れず、asset.json / .casproj / export ZIP / schema / version も変更しない。
