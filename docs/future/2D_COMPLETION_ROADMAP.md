...
| 11 | `2D-2-IMPORT-GATE` + `2D-2-IMPORT-OPTIONAL` + `2D-2-AI-BOUNDARY` | PNG / JPEG / WebP、連番、sprite sheet、tileset、既知atlas bundleの取り込みと、任意形式・AI境界のADR判断を行う。 | Fable5または人間判断 + Codex実装 / docs | 10 | 判断必須項目はADR後のみ | in progress | 契約監査PR #124、Slice A PR #125（merge `7018984` / CI Run #404全成功）、Slice B PR #126（merge `5a1663b` / CI Run #407全成功）はmainへmerge済み。PR #126の独立read-only reviewは`BLOCKER 0 / MUST 0`だがGitHub上のreview記録は0件のためOpus review完了とは扱わない。現在はSlice C（連番・手動格子sheet・共通preview・quarantine）を進め、以後D→Eを直列で進める。`2D-2-IMPORT-OPTIONAL` はGate後へ移動しない。 |
