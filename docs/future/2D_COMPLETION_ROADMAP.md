...
| `2D-1B-CASPROJ` | 必須 | `.casproj` の段階的検査と migration を実装する。 | implementation completed / Draft PR #88 merge pending。CI Run #258 success。契約は`2D_1B_CASPROJ_PLAN.md`、証拠は`2D_1B_CASPROJ_REPORT.md`を正本とする。 |
...
| v0.1.0 fixture、roundtrip、quarantine | `2D-1B-CASPROJ` | implementation completed / merge pending | Draft PR #88で段階import、文書間整合、migration結果表示、canonical保存前検査、import後の再exportを実装し、CI Run #258が成功した。数量上限は`2D-1B-INPUT-SAFETY`で扱う。 |
