...
| 16 | `2D-4-PACKAGE` + `2D-4-PREFLIGHT` + `2D-4-GENERIC-WEB` | 人間が契約を採択した後に、generic manifest、sidecar、README、import notes、verification record、preflight、Generic Web / Canvas 2D fixtureを実装する。 | Codex + Opus 4.8 必須 review | 15 | 2D-6 と並行可 | 15後 | docs-only契約監査はPR #240 / merge `b5401529a552c38147d308d7209ad8483ffd85c4`でmainへ反映済み。これはGate Aの完了であり仕様採用ではない。契約は`proposed / human-decision-pending`、製品実装は`not-started / unverified`、進捗17/27を維持。正本は`docs/future/2D_4_PACKAGE_PREFLIGHT_GENERIC_WEB_PLAN.md`。 |
...
この割当はproposal-onlyである。Group 16の`G16-C1`〜`G16-C3`、後続UI、構造整理の具体契約は、各work packageで人間が採用するまで製品実装へ使わない。27工程の分母、17/27の完了数、Group 16〜23の順序は変更しない。
