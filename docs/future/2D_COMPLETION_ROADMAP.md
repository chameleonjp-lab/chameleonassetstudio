...
Group 12の詳細契約は`2D_3_TIMELINE_RIG_PLAN.md`を正本とする。T1 / R1 / P1の中核は2026-07-22に人間承認され、ADR-0021〜0023へ固定した。派生exportの初期挙動H1、Layer所属と空集合H2、rig bake資源上限H3は未決定であり、3件の人間判断とH3のiPhone Safari測定が揃うまで製品実装を開始しない。ADR-0010のcollider override / polygonはGroup 13のまま維持する。
...
| 12 | `2D-3-TIMELINE` + `2D-3-RIG` | frame animation、onion skin、可変時間、loop、event、part parent、pivot、bind pose、rotation / scale、part replace、簡易rig、frame bake、rig flipを実装する。 | Codex。新しいデータ意味はFable5または人間判断 | 8B、関連2D-1A契約 | 2D-2 と条件付き並行可 | contract audit | `main@236571c`で3担当のread-only監査を開始し、T1 / R1 / P1の中核をADR-0021〜0023へ固定した。H1派生export、H2 Layer所属・空集合、H3 bake上限は人間判断待ち。製品実装は未着手で、正本は`2D_3_TIMELINE_RIG_PLAN.md`。ADR-0010のcollider override / polygonはgroup 13に残す。 |
