...
| 8A | `2D-2-PROJECT` + `2D-2-CREATE` | Project内の複数Asset / Family / Variant管理と、空キャンバス、template、複製、図形、パーツ、文字からの作成を完成させる。 | Codex + Opus 4.8 review | 7 | 契約が重ならない 8B 準備のみ可 | C-slice completed / overall partial | `C` accepted。契約PR #93と実装PR #94はmerge済み。独立Asset管理、要約同期、安全なcopyは完了。Family / Variantと残るCREATE項目は別契約まで保留し、PR #55のpartial / provisionalも維持する。 |
| 8B | `2D-3-TYPE-PROFILES` + `2D-3-INSPECT` | asset type profile と理由付き検査を先に固定し、以後の作成・game data・exportの不足表示を揃える。 | Codex。新しいデータ意味はFable5または人間判断 | 7 | 8A と条件付き並行可 | current / contract audit | 現行6種とUI / iconの扱い、profile厳格度、検査結果の強制境界を判断待ち。判断前はdocs / test設計のみ。 |
...
PR #53・PR #55は先行merge済みのためrevertしない。PR #53の`2D-1B-REVISION`該当範囲はPR #70、#71、#72、`2D-1B-LAYERS`該当範囲はPR #76、#77、#78、`2D-1B-RECOVERY`該当範囲はPR #80、#81、`2D-1B-CAPACITY`該当範囲はPR #87、`2D-1B-CASPROJ`該当範囲はPR #88、`2D-1B-INPUT-SAFETY`該当範囲はPR #89、#90で再監査・補修を完了した。`2D-1B-STORAGE-CROSS-REVIEW`はPR #84、#85による補修後に`BLOCKER 0 / MUST 0`で完了し、Gate実装PR #91 / CI Run #269、Opus 4.8 review問題なし、人間確認により`2D-1B-GATE`も正式完了した。`2D-2-PROJECT + 2D-2-CREATE`のaccepted C-sliceはPR #93、#94で完了し、Family / Variantと残るCREATE項目はpartialとして維持する。現在は`2D-3-TYPE-PROFILES + 2D-3-INSPECT`の契約監査である。
