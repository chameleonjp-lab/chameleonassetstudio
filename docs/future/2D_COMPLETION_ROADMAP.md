...
| 8A | `2D-2-PROJECT` + `2D-2-CREATE` | Project内の複数Asset / Family / Variant管理と、空キャンバス、template、複製、図形、パーツ、文字からの作成を完成させる。 | Codex + Opus 4.8 review | 7 | 契約が重ならない 8B 準備のみ可 | accepted slices completed / overall partial | `C` acceptedの独立Asset管理はPR #93、#94で完了。残るCREATE `A+B+X` sliceはPR #98〜#101で完了した。Family / Variantは別契約までpartialを維持する。 |
...
| 10 | `2D-2-VARIANT` + `2D-2-BATCH` | linked variant、独立copy、装備差分、解像度差分、手動調整保護、一括変更、対象preview、部分失敗、Undoを実装する。 | Codex + Opus 4.8 review | 9の現在実装可能範囲完了 | 契約が重ならない 2D-3 と条件付き並行可 | contract audit / human decision pending | `docs/future/2D_2_VARIANT_BATCH_PLAN.md`で現行型、schema、保存、History、snapshot、`.casproj`を監査した。推奨`F1+C1+V1+T1+B1+O1+H1+L1`のaccept前はproduct implementationを開始しない。 |
