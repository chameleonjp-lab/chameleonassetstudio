...
| 19 | `2D-5-UNITY` + `2D-5-GODOT` | Unity 2D と Godot 2D を対象バージョン・素材種別ごとに fixture で検証する。 | Codex + Opus 4.8 必須 review + 必要に応じて人間実機確認 | 18 | target PR 最大2本まで可 | 18後 | 契約handoff中。`human-decision-pending / implementation-not-started`。正本は `docs/future/2D_5_UNITY_GODOT_PLAN.md`。G19-C1〜C3採用前はproduct code、target fixture、CI、dependencyを変更しない。 |
