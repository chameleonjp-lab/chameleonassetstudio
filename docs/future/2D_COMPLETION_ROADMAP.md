...
| 17 | `2D-4-PIXIJS` + `2D-4-PHASER` + `2D-4-DOCS` | PixiJS / Phaser fixtureと実行確認、export手順、import notes、既知制限、検証記録を docs に反映する。 | Codex + Opus 4.8 必須 review | 16 | 2D-5手順準備と並行可 | 16後 | `Gate A: docs-only handoff PR #243 / merge 9d33306fd7ad340065dac22548c218a8c4500383`、merge後closeout PR #244 / merge `5a05100f66c8e6e84854028995a3ae31ba1c36a4`。契約状態は`accepted / implementing`であり、2026-08-14の`G17-C1 A + G17-C2 A + G17-C3 A`採用後に実装を開始した。正本は`docs/future/2D_4_PIXIJS_PHASER_PLAN.md`。 |
...
`G17-C1 A + G17-C2 A + G17-C3 A`をacceptedとし、基準main `ea7f3964cf7f267622c23d386d8c59cacc4d117c`から専用fixture、engine別E2E、CI証拠、docsの実装を進める。独立検証とmergeまでは完了数を17/27として扱う。
