...
| 16 | `2D-4-PACKAGE` + `2D-4-PREFLIGHT` + `2D-4-GENERIC-WEB` | ユーザー採用済みのgeneric manifest、sidecar、README、import notes、verification record、preflight、Generic Web / Canvas 2D fixtureを実装する。 | Codex + Opus 4.8 必須 review | 15 | 2D-6 と並行可 | 15後 | `accepted: G16-C1 A + G16-C2 A + G16-C3 A / implementing / CI-pending / independently-verified-pending`。PR #240でGate A、2026-08-13の明示判断でGate Bを完了。進捗17/27は実装PRの検証・mergeまで維持する。 |
