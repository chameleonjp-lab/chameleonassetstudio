...
| 19 | `2D-5-UNITY` + `2D-5-GODOT` | Unity 2D と Godot 2D を対象バージョン・素材種別ごとに fixture で検証する。 | Codex + Opus 4.8 必須 review + 必要に応じて人間実機確認 | 18 | target PR 最大2本まで可 | 18後 | `accepted: G19-C1 A（Unity 6000.3.21f1 / Godot 4.7.1-stable） + G19-C2 A + G19-C3 A / implementing / verification-unverified`。candidate fixture、静的closure test、import notesを開始。runtime未完了のため進捗は18/27。正本は `docs/future/2D_5_UNITY_GODOT_PLAN.md`。 |
