...
| 8A | `2D-2-PROJECT` + `2D-2-CREATE` | Project内の複数Asset / Family / Variant管理と、空キャンバス、template、複製、図形、パーツ、文字からの作成を完成させる。 | Codex + Opus 4.8 review | 7 | 契約が重ならない 8B 準備のみ可 | accepted slices completed / overall partial | `C` acceptedの独立Asset管理はPR #93、#94で完了。残るCREATE `A+B+X` sliceはPR #98〜#101で完了した。Family / VariantはGroup 10契約がacceptedとなり、Slice Aを実装中。管理UI完了までoverall partialを維持する。 |
...
| 10 | `2D-2-VARIANT` + `2D-2-BATCH` | linked variant、独立copy、装備差分、解像度差分、手動調整保護、一括変更、対象preview、部分失敗、Undoを実装する。 | Codex + Opus 4.8 review | 9の現在実装可能範囲完了 | 契約が重ならない 2D-3 と条件付き並行可 | accepted / Slice A in progress | 契約監査はPR #116でmainへmerge済み。推奨`F1+C1+V1+T1+B1+O1+H1+L1`は2026-07-17に人間accepted。正本§5のA→B→C→D直列順に従い、現在はoptional Family / Variantデータ契約のSlice Aを実装中。 |
...
PR #53・PR #55は先行merge済みのためrevertしない。PR #53の`2D-1B-REVISION`該当範囲はPR #70、#71、#72、`2D-1B-LAYERS`該当範囲はPR #76、#77、#78、`2D-1B-RECOVERY`該当範囲はPR #80、#81、`2D-1B-CAPACITY`該当範囲はPR #87、`2D-1B-CASPROJ`該当範囲はPR #88、`2D-1B-INPUT-SAFETY`該当範囲はPR #89、#90で再監査・補修を完了した。`2D-1B-STORAGE-CROSS-REVIEW`はPR #84、#85による補修後に`BLOCKER 0 / MUST 0`で完了し、Gate実装PR #91 / CI Run #269、Opus 4.8 review問題なし、人間確認により`2D-1B-GATE`も正式完了した。`2D-2-PROJECT + 2D-2-CREATE`はPR #93、#94とPR #98〜#101でaccepted sliceを完了し、Family / Variantだけをpartialとして維持する。`2D-3-TYPE-PROFILES + 2D-3-INSPECT`はPR #95〜#97で完了した。`2D-2-RASTER + 2D-2-REPAIR`はPR #113で複数Layer align / distribute、PR #115でAsset canvas resize / game data追従をmainへmergeした。PR #115 final head `63d92a1dd56679edc047fa62ab450ad743323cfc`のCI Run #384は全成功、merge commitは`1838f58918a2958f9ebce2f8379f87a45fb17c26`。GitHub上のreview / comment記録は0件で、Opus review完了とは扱わない。frameずれ修正は`2D-3-TIMELINE`後へ保留した。次の`2D-2-VARIANT + 2D-2-BATCH`契約監査はPR #116でmainへmerge済み、推奨組み合わせはacceptedとなり、現在はSlice Aを実装中である。
...
| §4〜§5 | Project / Asset / Family / Variant、ID・名前・参照 | `2D-1A-LAYERS`, `2D-2-PROJECT` | Family / Variant契約はaccepted、Slice A実装中。管理UIを含む`2D-2-PROJECT`全体は未完了。 |
