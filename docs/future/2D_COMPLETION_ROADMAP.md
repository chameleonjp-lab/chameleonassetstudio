...
| `2D-2-IMPORT-OPTIONAL` | 判断必須 | SVG、GIF、APNG、Aseprite、PSD、OpenRaster、Kritaをeditable / rasterized / reference-only / unsupportedのどれにするか決める。ADR-0016で分類済み。ADR-0019のsource MIME / Asset 0.2.0契約補正を先行し、Slice E UI / decodeはそのmerge後に実装する。 |
...
| 11 | `2D-2-IMPORT-GATE` + `2D-2-IMPORT-OPTIONAL` + `2D-2-AI-BOUNDARY` | PNG / JPEG / WebP、連番、sprite sheet、tileset、既知atlas bundleの取り込みと、任意形式・AI境界のADR判断を行う。 | Fable5または人間判断 + Codex実装 / docs | 10 | 判断必須項目はADR後のみ | in progress | 契約監査PR #124、Slice A PR #125、Slice B PR #126、Slice C PR #127/#128、Slice D PR #129、Slice D closeout PR #133はmainへmerge済み。Slice Eは人間承認A / ADR-0019のsource MIME・Asset 0.2.0契約補正を先行し、通常import UI / rasterize / animated decodeをmerge後の別PRで直列実装する。`2D-2-IMPORT-OPTIONAL` はGate後へ移動しない。 |
