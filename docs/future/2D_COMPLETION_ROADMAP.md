...
| `2D-1B-GATE` | 必須 | fixtureと保存回帰テストを通し、2D-2 / 2D-3本実装を解禁する。 | completed。PR #91 / CI Run #269は全成功。2026-07-16にOpus 4.8 review問題なしが報告され、少なくともBLOCKER 0 / MUST 0として人間確認済み。 |
...
PR #53（`2D-1B-STORAGE`）は、詳細契約 Gate より先に main へマージされた先行実装である。revertせず、PR #70、#71、#72により`2D-1B-REVISION`、PR #76、#77、#78により`2D-1B-LAYERS`、PR #80、#81により`2D-1B-RECOVERY`、PR #87により`2D-1B-CAPACITY`、PR #88により`2D-1B-CASPROJ`、PR #89、#90により`2D-1B-INPUT-SAFETY`の該当範囲を再監査・補修した。PR #91 / CI Run #269、Opus 4.8 review、人間確認により`2D-1B-GATE`も正式完了し、PR #53全体のprovisional状態を解消した。対応付けは次の通り。
...
| 全保存回帰 Gate | `2D-1B-GATE` | completed | ADR-0015再監査、全保存回帰、横断SHOULD補修を実施し、PR #91最終head CI Run #269は全成功。2026-07-16にOpus 4.8 review問題なしが報告され、少なくともBLOCKER 0 / MUST 0として人間確認済み。 |

`2D-1B-GATE`の正式完了により、`2D-2-*`と`2D-3-*`は本ロードマップの正式キューに従って開始できる。新しいデータ意味は各判断Gateを通し、2D Pro Gateの人間承認までは3D / WebGPUを開始しない。
...
| `2D-2-IMPORT-OPTIONAL` | 判断必須 | SVG、GIF、APNG、Aseprite、PSD、OpenRaster、Kritaをeditable / rasterized / reference-only / unsupportedのどれにするか決める。ADR-0016で分類済み。ADR-0019 / PR #135のsource MIME / Asset 0.2.0契約補正、ADR-0020のSlice E UI / safety / decode、post-merge補修PR #144までmainへmerge済み。 |
...
| `2D-3-COLLIDER-OVERRIDE` | completed | asset共通判定を基準とするFrame別上書き規則を実装する。Animation別上書きは採用せず、必須へ変更しない。canonical契約はADR-0024 / Slice B / PR #218、製品実装と補修はPR #219 / #220、E2E待機安定化はPR #221でmainへ反映し、CI Run #669と固定head独立監査を成功した。 |
...
Opus 4.8を利用できないWorkでは、ユーザー提供の運用仕様に従い、未実施のOpus範囲を明記し、writer以外の複数担当による固定headの独立read-only reviewで代替してよい。代替結果と残リスクをPR本文または正本文書へ記録し、人間が確認してmergeを判断する。この場合はOpus review完了と表示しない。
...
| completed | `2D-3-COLLIDER-OVERRIDE` | Frame単位だけを採用し、Animation単位は採用しない。保存形式・fallback・検証・反転・export拒否はADR-0024 / PR #218で固定し、製品実装と補修はPR #219 / #220、E2E待機安定化はPR #221でmainへ反映済みである。 |
...
| 7 | `2D-1B-CASPROJ` + `2D-1B-INPUT-SAFETY` + `2D-1B-GATE` | `.casproj` 段階的検査、migration、不正入力隔離、保存回帰 Gate 証拠を揃え、2D-2 / 2D-3本実装を解禁する。 | Codex + Opus 4.8 必須 review + 人間確認 | 6 | Gate正式完了まで 2D-2 / 2D-3 本実装不可 | completed | CASPROJ、INPUT-SAFETY、GATEはcompleted。PR #91 / CI Run #269、Opus 4.8 review問題なし、人間確認済み。 |
| 8A | `2D-2-PROJECT` + `2D-2-CREATE` | Project内の複数Asset / Family / Variant管理と、空キャンバス、template、複製、図形、パーツ、文字からの作成を完成させる。 | Codex + Opus 4.8 review | 7 | 契約が重ならない 8B 準備のみ可 | completed | `C` acceptedの独立Asset管理はPR #93、#94、残るCREATE `A+B+X` sliceはPR #98〜#101で完了した。Family / Variant永続契約のSlice AはPR #117、管理UIのSlice CはPR #121、一括変更のSlice DはPR #122で完了した。Group 10の遡及Opus review（`BLOCKER 0 / MUST 0`）完了によりoverall partialを解消した。 |
| 8B | `2D-3-TYPE-PROFILES` + `2D-3-INSPECT` | asset type profile と理由付き検査を先に固定し、以後の作成・game data・exportの不足表示を揃える。 | Codex。新しいデータ意味はFable5または人間判断 | 7 | 8A と条件付き並行可 | completed | `A+B+X` accepted。契約PR #95、#96、実装PR #97で現行6種のbalanced profileとread-only inspectorを完了した。CI Run #293、Opus review、人間確認も完了。 |
| 9 | `2D-2-RASTER` + `2D-2-REPAIR` | brush、eraser、fill、selection、shape、text、transform、align、grid、snap、透明化、trim、余白、resize、palette、色違い、flip、outline、frameずれ修正を完成させる。 | Codex + Opus 4.8 review | 8A | 8B / 2D-6 と条件付き並行可 | completed | `A+X+P+M` accepted。PR #102〜#110でRaster Slice 1とLayer Repair Slice 2、PR #113で複数Layer align / distribute、PR #115でcanvas resize / game data追従をmainへmergeした。PR #115 final headのCI Run #384は全成功。frameずれ修正はGroup 12 D4としてPR #209 final head `16f5ae2`、merge `5e25d0d`、CI Run #623全成功、固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`を経て完了した。 |
...
| 13 | `2D-3-GAME-DATA` + `2D-3-COLLIDER-OVERRIDE` + `2D-3-POLYGON` | origin、anchor、rect / circle、tile collision、background、gimmick、effect情報を編集し、frame別上書きとpolygonは判断後に扱う。 | Fable5または人間判断 + Codex実装 | 12、2D-1A-MOTION | 判断必須項目はADR後のみ | completed | PR #215のdocs-only監査とPR #216の`G1+O1+P1`採用記録をmainへ反映した。G1 Slice AはPR #217 final head `e28e4de4`、merge `bc48487e`、CI Run #650全成功、固定headの3方向独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として完了した。O1のcanonical data、fallback、検証、UI、保存、変換、export拒否を固定したSlice BはPR #218、製品実装と補修はPR #219 / #220、E2E待機安定化はPR #221 final head `45a41a19`、merge `65df697e`でmainへ反映した。CI Run #669はUnit 842件、Chromium E2E 184件、H3 1件、Pages公開・閉鎖各1件を成功し、fixed-head reviewは`BLOCKER 0 / MUST 0 / SHOULD 0`だった。進捗は15/27で、polygonはP1により`unsupported`を維持する。正本は`docs/future/2D_3_GAME_DATA_PLAN.md`。 |
| 14 | `2D-3-PREVIEW` + `2D-3-IMPACT` | 接地、origin、anchor、collider、tile、背景、gimmick、effectのゲーム風previewと、source変更影響の列挙を実装する。 | Codex + Opus 4.8 必須 review | 13 | 2D-4 fixture設計と並行可 | completed | PR #225 / merge `cd3c3ffa`で初期実装、PR #226 / merge `280ab99d`でverification hardening、PR #227 final head `95c1e9de` / merge `217c695f`で最終補修をmainへ反映した。CI Run #689（Actions ID `31319970728`）は全job成功、Unit 870件、Chromium E2E 194件、failed / flaky / skipped / retry 0件。artifact ID `9039925430`、digest `sha256:2456b487e140a6ef3ada2aa8cec6f94ca15747581d69b651f6b527bebb5d0bde`を記録し、固定headの3方向read-only reviewも完了した。Group 14をcompleted、進捗を16/27とする。 |
| 15 | `2D-4-CORE` + `2D-4-SHEET` + `2D-4-SCALE` | 共通export core、決定的再出力、fixed grid sheet、packed atlas、trim、padding、multi-page、1x / 2x / 3x、scale、extrudeを固定する。 | Fable5契約固定 + Codex + Opus 4.8 必須 review | 9、14 | SHEET実装後にSCALEへ進む。 | accepted: G15-C1 A + G15-C2 A + G15-C3 A / implemented: 2D-4-CORE + 2D-4-SHEET + 2D-4-SCALE / CI-passed / independently-verified / merged / next: 2D-4-PACKAGE + 2D-4-PREFLIGHT + 2D-4-GENERIC-WEB | PR #229 / #231 / #232 / #233 / #235 / #236 / #238をmainへ反映した。PR #238 final head `c2dc87ee1506f0a68b283eb3accf09d78921011a`、merge `a6f15d0b8bff90323d363827be62804959a0e7b0`、CI Run #724（Actions ID `31518786336`）は全job成功し、SCALE evidence artifact `9112190870`（digest `sha256:00b3fb7b2b266c8ff4eb306d64d1a88b9f0ebfc1733054ea8cff0e0d3573a2e4`）とPlaywright artifactを記録した。親の固定head read-only確認は`BLOCKER 0 / MUST 0`で、既存Atlas `0.1.0`、legacy ZIP、現行の理由付き拒否は維持する。進捗は17/27とする。
| 16 | `2D-4-PACKAGE` + `2D-4-PREFLIGHT` + `2D-4-GENERIC-WEB` | ユーザー採用済みのgeneric manifest、sidecar、README、import notes、verification record、preflight、Generic Web / Canvas 2D fixtureを実装する。 | Codex + Opus 4.8 必須 review | 15 | 2D-6 と並行可 | 15後 | `accepted: G16-C1 A + G16-C2 A + G16-C3 A / implemented / CI-passed / independently-verified / merged / next: Group 17`。PR #240でGate A、PR #242 final head `6616ad30fdae7a05f98e0a0146ce69555bab1bfa`、merge `711bcec268d6e732a24c0c787c6054b41e415c27`、CI Run #765で完了した。進捗17/27とする。 |
...
| 22 | `2D-6-REFERENCE` + `2D-6-DOCS` + `2D-6-GATE-AUDIT` | 代表project、最終証拠、README / user guide / release checklist / docs整合、最終品質auditを通す。 | Codex docs / evidence + 人間実機確認 + Opus 4.8 audit | 21A〜21C、18〜20 | 最終auditのため並行不可 | 21C後 | `implemented-candidate / CI-passed / merged / runtime-verification-unverified`。PR #264（head `4d513fa7a105336f00a51fa2c0ede9ee5d339f17`、merge `15e252d8339361cd153ac784cba5752189ceeabd`）で静的台帳・文書入口監査を反映し、PR #272（head `6270e59abbb999d00d7c434ff66c76db5836b0fc`、merge `17d62c49792202ef411124df03e3809ded5f2d8c`）で代表IDの自動一体フローとGeneric Web package closureを追加した。Run #892（Actions ID `33248089842`、attempt 2）は全job成功、E2E 205件、H3 1件、Pages open / closed各1件。後続Draft PR #273でdocs＋Gate testの証拠台帳を同期し、Run #895（Actions ID `33347654457`）はclassify / build-and-test success、unit 87 files / 916 tests success、E2Eは変更分類によりskip。 Draft PR #274のhandoff-content head Run #898（Actions ID `33349026436`）もsuccess。続くbookkeeping head `b961c2086c92b18a0df6e24688bf7ae48d428567`のRun #901（Actions ID `33349818980`）もsuccessで、3方向read-only reviewはPASS。artifact内容の人間レビュー、初回レビュー、物理端末、Group 19 / 20 runtimeは未実施で、`independently-verified`へは昇格していない。進捗は18/27。 |
| 23 | `2D Pro Gate` | 全必須package、判断必須ADR、証拠、CI、実機、target fixture、Opus 4.8、PR #53 / #55再監査、人間承認を確認し、3D開始可否を判断する。 | Fable5が証拠整理、Opus 4.8最終review、人間が最終承認 | 22 | Gate中は新規実装しない | 22後 | PR #266で監査記録を、PR #267、PR #268、PR #269、PR #270でマージ後状態をmainへ反映し、PR #272で代表projectの自動証拠、Draft PR #273でhandoff台帳を追加した。 Draft PR #274でCI記録とartifact名を補正した。handoff-content Run #898とbookkeeping Run #901を分離記録し、3方向read-only reviewはPASSとなった。ただしartifact内容の人間レビュー、初回レビュー、物理端末、Group 19 / 20 runtime、人間承認は未完了で、2D Pro Gateは未完了。`candidate / gate-pending / runtime-verification-unverified`、18/27、3D停止を維持する。 |

PR #53・PR #55は先行merge済みのためrevertしない。PR #53の`2D-1B-*`再監査・補修はPR #70〜#91で完了し、`2D-1B-GATE`もCI Run #269、Opus 4.8 review、人間確認を経て正式完了した。`2D-2-PROJECT + 2D-2-CREATE`はPR #93〜#101、Family / VariantはPR #117〜#122、`2D-2-RASTER + 2D-2-REPAIR`はPR #102〜#115、group 11 importはPR #124〜#144で現在契約範囲を完了した。Group 12は契約監査PR #146、計測準備PR #147、Pages分離PR #148 / #149、T1 Slice A PR #153、P1 Slice C PR #154、R1 Slice B1 PR #157、D1 PR #201、D2 PR #204、D3 PR #206、D4契約PR #208、D4製品実装PR #209をmainへ反映した。D4の最終head `16f5ae2`に対するCI Run #623と固定headの3方向独立reviewは全成功した。D1〜D4の物理Safari確認は完了した。2026-08-02の人間判断でT1 / P1 / B1の物理Safari Gateも完了扱いとした。正式B0、H3数値budget、B2は`2D-6-PERFORMANCE`へ延期し、Group 12の完了条件から外した。Group 12をcloseoutし、Group 13の契約監査PR #215、`G1+O1+P1`採用記録PR #216、G1実装PR #217、O1契約PR #218、O1製品実装PR #219、補修PR #220、E2E待機安定化PR #221をmainへ反映してGroup 13をcloseoutした。Group 14 `2D-3-PREVIEW + 2D-3-IMPACT`は2026-08-09に`G14-P1 + G14-I1 + G14-U1`をacceptedとし、PR #225、PR #226、PR #227をmainへ反映した。CI Run #689と固定headの3方向read-only reviewを完了し、Group 14をcloseoutした。Group 15 `2D-4-CORE + 2D-4-SHEET + 2D-4-SCALE`の契約監査PR #229、採用記録PR #231、handoff同期PR #232、CORE実装PR #233、CORE closeout PR #235、SHEET実装PR #236をmainへ反映した。PR #236の最終head `d3a178b5614a00f72f1bd0e706b95e28d4c5189b`に対するCI Run #716（Actions ID `31510296044`）は全3 job成功し、SHEET artifact ID `9108792430`、CORE artifact ID `9108792006`、Playwright artifact ID `9108931876`を取得した。固定headの3方向read-only確認は`BLOCKER 0 / MUST 0`で、SHEETは`implemented / CI-passed / independently-verified / merged`とする。Group 15当時のmainは`a6f15d0b8bff90323d363827be62804959a0e7b0`であり、SCALEはPR #238でmainへmerge済み、distribution UIと物理iPhone Safariは後続範囲である。その後、Group 16はPR #242で実装・検証・merge済み、Group 17のdocs-only handoffはPR #243 / merge `9d33306fd7ad340065dac22548c218a8c4500383`、merge後closeoutはPR #244 / merge `5a05100f66c8e6e84854028995a3ae31ba1c36a4`、実装・検証PR #246 / merge `b79327857b2d9f9bfbc9a4c0a4a4566d1000a69a`でmainへ反映済みであり、G17-C1〜C3は完了した。Group 18は`G18-C1 A + G18-C2 A + G18-C3 A`を採用し、PR #248 / merge `3ab844d28d155a438dc8f10f8f9b22099a40093a`で実装・CI・独立確認まで完了した。PR #249 / merge `fcbf1cc9b7a1a9d0cdd588eaed59de3999bdcabb`でcloseout済みである。Group 19はG19-C1〜C3をacceptedし、PR #251（merge `e77a721ff3d479bea0f7475f0b0fbc296ce91595`、CI Run #805全job成功）でcandidate fixture・静的closure test・import notesをmainへ反映した。runtime検証は未完了で、進捗は18/27を維持する。

D1〜D4は`implemented / CI-passed / independently-verified / merged`で、指定3端末の物理Safari確認も完了した。T1 / P1 / B1の物理Safari Gateは2026-08-02の人間判断で完了扱いとする。B2は`deferred to 2D-6-PERFORMANCE / unimplemented / unmeasured`であり、Group 12はcompletedである。実行表は分割行を含む全27工程で、18工程が完了した。Group 13は`G1+O1+P1`をacceptedし、PR #215〜#221を経てcompletedとした。Group 14はone-sheet handoff、PR #225製品実装、PR #226 verification hardening、PR #227最終補修、CI Run #689、固定headの3方向read-only reviewを完了し、`implemented / CI-passed / independently-verified / merged / completed`とする。Group 15は契約監査PR #229、採用記録PR #231、handoff同期PR #232、CORE実装PR #233、CORE closeout PR #235、SHEET実装PR #236、SCALE実装PR #238をmainへ反映した。当時のGroup 15状態は`implemented: 2D-4-CORE + 2D-4-SHEET + 2D-4-SCALE / CI-passed / independently-verified / merged / next: 2D-4-PACKAGE + 2D-4-PREFLIGHT + 2D-4-GENERIC-WEB`であり、distribution UI、engine読込、物理Safariの確認は後続範囲だった。その後、Group 16はPR #242、Group 17はPR #246でmainへ反映済みである。Group 18はPR #248で実装・CI・独立確認・mergeまで完了し、進捗を18/27へ更新する。Group 19はUnity / Godotの採用済み契約に基づくcandidate実装をmainへ反映済みだが、runtime Gateは未完了である。Unity/Godot runtimeの未完了状態は維持する。PCが使用できないという人間判断を例外として、Group 20（RPG Maker MZ / helper gate）のdocs-only handoffを開始し、runtime実行とverified昇格は保留する。

本書に新しいアイデアがあっても、実装担当が独断で `asset.json`、`.casproj`、export ZIP、dependencies、3D、外部 API を変えてはいけない。

### 6.8 2026-08-13 五視点レビューの割当

プロダクトマネージャー、クリエイター、デザイナー、シニアエンジニア、敵対的視点のレビューは、`docs/future/2D_FIVE_PERSPECTIVE_REVIEW_ACTION_PLAN_2026-08-13.md`へ統合した。新しい必須工程は確定せず、次の既存Groupへ割り当てる。

| レビュー由来の内容 | 割当先 |
|---|---|
| package、preflight、Generic Web fixture | Group 16 |
| PixiJS、Phaser、関連docs | Group 17 |
| `candidate` / `verified` / 証拠形式 | Group 18 |
| Unity、Godot | Group 19 |
| RPG Maker MZ、helper採否 | Group 20 |
| distribution製品UI、375×667 product-path、端末入力 | Group 21A。Group 16完了後に継続並行で開始。 |
| 保存失敗、`.casproj`再開・再生成、offline | Group 21B。Group 16完了後に継続並行で開始。 |
| 性能、アクセシビリティ、安全性、Editor責務の段階分離 | Group 21C |
...
| §4 Generic Web / Canvas 2D / PixiJS / Phaser | P0 出力と fixture 実行確認 | `2D-4-GENERIC-WEB`, `2D-4-PIXIJS`, `2D-4-PHASER` | Generic Web、PixiJS 8.12.0専用fixture、Phaser 4.2.0専用fixtureはcompleted。verifiedは各fixture-local範囲に限る。 |
...
| §5 | IndexedDB、`.casproj`、保存状態、復旧、容量不足、壊れた入力 | `2D-1B-REVISION`, `2D-1B-RECOVERY`, `2D-1B-CAPACITY`, `2D-1B-CASPROJ`, `2D-1B-INPUT-SAFETY`, `2D-6-DEVICE-FLOW` | 2D-1B各実装、PR #91 / CI Run #269、Opus 4.8 review、人間確認までcompleted。2D-6-DEVICE-FLOWは未完了。 |
...
Group 20のマージ後、次の並行工程であるGroup 21A（`2D-6-DEVICE-FLOW` + `2D-6-INPUT`）を開始した。

今回の対象は、スマホ最小幅320×568、スマホ標準幅375×667、iPad幅768×1024で、作成・取り込み・編集画面・プロパティ・タイムライン・書き出し・ホーム復帰・再読み込み後の再開へ到達する自動検査である。入力欄の16px基準、主要ボタンの44px基準、ページ全体とキャンバスの`touch-action`分離も固定した。正本は`docs/future/2D_6_DEVICE_FLOW_CONTRACT.md`と`e2e/device-flow.spec.ts`に置く。
...
## Group 22 post-merge state (2026-08-27)

PR #264の静的監査・証拠台帳・文書入口変更は、head `4d513fa7a105336f00a51fa2c0ede9ee5d339f17`からmerge commit `15e252d8339361cd153ac784cba5752189ceeabd`としてmainへ反映された。CI Run #869（Actions ID `32953397489`）は全job成功し、Group 22 artifact `9601053891`（`sha256:d8d203289543a13157160bdc6461ea0744e1d591024fe0b32786dc3113465339`）を生成した。

状態は `implemented-candidate / CI-passed / merged / runtime-verification-unverified` とする。GitHub上のPR #264に独立review投稿は記録されていないため、今回のcloseoutは `independently-verified` を主張しない。stable manifestは引き続き `candidate` であり、代表projectの一体実行、初回利用者レビュー、物理端末、Group 19 / 20 runtimeは未実施である。

Group 23（`2D Pro Gate`）の監査記録はPR #266で、マージ後状態はPR #267、PR #268、PR #269、PR #270でmainへ反映済みである。ただしGate中は新規製品実装を行わず、人間の明示承認まで3Dを開始しない。完了数は18/27のままとする.

> **補正handoff（2026-08-31）:** Draft PR #274は、PR #273マージ後の証拠台帳を最新化した。handoff-content head `9a857574ba5eeffc6d87f242236d9127c3459f9a`のRun #898（Actions ID `33349026436`）はsuccess（unit 87 files / 916 tests、E2E skip）、Group 22 artifact `9742974357`を取得した。bookkeeping head `b961c2086c92b18a0df6e24688bf7ae48d428567`のRun #901（Actions ID `33349818980`）もsuccess（artifact `9743224756`）。3方向read-only reviewはPASSである。
