...
| `2D-1B-INPUT-SAFETY` | 必須 | ZIP、JSON、画像など不正入力の隔離と拒否を実装する。 | current。`B+X` accepted。監査・契約PR #89はmerge済み。Draft PR #90の実装head CI Run #264と完了報告head CI Run #265は全成功。正式完了はPR #90 merge後に同期する。 |
...
| 壊れた import 隔離 | `2D-1B-INPUT-SAFETY` | implemented in Draft PR #90 | ZIP / JSON / 画像を信頼しない入力として扱うB profile上限、理由付き拒否、quarantine、Asset / Layer batch原子性を実装し、CI Run #264、#265が成功した。正式完了はmerge後に同期する。 |
...
PR #53・PR #55 は先行 merge 済みのため revert しない。PR #53の`2D-1B-REVISION`該当範囲はPR #70、#71、#72、`2D-1B-LAYERS`該当範囲はPR #76、#77、#78、`2D-1B-RECOVERY`該当範囲はPR #80、#81、`2D-1B-CAPACITY`該当範囲はPR #87、`2D-1B-CASPROJ`該当範囲はPR #88で再監査・補修を完了した。`2D-1B-STORAGE-CROSS-REVIEW`はPR #84、#85による補修後に`BLOCKER 0 / MUST 0`で完了し、現在は`2D-1B-INPUT-SAFETY`である。残る`2D-1B-*`の完了と`2D-1B-GATE`のmergeまでは、追加の`2D-2-*`と`2D-3-*`本実装を停止し、調査、prototype、test設計に限定する。product code を変更しない準備作業として、`2D-6-PERFORMANCE` / `2D-6-A11Y` の baseline だけを並行可能にする。
...
| §5 | IndexedDB、`.casproj`、保存状態、復旧、容量不足、壊れた入力 | `2D-1B-REVISION`, `2D-1B-RECOVERY`, `2D-1B-CAPACITY`, `2D-1B-CASPROJ`, `2D-1B-INPUT-SAFETY`, `2D-6-DEVICE-FLOW` | `2D-1B-REVISION`はPR #70、#71、#72、`2D-1B-LAYERS`はPR #76、#77、#78、`2D-1B-RECOVERY`はPR #80、#81、`2D-1B-CAPACITY`はPR #87、`2D-1B-CASPROJ`はPR #88で完了。現在は`2D-1B-INPUT-SAFETY`。Gateは未完了。 |
