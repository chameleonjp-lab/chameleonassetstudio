...
| 15 | `2D-4-CORE` + `2D-4-SHEET` + `2D-4-SCALE` | 共通export core、決定的再出力、fixed grid sheet、packed atlas、multi-page、1x / 2x / 3x、scale、trim、padding、extrudeを固定する。 | Fable5契約固定 + Codex + Opus 4.8 必須 review | 9、14 | 2D-5手順準備可（CORE完了、SHEETへ移行） | accepted: G15-C1 A + G15-C2 A + G15-C3 A / implemented: 2D-4-CORE / CI-passed / independently-verified / implementation-handoff-complete | 契約監査PR #229と採用記録PR #231をmainへ反映し、G15-H1〜H3 Aでmanifest path / version、packed tie-break、page境界・上限、命名、丸め規則、G15専用fixture・unit・E2E・CI証拠、実装対象ファイルを固定した。PR #233（merge `4ae44f647d66c184c55febfbe8577e682be2877f`）のCI Run #707（workflow ID `31478935075`）は全job成功し、CORE証拠artifact ID `9096417804`、Playwright artifact ID `9096547560`を記録した。固定headの親担当read-only独立確認は`BLOCKER 0 / MUST 0`である。既存Atlas `0.1.0`、既存ZIP、現行の理由付き拒否は維持する。進捗は16/27のまま、次は`2D-4-SHEET`。 |
