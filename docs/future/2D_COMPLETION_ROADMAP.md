...
| 16 | `2D-4-PACKAGE` + `2D-4-PREFLIGHT` + `2D-4-GENERIC-WEB` | 人間が契約を採択した後に、generic manifest、sidecar、README、import notes、verification record、preflight、Generic Web / Canvas 2D fixtureを実装する。 | Codex + Opus 4.8 必須 review | 15 | 2D-6 と並行可 | 15後 | `proposed / human-decision-pending / product implementation not started`。契約監査正本は`docs/future/2D_4_PACKAGE_PREFLIGHT_GENERIC_WEB_PLAN.md`、進捗17/27を維持。 |
