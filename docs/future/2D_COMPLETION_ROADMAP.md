...
| `2D-1B-GATE` | 必須 | fixtureと保存回帰テストを通し、2D-2 / 2D-3本実装を解禁する。 | current。Draft PR #91の実装head CI Run #268は全成功。Opus 4.8 review、人間確認、merge前のため未完了。 |
...
| 全保存回帰 Gate | `2D-1B-GATE` | Draft PR #91 / CI success | ADR-0015再監査、全保存回帰、横断SHOULD補修を実施し、CI Run #268は全成功。Opus 4.8 review、人間確認、mergeが未完了。 |
...
| 7 | `2D-1B-CASPROJ` + `2D-1B-INPUT-SAFETY` + `2D-1B-GATE` | `.casproj` 段階的検査、migration、不正入力隔離、保存回帰 Gate 証拠を揃え、2D-2 / 2D-3本実装を解禁する。 | Codex + Opus 4.8 必須 review + 人間確認 | 6 | Gate mergeまで 2D-2 / 2D-3 本実装不可 | 6後 | CASPROJはPR #88、INPUT-SAFETYはPR #89 / #90でcompleted。現在は`2D-1B-GATE`。 |
...
PR #53・PR #55 は先行 merge 済みのため revert しない。PR #53の`2D-1B-REVISION`該当範囲はPR #70、#71、#72、`2D-1B-LAYERS`該当範囲はPR #76、#77、#78、`2D-1B-RECOVERY`該当範囲はPR #80、#81、`2D-1B-CAPACITY`該当範囲はPR #87、`2D-1B-CASPROJ`該当範囲はPR #88、`2D-1B-INPUT-SAFETY`該当範囲はPR #89、#90で再監査・補修を完了した。`2D-1B-STORAGE-CROSS-REVIEW`はPR #84、#85による補修後に`BLOCKER 0 / MUST 0`で完了し、現在は`2D-1B-GATE`である。Gateのmergeまでは、追加の`2D-2-*`と`2D-3-*`本実装を停止し、調査、prototype、test設計に限定する。product code を変更しない準備作業として、`2D-6-PERFORMANCE` / `2D-6-A11Y` の baseline だけを並行可能にする。
...
| §5 | IndexedDB、`.casproj`、保存状態、復旧、容量不足、壊れた入力 | `2D-1B-REVISION`, `2D-1B-RECOVERY`, `2D-1B-CAPACITY`, `2D-1B-CASPROJ`, `2D-1B-INPUT-SAFETY`, `2D-6-DEVICE-FLOW` | `2D-1B-REVISION`はPR #70、#71、#72、`2D-1B-LAYERS`はPR #76、#77、#78、`2D-1B-RECOVERY`はPR #80、#81、`2D-1B-CAPACITY`はPR #87、`2D-1B-CASPROJ`はPR #88、`2D-1B-INPUT-SAFETY`はPR #89、#90で完了。現在はGate。 |
