...
| 12 | `2D-3-TIMELINE` + `2D-3-RIG` | frame animation、onion skin、可変時間、loop、event、part parent、pivot、bind pose、rotation / scale、part replace、簡易rig、frame bake、rig flipを実装する。 | Codex。新しいデータ意味はFable5または人間判断 | 8B、関連2D-1A契約 | 2D-2 と条件付き並行可 | public service + Slice A in review | 契約監査PR #146、計測資産PR #147、Pages配信PR #148 / #149をmainへ反映した。2026-07-23の人間判断で正式B0の4端末収集を保留し、H3を問題発生時の診断手段へ切り替えた。H3数値は未決定のため数値上限は実装しない。T1 Slice Aは本実装PRで検証中。正本は`2D_3_TIMELINE_RIG_PLAN.md`と`2D_3_H3_MEASUREMENT_PROTOCOL.md`。ADR-0010のcollider override / polygonはgroup 13に残す。 |
