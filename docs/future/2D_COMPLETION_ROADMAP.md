...
| 8A | `2D-2-PROJECT` + `2D-2-CREATE` | Project内の複数Asset / Family / Variant管理と、空キャンバス、template、複製、図形、パーツ、文字からの作成を完成させる。 | Codex + Opus 4.8 review | 7 | 契約が重ならない 8B 準備のみ可 | accepted slices completed / overall partial | `C` acceptedの独立Asset管理はPR #93、#94、残るCREATE `A+B+X` sliceはPR #98〜#101で完了した。Family / Variant永続契約のSlice AはPR #117、管理UIのSlice CはPR #121で完了した。Group 10 Slice Dのmerge / CI / review完了までoverall partialを維持する。 |
...
| 10 | `2D-2-VARIANT` + `2D-2-BATCH` | linked variant、独立copy、装備差分、解像度差分、手動調整保護、一括変更、対象preview、部分失敗、Undoを実装する。 | Codex + Opus 4.8 review | 9の現在実装可能範囲完了 | 契約が重ならない 2D-3 と条件付き並行可 | accepted / Slice A+B+C merged / Slice D review pending | 契約監査はPR #116、Slice AはPR #117、Slice BはPR #118、補修はPR #120、Slice CはPR #121でmainへmerge済み。正本§5のA→B→C→D直列順に従い、複数target preview・除外・progress・取消・原子Undo / RedoのSlice Dを現在のbranchへ実装し、merge / CI / reviewを待つ。 |
