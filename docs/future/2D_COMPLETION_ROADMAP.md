...
Group 12の詳細契約は`2D_3_TIMELINE_RIG_PLAN.md`を正本とする。T1 / R1 / P1の中核はPR #146 final head `20871f7`、merge `cb21ea4`でADR-0021〜0023へ反映した。H1=E1、H2=L1、H3=M1を2026-07-22に追加人間承認し、計測準備PR #147 final head `1ba671f7`、merge `24a089c`で再現資産をmainへ反映した。PR #148 / #149でサービス本体とH3のPages配信を分離した。2026-07-23の人間判断により、正式B0の4端末収集は当面保留し、サービス利用で性能や端末固有の問題が見えた時に同じH3を再実行する。H3の数値budgetは未決定のままなので数値warning / hard capは実装しないが、サービス公開と数値に依存しないT1 Slice Aは進めてよい。ADR-0010のcollider override / polygonはGroup 13のまま維持する。
...
| 12 | `2D-3-TIMELINE` + `2D-3-RIG` | frame animation、onion skin、可変時間、loop、event、part parent、pivot、bind pose、rotation / scale、part replace、簡易rig、frame bake、rig flipを実装する。 | Codex。新しいデータ意味はFable5または人間判断 | 8B、関連2D-1A契約 | 2D-2 と条件付き並行可 | public service + Slice A next | 契約監査PR #146、計測資産PR #147、Pages配信PR #148 / #149をmainへ反映した。2026-07-23の人間判断で正式B0の4端末収集を保留し、H3を問題発生時の診断手段へ切り替えた。H3数値は未決定のため数値上限は実装しない。サービス本体を継続公開した後、数値に依存しないT1 Slice Aを開始する。正本は`2D_3_TIMELINE_RIG_PLAN.md`と`2D_3_H3_MEASUREMENT_PROTOCOL.md`。ADR-0010のcollider override / polygonはgroup 13に残す。 |
