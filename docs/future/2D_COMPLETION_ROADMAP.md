...
| `2D-3-COLLIDER-OVERRIDE` | O1 accepted / Slice C Draft実装中 | asset共通判定を基準とするFrame別上書き規則を実装する。Animation別上書きは採用せず、必須へ変更しない。canonical契約はADR-0024 / Slice B / PR #218でmainへ反映済み、製品実装Slice Cは未検証である。 |
...
| O1 accepted / Slice C Draft実装中 | `2D-3-COLLIDER-OVERRIDE` | Frame単位だけを採用し、Animation単位は採用しない。保存形式・fallback・検証・反転・export拒否はADR-0024 / PR #218で固定済みで、現在は別PRのSlice Cだけで製品実装中・未検証である。 |
