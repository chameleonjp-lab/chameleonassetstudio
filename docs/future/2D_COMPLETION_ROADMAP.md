...
| 13 | `2D-3-GAME-DATA` + `2D-3-COLLIDER-OVERRIDE` + `2D-3-POLYGON` | origin、anchor、rect / circle、tile collision、background、gimmick、effect情報を編集し、frame別上書きとpolygonは判断後に扱う。 | Fable5または人間判断 + Codex実装 | 12、2D-1A-MOTION | 判断必須項目はADR後のみ | G1 Slice A implementation-in-draft | PR #215のdocs-only監査とPR #216の`G1+O1+P1`採用記録をmainへ反映した。G1のSlice Aは本Draft PRで安全性補修と回帰試験を実装中である。進捗は14/27を維持し、Slice A merge後にO1契約Slice Bへ進む。O1製品実装はSlice C、polygonはP1により`unsupported`を維持する。正本は`docs/future/2D_3_GAME_DATA_PLAN.md`。 |
