...
| `2D-1B-INPUT-SAFETY` | 必須 | ZIP、JSON、画像など不正入力の隔離と拒否を実装する。 | current。`B+X` accepted。監査・契約PR #89はmerge済み。Draft PR #90の実装head CI Run #264は全成功。正式完了はPR #90 merge後に同期する。 |
...
| 壊れた import 隔離 | `2D-1B-INPUT-SAFETY` | implemented in Draft PR #90 | ZIP / JSON / 画像を信頼しない入力として扱うB profile上限、理由付き拒否、quarantine、Asset / Layer batch原子性を実装し、CI Run #264が成功した。正式完了はmerge後に同期する。 |
