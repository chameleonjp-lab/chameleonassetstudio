...
| 15 | `2D-4-CORE` + `2D-4-SHEET` + `2D-4-SCALE` | 共通export core、決定的再出力、fixed grid sheet、packed atlas、multi-page、1x / 2x / 3x、scale、trim、padding、extrudeを固定する。 | Fable5契約固定 + Codex + Opus 4.8 必須 review | 9、14 | 2D-5手順準備可（2D-4-CORE実装中） | accepted: G15-C1 A + G15-C2 A + G15-C3 A / implementing: 2D-4-CORE / implementation-handoff-complete | 契約監査PR #229と採用記録PR #231をmainへ反映し、G15-H1〜H3 Aでmanifest path / version、packed tie-break、page境界・上限、命名、丸め規則、G15専用fixture・unit・E2E・CI証拠、実装対象ファイルを固定した。既存Atlas `0.1.0`、既存ZIP、現行の理由付き拒否は維持する。進捗は16/27のまま、次は`2D-4-CORE`実装。 |
