...
| `2D-3-COLLIDER-OVERRIDE` | O1 accepted / Slice C Draft実装 | asset共通判定を基準とするFrame別上書き規則を実装する。Animation別上書きは採用せず、必須へ変更しない。canonical契約はADR-0024 / Slice B / PR #218でmainへ反映済み、製品実装Slice Cは固定head検証必須・未mergeである。 |
...
| O1 accepted / Slice C Draft実装 | `2D-3-COLLIDER-OVERRIDE` | Frame単位だけを採用し、Animation単位は採用しない。保存形式・fallback・検証・反転・export拒否はADR-0024 / PR #218で固定済みで、現在は別PRのSlice Cを固定head検証必須・未mergeとして扱う。 |
...
| 13 | `2D-3-GAME-DATA` + `2D-3-COLLIDER-OVERRIDE` + `2D-3-POLYGON` | origin、anchor、rect / circle、tile collision、background、gimmick、effect情報を編集し、frame別上書きとpolygonは判断後に扱う。 | Fable5または人間判断 + Codex実装 | 12、2D-1A-MOTION | 判断必須項目はADR後のみ | G1 merged / O1 Slice B merged / Slice C implementation-in-draft | PR #215のdocs-only監査とPR #216の`G1+O1+P1`採用記録をmainへ反映した。G1 Slice AはPR #217 final head `e28e4de4`、merge `bc48487e`、CI Run #650全成功、固定headの3方向独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として完了した。O1のcanonical data、fallback、検証、UI、保存、変換、export拒否を固定したSlice BもPR #218 / merge `bbe9df96`でmainへ反映済みである。現在のADR-0024製品実装Slice CはDraft実装・固定head検証必須・未mergeである。進捗は14/27を維持し、polygonはP1により`unsupported`を維持する。正本は`docs/future/2D_3_GAME_DATA_PLAN.md`。 |
