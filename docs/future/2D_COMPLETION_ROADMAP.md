...
Group 12の詳細契約は`2D_3_TIMELINE_RIG_PLAN.md`を正本とする。T1 / R1 / P1の中核はPR #146 final head `20871f7`、merge `cb21ea4`でADR-0021〜0023へ反映した。H1=E1、H2=L1、H3=M1を2026-07-22に追加人間承認した。H3の数値budgetは`2D_3_H3_MEASUREMENT_PROTOCOL.md`のPC / iPhone / iPad測定と後続人間承認まで未決定であり、別の明示指示まで製品実装を開始しない。ADR-0010のcollider override / polygonはGroup 13のまま維持する。
...
| 12 | `2D-3-TIMELINE` + `2D-3-RIG` | frame animation、onion skin、可変時間、loop、event、part parent、pivot、bind pose、rotation / scale、part replace、簡易rig、frame bake、rig flipを実装する。 | Codex。新しいデータ意味はFable5または人間判断 | 8B、関連2D-1A契約 | 2D-2 と条件付き並行可 | measurement prep | 契約監査PR #146はfinal head `20871f7`、merge `cb21ea4`で完了。T1 / R1 / P1とH1=E1 / H2=L1 / H3=M1をacceptedとした。`tools/h3/`で再現可能なcore測定を準備するが、H3数値と製品実装は未着手。正本は`2D_3_TIMELINE_RIG_PLAN.md`と`2D_3_H3_MEASUREMENT_PROTOCOL.md`。ADR-0010のcollider override / polygonはgroup 13に残す。 |
