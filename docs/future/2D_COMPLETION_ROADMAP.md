...
| `2D-3-COLLIDER-OVERRIDE` | O1 accepted / Slice C main反映済み / closeout pending | asset共通判定を基準とするFrame別上書き規則を実装する。Animation別上書きは採用せず、必須へ変更しない。canonical契約はADR-0024 / Slice B / PR #218、製品実装と補修はPR #219 / #220でmainへ反映済みである。E2E待機安定化PR #221とGroup 13 closeoutを残す。 |
...
| O1 accepted / Slice C main反映済み / closeout pending | `2D-3-COLLIDER-OVERRIDE` | Frame単位だけを採用し、Animation単位は採用しない。保存形式・fallback・検証・反転・export拒否はADR-0024 / PR #218で固定し、製品実装と補修はPR #219 / #220でmainへ反映済みである。E2E待機安定化PR #221は固定head検証必須・未mergeとして扱う。 |
