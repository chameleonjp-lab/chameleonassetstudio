...
| 15 | `2D-4-CORE` + `2D-4-SHEET` + `2D-4-SCALE` | 共通export core、決定的再出力、fixed grid sheet、packed atlas、multi-page、1x / 2x / 3x、scale、trim、padding、extrudeを固定する。 | Fable5契約固定 + Codex + Opus 4.8 必須 review | 9、14 | 2D-5手順準備のみ可 | contract-audit-in-progress / product-not-started / unaccepted | 契約監査だけを開始可能とする。common manifest、target優先度、determinism、pack順、rotation、page上限、trim座標、padding / extrude、倍率出力、version / 既存ZIP互換の契約と受入条件を人間が採用するまで、製品実装を開始しない。 |
