...
Group 12の詳細契約は`2D_3_TIMELINE_RIG_PLAN.md`を正本とする。T1 / R1 / P1の中核はPR #146 final head `20871f7`、merge `cb21ea4`でADR-0021〜0023へ反映した。H1=E1、H2=L1、H3=M1を2026-07-22に追加人間承認し、計測準備PR #147 final head `1ba671f7`、merge `24a089c`で再現資産をmainへ反映した。PR #148 / #149でサービス本体とH3のPages配信を分離した。T1 Slice AはPR #153 final head `7f684a7`、merge `e8fac95`、P1 Slice CはPR #154 final head `fdf75f0`、merge `1c700e7`でmainへ反映済みで、CI Run #489 / #492は全成功した。2026-07-23の人間判断により、正式B0の4端末収集は当面保留し、サービス利用で性能や端末固有の問題が見えた時に同じH3を再実行する。H3の数値budgetは未決定のままなので数値warning / hard capは実装しない。ADR-2026-07-24-027でA1を採用し、Slice BをB1 / B2へ分割した。PR #157 final head `834cc38397c300895f50c1efdb86d94f3870a0a8`、merge `bf13cac3db854c30b33e9b2ef97d389a2372e961`でB1をmainへ反映した。CI Run #501は全job成功し、非GitHub・非Opusの固定head独立reviewは`BLOCKER 0 / MUST 0 / SHOULD 0`だった。B2は資源上限、採用上限での実機測定、Group 12完了判定を扱い、後続の明示判断まで保留する。ADR-2026-07-27-028でA1+B1を採用し、Slice DをD1〜D4へ分けた。D1はPR #201 final head `25cd3327b93850f1af1733c2b43585e3fa0a667b`、merge `c1d08e3b4cadd7c3a3064ab8e824b17f67feb243`、CI Run #596全成功、固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として完了した。ADR-2026-07-28-029でA1+B1+C1を採用し、D2を選択中Animationの再生順に従うread-only・UI-onlyのonion skinとして契約した。D2はPR #204 final head `8ebeb279e9d7b9ef9a15700d80d4a6cd7ab1d57f`、merge `eeaea39522d0f31bfe786ca0da27176bfd5ee859`、CI Run #603のclean attempt、固定head独立reviewを経て完了した。ADR-2026-07-29-030でD3の`P1+A1+B1+C1`を承認し、選択中Animationのevent追加・名前変更・参照Frame変更・削除、1 History、保存・reload、非対象データ不変の契約を固定した。D3製品実装はこのdocs-only決定PRのmerge後に別Draft PRで開始し、D4は未着手とする。ADR-0010のcollider override / polygonはGroup 13のまま維持する。

PR #203 merge `73f0eafe69dd42c820f92e9f039e49fe2e2d9d28`でD2のmerge前Gateは満了し、PR #204 merge `eeaea39522d0f31bfe786ca0da27176bfd5ee859`でD2を`implemented / CI-passed / independently-verified / merged`としてcloseoutした。D3は契約承認後の製品実装を完了し、CI・独立review・merge待ちである。D4、B2、物理iPhone SafariのGroup 12 closeout Gateは未完了のまま維持する。

この節と工程表に残る「D3製品実装は決定PRのmerge後に開始する」「product implementation not started」は、PR #205 merge前のGate記録である。PR #205でGateは満了し、現在はPR #206に製品実装と必須testが存在する。CI・独立review・mainへのmergeが終わるまではD3 closeoutとは扱わない。
...
| 12 | `2D-3-TIMELINE` + `2D-3-RIG` | frame animation、onion skin、可変時間、loop、event、part parent、pivot、bind pose、rotation / scale、part replace、簡易rig、frame bake、rig flipを実装する。 | Codex。新しいデータ意味はFable5または人間判断 | 8B、関連2D-1A契約 | 2D-2 と条件付き並行可 | public service + Slice A/C/B1/D1/D2 merged and independently verified / D3 contract accepted, product implementation not started / B2 and D4 pending | 契約監査PR #146、計測資産PR #147、Pages配信PR #148 / #149、T1 Slice A PR #153 merge `e8fac95`、P1 Slice C PR #154 merge `1c700e7`をmainへ反映した。2026-07-23の人間判断で正式B0の4端末収集を保留し、H3を問題発生時の診断手段へ切り替えた。ADR-2026-07-24-027でA1を採用し、PR #157 final head `834cc38`、merge `bf13cac`、CI Run #501全成功、非GitHub・非Opusの固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`としてB1を完了した。B2のH3数値、資源上限、実機product-path、Group 12完了判定は保留する。ADR-2026-07-27-028でA1+B1を採用し、PR #201 final head `25cd332`、merge `c1d08e3`、CI Run #596全成功、固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`としてD1を完了した。ADR-2026-07-28-029でA1+B1+C1を採用し、D2契約と試験条件を確定した。D2はPR #204 final head `8ebeb27`、merge `eeaea39`、CI Run #603のclean attempt、固定head独立reviewを経て完了した。ADR-2026-07-29-030でD3の`P1+A1+B1+C1`を承認した。D3製品実装はこのdocs-only決定PRのmerge後に別Draft PRで開始し、D4は未着手とする。正本は`2D_3_TIMELINE_RIG_PLAN.md`と`2D_3_H3_MEASUREMENT_PROTOCOL.md`。ADR-0010のcollider override / polygonはGroup 13へ残す。 |
...
PR #53・PR #55は先行merge済みのためrevertしない。PR #53の`2D-1B-REVISION`該当範囲はPR #70、#71、#72、`2D-1B-LAYERS`該当範囲はPR #76、#77、#78、`2D-1B-RECOVERY`該当範囲はPR #80、#81、`2D-1B-CAPACITY`該当範囲はPR #87、`2D-1B-CASPROJ`該当範囲はPR #88、`2D-1B-INPUT-SAFETY`該当範囲はPR #89、#90で再監査・補修を完了した。`2D-1B-STORAGE-CROSS-REVIEW`はPR #84、#85による補修後に`BLOCKER 0 / MUST 0`で完了し、Gate実装PR #91 / CI Run #269、Opus 4.8 review問題なし、人間確認により`2D-1B-GATE`も正式完了した。`2D-2-PROJECT + 2D-2-CREATE`はPR #93、#94とPR #98〜#101でaccepted sliceを完了し、Family / Variant管理UIはPR #121でmainへmergeした。`2D-3-TYPE-PROFILES + 2D-3-INSPECT`はPR #95〜#97で完了した。`2D-2-RASTER + 2D-2-REPAIR`はPR #113で複数Layer align / distribute、PR #115でAsset canvas resize / game data追従をmainへmergeした。PR #115 final head `63d92a1dd56679edc047fa62ab450ad743323cfc`のCI Run #384は全成功、merge commitは`1838f58918a2958f9ebce2f8379f87a45fb17c26`。GitHub上のreview / comment記録は0件であり、Opus review完了とは扱わない。frameずれ修正は`2D-3-TIMELINE`後へ保留した。`2D-2-VARIANT + 2D-2-BATCH`は契約監査PR #116、Slice A PR #117、Slice B PR #118、補修PR #120、Slice C PR #121、Slice D PR #122がmainへmerge済みで、Slice D遡及Opus review（`BLOCKER 0 / MUST 0 / SHOULD 2 / NOTE 5`）とcloseout補修（縮小canvas warning E2E、docs status同期）により工程を完了した。group 11は契約監査PR #124、Slice A PR #125、provenance基盤のSlice B PR #126、連番・手動格子sheet・common preview・quarantineのSlice C PR #127/#128、Tileset + Chameleon Atlas意味上roundtripのSlice D PR #129、closeout PR #133、Slice E source契約補正PR #135、製品実装PR #138、post-merge補修PR #144までmainへmerge済みである。PR #144 final head `1980ae6`のCI Run #450は全成功し、固定head独立read-only reviewは`BLOCKER 0 / MUST 0 / SHOULD 2 / NOTE 1`（非Opus）だった。SHOULD 2件は取り込みtest debt、NOTE 1件はiPhone Safari実機Gateへ残し、group 11をcompletedとする。group 12の契約監査はPR #146 merge `cb21ea4`、H1=E1 / H2=L1 / H3=M1の記録と計測準備はPR #147 merge `24a089c`、Pages配信分離はPR #148 / #149でmainへ反映済みである。T1 Slice AはPR #153 final head `7f684a7`、merge `e8fac95`、P1 Slice CはPR #154 final head `fdf75f0`、merge `1c700e7`としてmainへ反映済みで、CI Run #489 / #492は全成功した。2026-07-23の人間判断により、正式B0は問題発生時まで保留する。H3数値budget、warning、hard capは未承認のため先取りしない。ADR-2026-07-24-027でB1先行を承認し、PR #157 final head `834cc38`、merge `bf13cac`、CI Run #501全成功、非GitHub・非Opusの固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`としてB1を完了した。B2とGroup 12完了判定は後続の人間判断へ残す。ADR-2026-07-27-028でA1+B1を承認し、PR #201 final head `25cd332`、merge `c1d08e3`、CI Run #596全成功、固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`としてD1を完了した。ADR-2026-07-28-029でA1+B1+C1を承認し、D2の契約・試験条件を確定した。PR #203 merge `73f0eaf`で実装Gateを満了し、PR #204 final head `8ebeb27`、merge `eeaea39`、CI Run #603のclean attempt、固定head独立reviewを経てD2を完了した。ADR-2026-07-29-030でD3の`P1+A1+B1+C1`を承認した。D3製品実装はこのdocs-only決定PRのmerge後に別Draft PRで開始し、D4は未着手とする。

D2は`implemented / CI-passed / independently-verified / merged`である。D3は`contract accepted / product implementation complete in PR #206 / CI and independent review pending`である。B2、D4、物理iPhone Safari、Group 12完了判定は未完了のまま維持する。
