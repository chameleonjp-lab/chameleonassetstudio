...
- 既存History、autosave、IndexedDB、原子的rollbackを使う。保存失敗ではReact state、Asset、Project参照、Blob、History、IndexedDB、自動保存状態を保存前へ戻す。変更対象でないBlobも不変であることを確認する。
...
- schema / model: field不在、空配列、rect / circle / visible-only、参照切れ、shape不一致、rect / circle同時保持、非有限値、0以下サイズ、重複、未知field保持。
