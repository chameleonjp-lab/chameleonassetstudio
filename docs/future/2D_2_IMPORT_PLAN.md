...
- 取り込み経路: `EditorScreen.tsx`の`handleFiles`（`assertImageBatchCount`で最大16 file）、file inputとdrag & drop。Slice E製品実装は新規Assetの`NEW_ASSET_IMPORT_ACCEPT`だけへSVG / GIF / APNGを加え、`RASTER_IMPORT_ACCEPT`を使う既存Assetへのlayer追加と`ImportFrameSetPanel`の連番 / 手動格子sheet / tileset / Chameleon atlasはPNG / JPEG / WebPを維持する。
...
- 失敗時: `ImageImportError.kind`で原因を分類し、署名不一致・寸法超過・decode失敗だけを既存quarantine store（上限3件 / 50MiB超はbytes非保存）へ接続する。unsupported MIME、file size上限、hash / encode / 環境 / 保存失敗に加え、17frame超とactive SVGは理由表示のみで、入力破損として隔離しない。
...
- **S1（推奨）**: 直列5 slice。各sliceは最新mainからbranchを作り、Draft PR → CI → 独立Opus review → 人間確認で進める。Opus 4.8を利用できない場合の代替は§8のreview条件に従う。
...
PR #135（merge `f5bb322`）で上記source保存契約補正はmainへ反映済みである。
...
- 既存の署名検査・25MiB / 4096px上限・batch件数上限を維持する。署名・decode・寸法失敗だけをquarantineへ記録し、17frame超・active SVG・unsupported形式は理由付き拒否だけにする。
...
- SVGでscript / event / 外部URLを実行せず通信も発生させない。GIF / APNG対応環境では全frameとpixel / 時間 / loop写像、decode APIまたはMIME非対応環境では先頭frame + 8fps + loss warningになる。unsupported形式と17frame超は理由付きで明示拒否される。
...
- ADR-0020製品実装PR: bounded GIF / APNG parser、SVG safety、時間 / loop写像のunit、source bytes / hash、全frame decode、fallback、悪意あるSVG、17frame拒否、Desktop / touch / 375 x 667の製品E2Eを含め、lint / format / build / unit / Chromium E2E + CIを必須とする。Safari / iOS Filesは実機確認を残す。
...
- 実装review条件: 各slice Draft PR → CI → 独立Opus review → 人間確認を既定とする。Opus 4.8を利用できない場合は、ユーザー提供のWork運用仕様どおり、未実施範囲を記録し、writer以外の複数担当による固定headの独立read-only reviewで代替したうえで、人間が残リスクとmergeを明示確認する。A→B→C→D→Eの直列順。各sliceは前sliceのmerge後に最新mainからbranchを作る。
...
- 検証状態: PR #144 final head `1980ae6`のCI Run #450は全成功した。Opus 4.8を利用できなかったため、仕様、実装・データ安全、テスト・端末の3担当が同じheadを独立read-only reviewし、`BLOCKER 0 / MUST 0 / SHOULD 2 / NOTE 1`を確認した。これはOpus review完了とは扱わず、代替範囲と残リスクを本節へ記録する。
- PR状態: PR #138はmerge `c188e17`、PR #144は人間のmerge判断によりmerge `616d225`としてmainへ反映済み。merge後のユーザー指示「#144更新されました。後続対応開始」を人間確認として、代替reviewの結果と残リスクを維持したままgroup 11をcompletedとする。
...
- 残るNOTE: iPhone SafariのFiles MIME、native picker、safe area、下部バー、software keyboardは未確認であり、`docs/RELEASE_CHECKLIST.md` §3の実機Gateへ残す。実機memoryは同文書§4の非blockingな性能・メモリ将来課題へ残し、group 11の完了Gateには含めない。
