...
物理iPhone / iPadのB0測定では、`.github/workflows/h3-pages.yml`を使い、サービス本体をGitHub Pagesのroot、計測ハーネスを`/h3/`へまとめて配信する。公開URLは本体が`https://chameleonjp-lab.github.io/chameleonassetstudio/`、H3が`https://chameleonjp-lab.github.io/chameleonassetstudio/h3/`である。初回だけrepository SettingsのPagesで配信元を`GitHub Actions`に設定する。この設定とworkflowのmain反映は人間判断後に行い、Draft PRやPR CIから公開しない。
