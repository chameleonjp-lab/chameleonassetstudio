...
各結果へcommit、device、OS、browser version、user agent、viewport、device pixel ratio、orientation、Low Power Mode、温度状態を記録する。browserがJS heap、Long Task、storage estimateを提供しない場合は`unsupported`かつ値`null`とする。未対応を0件・0 byteと記録しない。
