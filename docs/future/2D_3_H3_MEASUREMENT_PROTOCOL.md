...
したがって、Node結果やブラウザcore結果だけを端末合格や数値budgetとして使わない。数値決定は本プロトコルの正式な実測後に、別の人間承認として記録する。
...
正式B0の4端末収集は削除せず、公開利用で性能低下、停止、保存・書き出しの遅延、Safari固有の問題などが見えた時に再開する。数値budgetが未決定の間はwarningやhard capを実装しない。T1 Slice A / P1 Slice Cに続き、数値budgetに依存しないR1 Slice B1はPR #157 final head `834cc38`、merge `bf13cac`、CI Run #501全成功、非GitHub・非Opusの固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として実装済みである。B1は座標修正、構造preflight、独立rig反転コピー、完全ID remap、保存・reload・parityに限り、資源上限とGroup 12完了判定はB2へ残す、というのが2026-07-23時点の運用判断だった。Group 12完了条件については2026-08-02の§9がこの部分を置き換え、B2は`2D-6-PERFORMANCE`へ延期した。新しい問題から必要な検査項目が見つかった場合は、再現条件を固定してから本書または製品path Gateへ追加する。
...
`.github/workflows/h3-pages.yml`は、main更新時にサービス本体をGitHub Pagesのrootへ自動公開し、通常のH3を`/h3/`の終了画面にする。公開URLは本体が`https://chameleonjp-lab.github.io/chameleonassetstudio/`、H3が`https://chameleonjp-lab.github.io/chameleonassetstudio/h3/`である。Draft PRやPR CIからは公開しない。

再計測が必要な時だけ、手動workflowの`publish-24h`をmainで実行する。実行時刻を`publishedAt`とし、正確に24時間後を`expiresAt`として静的buildへ埋め込む。公開画面は毎秒期限を確認し、`expiresAt`以降は新しい計測を拒否して期限終了状態を表示する。定期workflowや長時間待機jobは使わず、計測終了後も不要なActions実行を残さない。

全端末のJSONを保存できた場合は、24時間を待たず`close-now`を実行し、`/h3/`だけを終了画面へ置き換える。未完了で24時間に達した場合も画面側で計測は止まり、その後`close-now`を実行できる。サービス本体はrootで公開を維持するため、通常の計測終了でPagesをUnpublishしない。Unpublishはサービス本体も停止することを理解した人間が、サービス全体を非公開にするときだけ行う。再計測は新しい`publish-24h`を明示実行し、新しい24時間枠とsource commitを記録する。

rootから`/h3/`へ移した後の正式B0結果は、移設後の同一source commitで最初から収集する。移設前のroot計測結果と移設後の`/h3/`計測結果を、同一commitの証拠として混在させない。
