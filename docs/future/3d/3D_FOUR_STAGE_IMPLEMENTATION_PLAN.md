...
作業順: 01 → 10 → (02・03・11 並行) → (04・05・12 並行) → 06 → 07 → 08 → 09 → GATE。（`-10` の進捗・中断基盤を先に作り、以後の重処理 WP が乗る。`-11` は重処理と独立のため早期並行可。）
...
- デモ手順: 1. draco.glb 読み込み → 2. 一括検品 → 3. big-texture を 2048 化 + Meshopt 出力 → 4. Compare で保持チェック → 5. 派生を選んで決定的 ZIP を 2 回出し hash 一致確認 → 6. 中断とメモリ不足復旧の実演 → 7. character-anim.glb の humanoid 対応付け →「右手に anchor」→ 8. texture の背景透過編集 → Compare → 書き出し。
...
- 想定 PR 総数: Gate 6（記録中心）+ Stage1 約 13（00〜11 + dependency 分割）+ Stage2 約 11（+VRM/skeleton）+ Stage3 約 14（+humanoid、texture 編集は worker/UI 2 PR 可）+ Stage4 約 10（+条件付き WebXR） = **約 54 PR**（±20% の幅を見込む。条件付き WP の不採用でこの数は減る。カレンダー日程は約束しない）。
