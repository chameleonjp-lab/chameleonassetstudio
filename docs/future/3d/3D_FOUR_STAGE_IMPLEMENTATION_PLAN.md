...
作業順: (01・03・04・05・06 並行可) → 07 → 08 → GATE。02（外部接続）は `3D-DEC-EXTGEN-01` で承認された場合のみ 01 の後に実施し、承認されなければ省略する（06 以降は 02 の有無に関わらず進む）。
...
（STAGE1-00 の fixture 生成基盤は品質レーンで STAGE1-01 と並行に進むため critical path には現れないが、STAGE1-04 の前に完了している必要がある。STAGE4-06 は STAGE3-GATE 直後から STAGE4-07 までの間に実施する。）
...
- 想定 PR 総数: Gate 6（記録中心）+ Stage1 約 13（00〜11 + dependency 分割）+ Stage2 約 10 + Stage3 約 12 + Stage4 約 9 = **約 50 PR**（±20% の幅を見込む。カレンダー日程は約束しない）。
