...
最終更新日: 2026-07-21（第2改訂への参照資料追記: Three.js Object Sculptor。実装段階・Gate は変更しない）
...
各 work package（以下 WP）の記載は 53 項目の必須書式に従うが、全 WP で同一の項目は以下の共通既定を正とし、各 WP には**差分だけ**を書く。各 WP の項目番号（1)〜(53)は、本書末尾「付録 A: work package 53 項目書式」で定義する番号に対応する。この付録が番号の意味の正本である。
...
- **（19）変更してはいけないファイル・共通**: `src/core/**`、`src/features/editor/**`、`src/features/home/**`（明示した WP を除く）、`src/renderers/canvas2d/**`、`src/workers/imageOps.worker.ts`、`src/workers/imageAnalysis.worker.ts`、既存 `*.schema.json`、既存テストと fixture（期待値の緩和・削除・skip の禁止）、`docs/` の既存 2D 文書。`package.json` / `package-lock.json` は `3D-GATE-04` で承認された dependency PR でのみ変更可。
  - 既存 docs への**追記の例外**: `3D-GATE-03` は `docs/future/DECISION_LOG.md` へ ADR を追記する。`3D-STAGE4-08` は `docs/USER_GUIDE.md` / `docs/RELEASE_CHECKLIST.md` / ルート `README.md` へ 3D 節を**追記**する。いずれも既存 2D 記述の削除・書き換えは行わず、3D 節の追加に限る（本計画作成時の `docs/future/README.md` 索引への 1 行追記と同じ扱い）。
...
- **⚠️ 整合注意の読み方（2026-07-20 第2改訂で導入）**: 本計画の各所にある「⚠️ 整合注意（`3D-RISK-xx`）」は、**複数文書・複数 WP にまたがって齟齬が起きやすい箇所**の印である。台帳（番号・内容・防止規則）の正本は `3D_DECISION_LOG_AND_OPEN_ITEMS.md` 7 章。該当 WP の実装・レビュー時は、台帳の当該項目を必ず読み合わせる。
...
- （7〜9）範囲: 自作 fixture 生成スクリプトの設計（GLB を依存ライブラリなしでバイト列組み立て、または承認済みツールで生成）、第三者素材を使う場合の個別確認手順の承認、fixture 台帳の書式確定。**スクリプトの実装と第一段階 fixture の追加は `3D-STAGE1-00` で行う**（本 GATE は方針・書式の承認まで）。
- （10〜13）依存: `3D-GATE-01`。後続: `3D-STAGE1-00`（実装）、および `3D-STAGE1-04` 以降の全テスト WP。
...
作業順（依存順）: 00 →（01 と並行可）→ 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09 → 10 → 11 → GATE。00（fixture 生成基盤）は品質レーンで最初に着手し、04 以降の全テストの前提にする。02〜03 と 04 は一部並行可、06〜08 は 05 完了後に並行可（5 章のレーン規則に従い同時 open PR は最大 3）。

#### 3D-STAGE1-00 fixture 生成基盤（GLB 生成スクリプトと台帳）

- （1〜3）: `3D-STAGE1-00` / fixture 生成スクリプト / 第一段階
- （4）価値: 以後のすべてのテストが、利用条件の明確な試験素材で駆動できるようになる。
- （5〜6）現状と根拠: 方針・台帳書式は `3D-GATE-06` で承認済み。fixture 計画は `3D_TEST_EVIDENCE_AND_RELEASE_SPEC.md` 3 章。
- （7〜9）目的 / 範囲 / 非範囲: 第一段階で必要な fixture（minimal / box-textured / offset-origin / z-up / character-anim / corrupt-header / corrupt-chunk / truncated / oversized 系）を、依存ライブラリに頼らず GLB バイト列を組み立てる生成スクリプトで用意し、`e2e/fixtures-3d/` と台帳（`e2e/fixtures-3d/README.md`）に登録する。非範囲 = 第二段階以降の fixture（bundle / 圧縮 / 材質系は各段階の該当 WP で追加）、圧縮 fixture 生成に必要な gltfpack 等（Stage3 開始条件）。
- （10〜13）依存: `3D-GATE-06`。後続: STAGE1-04〜11 の全テスト。並行: STAGE1-01（route）と同時に品質レーンで進められる。
- （17〜18）変更 / 新規: 新規 = `e2e/fixtures-3d/**`（生成スクリプト・生成物・台帳）。
- （19）変更禁止: 共通既定どおり。既存 fixture・テストに触れない。
- （35）手順: 1. GLB バイナリ組み立てユーティリティ → 2. 各 fixture 生成 → 3. 台帳記録（目的・出所=生成スクリプト・ライセンス=自作/CC0・期待 stats・期待警告 ID）→ 4. 生成の再現性テスト（スクリプトから同一バイト列が得られる）。
- （36〜40）テスト / 証拠: 生成物の schema/GLB 妥当性チェック、台帳の網羅確認。
- （41〜42）受け入れ / 完了: 前提: なし。操作: 生成スクリプト実行。期待結果: 第一段階の全 fixture が生成され、台帳に全項目（目的・出所・ライセンス・再配布可否・期待結果・使用テスト・更新規則）が記入されている。
- （43〜44）rollback / 文書更新: PR revert で完全に戻る / テスト仕様 3 章の台帳へのリンク。
- （48）人間確認: 不要（GATE-06 で方針承認済み）。
- （49〜52）複雑度 M / 不確実性 中（破損 GLB の作り込み）/ 危険 低 / 対処: corrupt 系は「どのバイトをどう壊すか」をスクリプトのコメントで明示し再現可能にする。
- （53）依頼要約: 「GATE-06 の承認方針に沿って、第一段階の fixture を生成するスクリプトと台帳を `e2e/fixtures-3d/` に新設してください。依存ライブラリを使わず GLB バイト列を組み立て、破損・過大 fixture も再現可能にしてください。」
...
- （7〜9）範囲: `src/core3d/storage/db3d.ts`（DB `chameleon-asset-studio-3d` v1・7 store: projects3d / assets3d / blobs3d / trash3d / snapshots3d / reports3d / quarantine3d）、projectStore3d（単一トランザクション保存）、trash3d（上限 5）、quarantine3d（上限 3・50MB 規則）、snapshots3d（metadata のみ・上限 3/asset）、reports3d（検査 / 書き出し記録）、quota エラー処理（既存 StorageError 方式）、AutosaveQueue の 3D タスク接続。非範囲 = `.cas3dproj`（09）。
...
- （53）依頼要約: 「別 DB `chameleon-asset-studio-3d` の保存層（7 store・単一トランザクション・trash/quarantine/snapshot/reports・autosave 接続）を 2D の実装パターンに倣って新設し、fake-indexeddb テストを付けてください。」
...
- （10〜13）依存: 00（破損・過大 fixture）・02・03（隔離接続）。後続: 05・06。並行: 03 と部分並行可。
...
- （36〜39）テスト: parser unit（正常系 + corrupt-header / corrupt-chunk / truncated / oversized / json-bomb）、E2E（取り込み成功・失敗・隔離表示）、fixture = `3D-STAGE1-00` で生成済みのもの（GATE-06 方針）。
- （40〜42）受け入れ: 前提: minimal.glb。操作: Import。期待結果: 合格し stats が一覧表示。前提: corrupt-chunk.glb。操作: Import。期待結果: 理由つき拒否 + 隔離 1 件 + アプリ操作継続可能。
- （48）人間確認: 不要。（49〜52）: L / 中（多様な壊れ方）/ 高（安全の要）/ 対処: `3D-STAGE1-00` の fixture を前提にテスト駆動で実装する。
...
- （19）変更禁止（差分）: **2D 接点あり**。`src/features/home/HomeScreen.tsx` に 3D `.cas3dproj` 読み込み入口を追加する（共通既定 (19) の「home は明示 WP を除く」に該当）。この PR は 2D E2E 全件 + bundle 非汚染 assert を必須にする。既存 2D 読み込み経路の挙動は変えない。
...
#### 3D-STAGE1-11 Stage1 統合 E2E・不足 fixture 追加・2D 回帰証拠

- （1〜3）: `3D-STAGE1-11` / 統合検証 / 第一段階
- （4）価値: 一続きの体験が壊れていないことを継続的に保証する。
- （7〜9）範囲: `3D-STAGE1-00` で用意した fixture の**不足分の追加と台帳の最終確認**、動線 E2E（作成→取込→検品→設定→GameData→保存→リロード→`.cas3dproj` 往復→ZIP）、iPhone viewport E2E、bundle 非汚染 assert の再確認、CI での WebGL 動作確認（不可なら viewer E2E の job 分離）。fixture 生成スクリプト本体の新設は `3D-STAGE1-00` の範囲（本 WP では作らない）。
- （10〜13）依存: 00〜10 すべて。後続: STAGE1-GATE。
...
- （53）依頼要約: 「第一段階の一続き動線 E2E（PC + iPhone viewport）を追加し、不足 fixture を補い、2D 回帰と bundle 非汚染を含む CI を安定化してください。fixture 生成スクリプト自体は `3D-STAGE1-00` で作成済みの前提です。」
...
第二段階として検討対象に挙がっている機能群への判断（採用 / 延期 / 対象外）:
...
| VRM の検出・meta 表示・humanoid / VRM 検査（V1/V2） | 採用（2026-07-20 改訂で追加） | `-11`。依存追加なし（JSON 読取のみ）。詳細は `3D_INTEROP_VRM_VR_AND_CREATION_SPEC.md` 5 章 |
| skeleton 検査の拡張（joint ツリー・bind pose・T/A-pose 推定・ウェイト異常 = B1） | 採用（2026-07-20 改訂で追加） | `-05` の拡張スコープ + 検査 ID `3D-CHK-BONE-001〜003`（interop 仕様 4.1） |
| vr 検品プリセット | 採用（2026-07-20 改訂で追加） | `-06` の対象に vr を追加（interop 仕様 6.1。暫定しきい値から） |
| Blender / Unreal 向け import notes | 採用（2026-07-20 改訂で追加） | `-08` の対象を 4 → 6 に拡大（interop 仕様 3.1） |
| 焼き込み書き出し | 判断待ち（`3D-OPEN-05`）。承認時は `-08` に含める | 段階開始時に Fable5/人間が判断 |
| 材質の簡易編集 | 再定義（旧 `3D-OPEN-14`）: テクスチャ編集は第三段階 `3D-STAGE3-12` として採用。factor 調整は同 WP の任意拡張 | interop 仕様 7 章 |
...
| 項目 | 第二段階での内容 |
|---|---|
| 対象アセット / 入力 | GLB + glTF bundle（複数ファイル / ZIP）。animation・skin・morph 付きを実用対象化 |
| 出力 | Stage1 の出力 + import notes 4 種 + thumbnail。（焼き込みは `3D-OPEN-05` 承認時のみ） |
| 画面 | Stage1 の 7 画面 + Scene / Materials / Animation / Project Settings |
| データ形式 / 保存 | 契約 v0 のまま（source.files 複数対応・nodeRef・capsule は 0.1.0 内の追加フィールド。version 据え置きの可否は Opus レビューで確認） |
| 検品 | +12 項目、プリセットしきい値の確定（実測根拠つき） |
| ゲーム用情報 | capsule collider、node 追従 anchor |
| 性能 / 端末 | 確定しきい値と暫定予算の再確認。PC / iPad / iPhone 全動線 |
| アクセシビリティ | ツリーのキーボード操作、aria-live 選択通知、motion reduction（自動再生禁止） |
| セキュリティ | bundle URI 許可リスト・zip bomb 対策（攻撃面拡大への対応） |
| ライセンス | 新規依存なし（decoder 系は第三段階） |
| 未対応範囲 | 材質編集・圧縮読み込み・最適化・外部生成 |
| テスト証拠 | bundle / URI / 材質 fixture、Animation E2E、実機報告（iPhone / iPad） |
| 次段階へ進む条件 | STAGE2-GATE の受け入れ（デモ 9 手順 + BLOCKER/MUST ゼロ + 人間承認） |

### 2.5 work package

作業順: 01 → 02 → (03・04・05 並行) → (06・11 並行) → 07 → 08 → (09・10 並行) → GATE。
...
- （7〜9）範囲: GATE-05 で確定した端末での fixture 実測（読み込み時間・fps・メモリ）、mobile / generic-web / desktop **+ vr** の 4 プリセットのしきい値確定と定数化（vr は interop 仕様 6.1 の暫定値から。VR 実機が無い場合は「暫定のまま」と明示して残す）、UI の「暫定」表示解除、確定根拠の報告書。非範囲 = しきい値の利用者カスタム（`3D-OPEN-17`）。
...
- （7〜9）範囲: **6 対象**（Three.js / Babylon.js / Godot / Unity + Blender / Unreal。2026-07-20 改訂で +2）の import notes 生成（座標系変換・単位[UE は cm 換算]・anchor/collider の読み方・capsule 解釈・既知の制限）、Mixamo → Blender → GLB の supply notes（interop 仕様 3.2）、export ZIP への同梱、report の human-readable 版（README 内要約）。焼き込みオプション（`3D-OPEN-05` が承認された場合のみ）。非範囲 = verified 主張（証拠は第四段階）。
- （10〜13）依存: -06・-07。後続: -09。
- （41〜42）受け入れ: 前提: 設定済みアセット。操作: ZIP 書き出し。期待結果: engines/ に 6 notes が入り、各 notes に「import notes only（実行検証はしていません）」の区分が明記される。
- （48）: `3D-OPEN-05` の判断のみ Fable5/人間。（49〜52）: M / 低 / 低。
- （53）依頼要約: 「6 対象向け import notes 生成を純関数で実装し、compatibility 区分を明記して ZIP へ同梱してください。」
...
- （19）変更禁止（差分）: **2D 接点あり**。3D プロジェクトのサムネイル表示のため `src/features/home/HomeScreen.tsx` の一覧描画に触れる（3D 行のみ。2D 行の表示は変えない）。2D E2E 全件 + bundle 非汚染 assert を必須にする。
- （10〜13）依存: -03。並行: -10。
...
#### 3D-STAGE2-11 VRM 検出・meta 表示・VRM/skeleton 検査（V1/V2 + B1）

- （1〜3）: `3D-STAGE2-11` / VRM・skeleton 検査 / 第二段階（2026-07-20 改訂で追加）
- （4）価値: VRoid 等で作られた人型アバターを、利用条件つきで安全にゲーム素材として受け入れられる。骨の状態が中身を開かずに分かる。
- （5〜6）根拠: `3D_INTEROP_VRM_VR_AND_CREATION_SPEC.md` 4.1（B1）・5 章（V1/V2）。依存追加なし（glTF JSON の読取のみ）。
- （7〜9）範囲: `VRM` / `VRMC_vrm` 拡張の検出と版表示、meta（作者・利用許諾）の読み取り・表示・provenance/license への自動転記、humanoid ボーン一覧表示と必須ボーン欠落検査（`3D-CHK-VRM-001/-002`）、expression / spring bone の有無表示（`3D-CHK-VRM-003`）、joint 階層ツリー・bind pose 整合・T/A-pose 推定・ウェイト異常検査（`3D-CHK-BONE-001〜003`）。非範囲 = VRM 描画反映（V3 = `3D-OPEN-22`）、humanoid 対応付け編集（第三段階 `3D-STAGE3-11`）、VRM の作成・meta 書き換え。
- （10〜13）依存: -02（Scene）・-05（skeleton 表示）。後続: `3D-STAGE3-11`。並行: -06。
- （20〜25）影響: 3D 側のみ（inspection report に VRM/BONE 検査を追加。asset3d.json への転記は既存 provenance/license フィールドの範囲内）。
- （29〜30）エラー / 復旧: VRM 拡張が壊れている場合も glTF としての読み込みは続行し、「VRM 情報を読み取れませんでした」と表示する（隔離しない）。
- （35）手順: 1. 拡張検出（0.x / 1.0 両対応） → 2. meta 読取と表示 → 3. license/provenance 転記 → 4. humanoid/BONE 検査 → 5. fixture とテスト。
- （36〜39）テスト: VRM 0.x / 1.0 fixture（自作。`3D-STAGE1-00` の生成基盤を拡張）での検出・meta・検査 unit、E2E（VRM 取込→meta 表示→ZIP README への転記確認）。
- （40〜42）受け入れ: 前提: 商用不可 meta を持つ自作 VRM fixture。操作: 取込 → Inspect。期待結果: 利用条件が日本語で表示され、license.declared との矛盾時に警告が出て、export README に転記される。前提: 必須ボーン欠落 fixture。操作: 検査。期待結果: `3D-CHK-VRM-002` が欠落ボーン名つきで警告される。
- （43〜44）rollback / 文書更新: PR revert で完全に戻る / interop 仕様の実装状況注記。
- （48）人間確認: `3D-DEC-VRM-01`（対応レベルの承認）を段階開始時に得る。
- （49〜52）: M / 中（VRM 0.x と 1.0 の構造差）/ 低 / 対処: 版ごとの読取を分離し、未知版は「未対応の VRM 版」と表示して glTF として続行。
- （53）依頼要約: 「VRM 拡張の検出・meta 表示・license 転記と、VRM/skeleton 検査（3D-CHK-VRM-001〜003 / BONE-001〜003）を、依存追加なしの JSON 読取で実装してください。VRM の編集・描画反映は含めないでください。」

#### 3D-STAGE2-GATE 第二段階終了 Gate

- 範囲: デモ手順（下記）、実機・性能・証拠整理、`3D-OPEN-05/-08/-15` の再判断と `3D-OPEN-22/-23` の初回判断、人間承認。
- デモ手順: 1. external-texture.gltf を bundle 取込 → 2. Scene で node 選択・非表示 → 3. Materials で alpha 警告確認 → 4. Animation で 0.5x 再生 → 5. hand node へ anchor 追従 → 6. mobile プリセットで再検品（確定しきい値） → 7. thumbnail 生成 → 8. ZIP に import notes 6 種を確認 → 9. VRM サンプルを取込み meta（利用条件）と humanoid 一覧が表示される → 10. iPhone で同素材の検品と書き出し。
- 受け入れ: 全 WP merge・CI 成功、デモ 10 手順の実機成功報告、BLOCKER/MUST ゼロ、人間承認。
...
高度な検品・**軽量化**・自動化・比較に加えて、**静的モデルを「動くゲーム素材」へ引き上げるリグ作成とモーション付与**（2026-07-20 第2改訂で追加。`3D_INTEROP_VRM_VR_AND_CREATION_SPEC.md` 4 章）。「読める・整えられる」から「**ゲームに載る重さと動きへ安全に近づけられる**」へ進める。元モデルの完全保持と、見た目・animation を壊す処理を自動適用しない原則を守る。

### 3.2 段階開始条件

`3D-STAGE2-GATE` 承認。glTF-Transform / meshoptimizer / Draco / KTX2 decoder の版固定ライセンス確認と dependency 人間承認（GATE-04 と同一手順の第 2 回）。加えて、圧縮 fixture（meshopt.glb / draco.glb / ktx2.glb）の生成に使う gltfpack（または gltf-transform の対応機能）の版・同梱物・ライセンス確認も同承認に含める（性能仕様 8 章の未確認項目）。Fable5（または人間）による軽量化方式の優先順判断。
...
| humanoid ボーン対応付け（B2/B3: 自動推定 + 手動確定、anchor 補助、retarget notes） | 採用（2026-07-20 改訂で追加） | `-11`（interop 仕様 4 章） |
| テクスチャ編集（2D 編集資産の再利用。baseColor 限定・derived 書き戻し・UV overlay） | 採用（2026-07-20 改訂で追加） | `-12`（interop 仕様 7 章。旧 `3D-OPEN-14` の再定義） |
| **骨格作成（R2）: humanoid テンプレート骨格のフィット** | 採用（2026-07-20 **第2改訂**で追加。静的な生成モデルを動かす動線の要） | `-13`（interop 仕様 4.3） |
| **自動ウェイト + rigged derived 生成（R3/R4）** | 採用（第2改訂） | `-14`（interop 仕様 4.4〜4.5） |
| **モーションテンプレート（M1: idle/walk/attack 等の手続き生成 → clip 焼き込み）** | 採用（第2改訂） | `-15`（interop 仕様 4.6） |
| **外部モーション retarget（M2: humanoid→humanoid）** | 採用（第2改訂） | `-16`（interop 仕様 4.6） |

### 3.4 段階の必須構成（要約）

| 項目 | 第三段階での内容 |
|---|---|
| 対象アセット / 入力 | Stage2 の全対象 + Meshopt / Draco / KTX2 圧縮済みモデル |
| 出力 | Stage2 の出力 + optimized derived（選択同梱）+ verification record + 決定的 ZIP |
| 画面 | + Optimize / Compare |
| データ形式 / 保存 | `derived[]` の実運用開始（契約 8 章。0.1.0 の定義済みフィールドのため version 据え置き） |
| 検品 | + 重複 / unused / 圧縮状態検査、検査履歴 |
| ゲーム用情報 | + humanoid 対応付け（`humanoidMap`）と humanoid 起点の anchor 補助（`-11`）。既存 anchor / collider は保持チェックの対象になる |
| テクスチャ | + baseColor 限定のテクスチャ編集（2D 編集資産の再利用、derived 書き戻し。`-12`） |
| リグ・モーション（第2改訂） | + 骨格フィット（`-13`）・自動ウェイト + rigged derived（`-14`）・モーションテンプレート（`-15`）・retarget（`-16`）。静的な生成モデルが「動くゲーム素材」になる |
| 性能 / 端末 | 重処理の進捗 1 秒以内・中断 2 秒以内・メモリ不足復旧 |
| アクセシビリティ | 進捗・比較結果の非視覚提示（差分表・保持チェックの文章化） |
| セキュリティ | WASM decoder の同梱・hash 記録（CDN 不使用）、encoder 入力の上限 |
| ライセンス | glTF-Transform / meshoptimizer / draco / basis の版固定確認 + NOTICE 整備（段階開始条件） |
| 未対応範囲 | エンジン向けバイト列自動変換（`3D-OPEN-19` 判断まで）、外部生成 |
| テスト証拠 | 圧縮 fixture、round-trip hash テスト、screenshot 比較、中断・復旧 E2E |
| 次段階へ進む条件 | STAGE3-GATE の受け入れ（自動破壊なしのレビュー確認 + 人間承認） |

### 3.5 work package

作業順: 01 → 10 → (02・03・11 並行) → (04・05・12 並行) → **13 → 14 → (15・16 並行)** → 06 → 07 → 08 → 09 → GATE。（`-10` の進捗・中断基盤を先に作り、以後の重処理 WP が乗る。`-11` は重処理と独立のため早期並行可。リグ・モーション系 13〜16 は 11 の humanoid 語彙と 01 の derived 配管に依存する。）
...
#### 3D-STAGE3-11 humanoid ボーン対応付け（B2/B3）

- （1〜3）: `3D-STAGE3-11` / humanoid 対応付け / 第三段階（2026-07-20 改訂で追加）
- （4）価値: 人型モデルの「右手」「頭」等を機械可読にでき、anchor 付けとエンジンでのリターゲットが確実になる。
- （5〜6）根拠: interop 仕様 4 章。語彙は VRM 1.0 humanoid ボーン一覧を採用。`3D-STAGE2-11` の skeleton 検査が前提。
- （7〜9）範囲: node 名からの対応付け自動推定（候補提示のみ）、手動対応付け UI（スロット一覧 × node 選択）、`asset3d.json` の `humanoidMap`（optional。契約 6 章に追加済み）への保存、humanoid スロット起点の anchor 追加補助（「右手に anchor」）、engine import notes への retarget 情報反映、VRM 素材では VRM 側 humanoid との一致検証。非範囲 = ボーンの作成・変形・ウェイト編集（非目標）、自動推定の無確認確定。
- （10〜13）依存: `3D-STAGE2-11`。後続: なし（-GATE まで）。並行: -02〜-10 と独立並行可。
- （20〜25）影響: asset3d.json に optional フィールド `humanoidMap` 追加（version 0.1.0 のまま。additive）。schema 追記。export の asset3d.json に含まれる。
- （29〜30）エラー / 復旧: 対応先 node が消えた `.cas3dproj` を読んだ場合は該当スロットを未対応へ戻し警告（nodeRef 退避と同じ規則）。
- （35）手順: 1. 語彙定数と型 → 2. 名前推定（表記ゆれ辞書: mixamorig: 等の接頭辞対応） → 3. 対応付け UI → 4. anchor 補助 → 5. notes 反映 → 6. fixture とテスト。
- （36〜39）テスト: 推定 unit（Mixamo 命名 / VRoid 命名 / 無規則命名 fixture）、E2E（対応付け→右手 anchor 追加→保存往復）。
- （40〜42）受け入れ: 前提: character-anim.glb（Mixamo 系命名）。操作: 自動推定 → 確認 → 確定。期待結果: 主要スロットが候補提示され、確定後「右手に anchor」で hand_R 追従 anchor が 1 操作で作られ、`.cas3dproj` 往復後も保持される。
- （43〜44）rollback / 文書更新: PR revert 可（humanoidMap は optional のため旧アプリでも未知フィールドとして保持される）/ 契約・interop 仕様の実装状況注記。
- （48）人間確認: 自動推定の合格基準（`3D-OPEN-25`）を段階開始時に確定。
- （49〜52）: M / 中（命名の多様性）/ 低 / 対処: 推定は候補提示に限定し、確定を必ず利用者操作にする。
- （53）依頼要約: 「VRM 1.0 語彙の humanoid 対応付け（自動推定候補 + 手動確定 UI + humanoidMap 保存 + anchor 補助 + notes 反映）を実装してください。ボーンの編集機能は含めないでください。」

#### 3D-STAGE3-12 テクスチャ編集ブリッジ（2D 編集資産の再利用）

- （1〜3）: `3D-STAGE3-12` / テクスチャ編集 / 第三段階（2026-07-20 改訂で追加）
- （4）価値: 「テクスチャの背景ゴミを消す」「色味を揃える」を、外部ペイントツールなしで完了できる（Chameleon 最大の差別化。2D 編集基盤の再利用）。
- （5〜6）根拠: interop 仕様 7 章。`src/core/images` の純関数（背景透過・HSL・パレット置換・消しゴム相当の erase 適用）と Worker 方式を読み取り専用で再利用（アーキテクチャ 3 章の規則）。
- （7〜9）範囲: Materials / Scene からのテクスチャ選択 → 編集ビュー（UV 展開線 overlay 付き）→ 適用で **texture 差し替え derived を生成**（glTF-Transform。`DerivedModel.kind: "texture-replaced"`）→ Compare 確認 → 書き出し対象選択。対象は baseColor のみ。material factor 数値調整は任意拡張（段階開始時に採否判断）。非範囲 = normal / metallic-roughness 等の編集（`3D-OPEN-26`）、UV 編集、3D ペイント、source の書き換え。
- （10〜13）依存: `3D-STAGE3-01`（derived 配管）・`-10`（進捗基盤）。後続: -07（Compare で確認）。並行: -04・-05。
- （17〜19）変更 / 新規 / 禁止: 新規 = `core3d/texture-edit/**`、編集ビュー UI。**`src/core/images` は import のみで変更禁止**（変更が必要なら core3d 側へラップ/コピー）。
- （20〜25）影響: DerivedModel.kind に `texture-replaced` 追加（契約 8 章に追記済み。0.1.0 のまま）。export は既存 derived 同梱の仕組みに乗る。
- （29〜30）エラー / 復旧: 編集は一時バッファ上で行い、適用まで何も書かない。適用失敗時は derived を残さない。
- （31〜33）性能 / メモリ / セキュリティ: 4096px 超は編集前に縮小を促す。編集中バッファは 1 枚分のみ保持し、離脱時に解放。色空間は sRGB 固定で変更しない。
- （35）手順: 1. texture 抽出と UV overlay → 2. 2D 画像 ops の接続（Worker） → 3. 差し替え derived 生成 → 4. Compare 接続 → 5. fixture とテスト。
- （36〜39）テスト: 差し替え round-trip unit（source 不変 hash + derived の texture bytes 一致）、E2E（背景透過 → 適用 → Compare → 書き出し）、fixture = box-textured.glb + 汚れ入り texture 版。
- （40〜42）受け入れ: 前提: 背景に不要色が残る texture を持つ fixture。操作: テクスチャ編集で背景透過 → 適用 → Compare。期待結果: derived がモデル表示に反映され、source の sha256 不変、拒否したら derived が残らない。
- （43〜44）rollback / 文書更新: 操作 UI の無効化のみで安全に停止可（derived 方式のため）/ interop 仕様の実装状況注記。
- （48）人間確認: 段階開始時に対象 ops の範囲（factor 調整を含めるか）を判断。
- （49〜52）: L / 中（UV overlay と大判 texture の扱い）/ 中 / 対処: baseColor 限定・sRGB 固定・derived 方式で「壊せない」構造にする。
- （53）依頼要約: 「baseColor テクスチャ限定の編集ブリッジ（2D 画像 ops の読み取り専用再利用・UV overlay・texture-replaced derived 生成・Compare 接続）を実装してください。source と `src/core/images` は変更しないでください。」

#### 3D-STAGE3-13 骨格作成（humanoid テンプレートのフィット。R2）

- （1〜3）: `3D-STAGE3-13` / 骨格フィット / 第三段階（2026-07-20 第2改訂で追加）
- （4）価値: **画像→3D で作った静的モデルに骨を入れられる**。動くキャラクターへの最初の関門をツール内で越えられる。
- （5〜6）根拠: interop 仕様 4.3。前提 = Setup 完了（⚠️ `3D-RISK-12`）・humanoid 語彙（`-11` と共通の定数）。
- （7〜9）範囲: humanoid テンプレート骨格の初期自動配置（bounds / feet / 身長推定・標準比率）、ギズモ + 数値によるボーン端点調整（左右対称既定 ON）、`rigDraft` への保存（契約 6 章）と Undo / Redo、骨格プレビュー表示（骨の可視化）。非範囲 = ウェイト（`-14`）、非人型カスタム骨格（`3D-OPEN-28`）、ボーンの自由追加・削除。
- （10〜13）依存: `3D-STAGE3-11`（humanoid 語彙・UI 共通部）。後続: `-14`。並行: -02〜-10 と独立並行可。
- （20〜25）影響: asset3d.json に optional `rigDraft` 追加（0.1.0 のまま。additive）。schema 追記。
- （26〜28）操作: PC = ボーン端点のドラッグ + 数値。タッチ = ハンドル操作（44px）。すべての端点は数値入力でも編集可能（アクセシビリティ）。
- （29〜30）エラー / 復旧: Setup 未完了時は開始をブロックして誘導。フィット結果が bounds を大きく外れたら警告。rigDraft はいつでも破棄して再フィット可能。
- （31〜33）性能 / セキュリティ: プレビューは overlay 描画のみ（モデル変更なし）。保存は metadata のみで軽量。
- （35）手順: 1. テンプレ定義（標準比率・階層） → 2. 自動初期配置 → 3. 対称編集 UI → 4. rigDraft 保存 / Undo → 5. fixture とテスト。
- （36〜39）テスト: 初期配置 unit（unrigged-character fixture で全端点が bounds 内 + feet 接地）、E2E（フィット → 保存往復 → rigDraft 復元）。
- （40〜42）受け入れ: 前提: Setup 完了済みの unrigged-character.glb。操作: 「骨格を作成」→ 自動配置 → 肩位置を数値修正。期待結果: 骨格が overlay 表示され、rigDraft が保存され、リロード後も編集を再開でき、source の sha256 は不変。
- （43〜44）rollback / 文書更新: PR revert 可（rigDraft は optional・未知フィールド保持で旧アプリでも安全）/ interop 仕様の実装状況注記。
- （48）人間確認: `3D-DEC-RIG-01`（リグ作成の採用レベル）を段階開始時に承認。
- （49〜52）: L / 中（自動配置の当たり精度）/ 中 / 対処: 自動配置は初期値にすぎないと UI に明示し、手動調整を前提の動線にする。
- （53）依頼要約: 「humanoid テンプレート骨格の自動初期配置と対称編集（ギズモ + 数値、rigDraft 保存、Undo、Setup 完了ガード）を実装してください。ウェイトとモデル変更は含めないでください。」

#### 3D-STAGE3-14 自動ウェイトと rigged derived 生成（R3/R4）

- （1〜3）: `3D-STAGE3-14` / 自動ウェイト・rigged 生成 / 第三段階（第2改訂で追加）
- （4）価値: 骨格に皮膚（メッシュ）が追従するようになり、モーションを受け入れられるモデルが完成する。
- （5〜6）根拠: interop 仕様 4.4〜4.5、契約 7.3（nodeBinding）・8 章（kind: rigged / parentRef）。
- （7〜9）範囲: 距離ベース envelope 自動ウェイト（Worker、進捗・中断対応）、頂点あたり最大 4 影響（⚠️ `3D-RISK-11`）、ウェイトヒートマップ表示・テストポーズ確認・ウェイト検査（`3D-CHK-BONE-003` 再利用）、ボーン単位の影響半径・減衰調整 → 再計算、適用で rigged derived 生成（skin 書き込み・recipe に rigSnapshot・**nodeBinding の再バインド** ⚠️ `3D-RISK-02`）。非範囲 = 頂点単位ペイント（`3D-OPEN-29`）、heat diffusion 等の高品質方式。
- （10〜13）依存: `-13`（rigDraft）・`-01`（derived 配管）・`-10`（進捗基盤）。後続: `-15` / `-16`。
- （20〜25）影響: DerivedModel kind: rigged の実運用開始。nodeBinding の切替が発生（契約 7.3 の規則どおり）。
- （29〜30）エラー / 復旧: 生成失敗時は derived を残さない。nodeBinding 照合失敗時は node 追従系を未解決表示（黙って誤バインドしない）。
- （31〜33）性能: 10 万頂点 × 30 ボーンで 30 秒以内（暫定・Worker・進捗表示）。中断 2 秒以内。
- （35）手順: 1. envelope 計算（Worker） → 2. ヒートマップ / テストポーズ → 3. 検査接続 → 4. rigged derived 生成 + 再バインド → 5. Compare 接続 → 6. fixture とテスト。
- （36〜39）テスト: ウェイト unit（対称モデルで左右対称なウェイト・合計=1・最大 4 影響）、再バインド unit（node 追従 anchor が rigged derived の正しい node を指す）、E2E（フィット → ウェイト → 生成 → テストポーズ確認）。
- （40〜42）受け入れ: 前提: rigDraft 設定済み unrigged-character.glb。操作: 自動ウェイト → テストポーズ確認 → 適用。期待結果: rigged derived が生成され、hand_R 追従 anchor が rigged derived の手ボーンへ再バインドされ（nodeBinding 更新）、source 不変、拒否時は何も残らない。
- （43〜44）rollback: 操作 UI 無効化で停止可（derived 方式）。（48）人間確認: 不要（-13 の承認に含む）。
- （49〜52）: XL / 高（ウェイト品質のばらつき）/ 高（node 再バインドの整合） / 対処: **PR を「worker 計算」「UI・プレビュー」「derived 生成 + 再バインド」の 3 分割**にし、再バインドは専用テストで固める。品質限界は「外部ツール推奨」の文言で明示。
- （53）依頼要約: 「envelope 自動ウェイト（Worker・最大4影響・ヒートマップ・テストポーズ・検査）と rigged derived 生成（recipe 記録・nodeBinding 再バインド・専用テスト）を実装してください。頂点ペイントは含めないでください。」

#### 3D-STAGE3-15 モーションテンプレート（M1）

- （1〜3）: `3D-STAGE3-15` / モーションテンプレート / 第三段階（第2改訂で追加）
- （4）価値: 骨を入れたモデルが**その場で歩き・跳び・攻撃する**。ゲーム素材としての完成に直結する（2D のモーションテンプレートで実証済みの価値の 3D 版）。
- （5〜6）根拠: interop 仕様 4.6。2D の焼き込み方式（`docs/DATA_FORMAT.md` 6.8）と同じ「テンプレ → 生成 → 通常クリップとして扱う」思想。ただし実装は 3D 専用（⚠️ `3D-RISK-06`: 2D 実装を流用しない）。
- （7〜9）範囲: humanoid 手続き生成モーション（idle / walk / run / jump / attack / damage / dead。語彙は 2D の `ANIMATION_NAME_SUGGESTIONS` と統一）、パラメータ（速度・振り幅の少数ノブ）、プレビュー再生 → 承認で glTF animation clip として焼き込み（rigged derived へ追記 or motion-baked derived 生成。⚠️ `3D-RISK-01`: settings をクリップへ二重適用しない）。非範囲 = キーフレーム編集（`3D-OPEN-30`）、物理・IK、外部モーションデータの同梱（`3D-OPEN-31`）。
- （10〜13）依存: `-14`（rigged derived）または humanoid 対応付け済みの既存 rigged モデル（`-11`）。並行: `-16`。
- （29〜30）エラー: humanoid 必須スロット不足時は生成不可の理由（不足スロット名）を表示。
- （36〜39）テスト: 生成 unit（クリップ長・channel 数・対象 joint が humanoidMap と一致）、E2E（walk 適用 → Animation 画面で再生 → 書き出し ZIP に clip が含まれる）、visual（テストポーズ screenshot）。
- （40〜42）受け入れ: 前提: rigged derived を持つアセット。操作: walk テンプレを適用。期待結果: プレビューで歩行が再生され、承認後 clip が保存され、export ZIP のモデルで clip が再生可能（engine notes の再生手順つき）。
- （43〜44）rollback: clip 単位で削除可能（derived の recipe に記録）。（48）人間確認: 不要。
- （49〜52）: L / 高（見た目の自然さ）/ 中 / 対処: 「機械的な動きであり、品質が必要なら外部モーションを retarget」と UI に明記。テンプレは少数から始めて増やす。
- （53）依頼要約: 「humanoid 手続き生成のモーションテンプレート（7 種・パラメータ少数・プレビュー承認 → clip 焼き込み）を実装してください。キーフレーム編集は含めないでください。」

#### 3D-STAGE3-16 外部モーション retarget（M2）

- （1〜3）: `3D-STAGE3-16` / モーション retarget / 第三段階（第2改訂で追加）
- （4）価値: Mixamo 等の高品質モーションを自分のモデルで使える。テンプレの品質限界を外部資産で補える。
- （5〜6）根拠: interop 仕様 4.6 / 3.2（Mixamo→Blender→GLB 経路）。
- （7〜9）範囲: 「モーション元 GLB（humanoid 対応付け済み）」の取り込み枠、スロット単位の回転 retarget + hips 位置スケール補正、プレビュー → 承認 → clip 焼き込み、保持チェック（`-07`）接続。非範囲 = 骨長差の高度な補正（足滑り補正等）・表情/指の細部 retarget（対象スロットのみ）。
- （10〜13）依存: `-14`・`-11`（両モデルの humanoidMap）。並行: `-15`。
- （29〜30）エラー: 元・先いずれかの humanoidMap 不足時は不足スロットを列挙。骨長差が極端な場合は品質警告。
- （36〜39）テスト: retarget unit（同一骨格間で恒等・鏡像骨格で対称）、E2E（Mixamo 系 fixture クリップ → 自作モデルへ retarget → 再生）。fixture のモーション元は**自作**（テンプレ生成クリップを別骨格へ載せた物）とし、Mixamo 実データはリポジトリに入れない（ライセンス）。
- （40〜42）受け入れ: 前提: humanoid 対応付け済みのモデル A（クリップ持ち）と B。操作: A の walk を B へ retarget。期待結果: プレビューで破綻なく再生され、承認後 B の clip として保存・書き出しできる。拒否時は何も残らない。
- （43〜44）rollback: clip 単位で削除可能。（48）人間確認: 不要。
- （49〜52）: L / 高（骨格差の吸収）/ 中 / 対処: 対応範囲（humanoid 主要スロットのみ・足滑りは許容）を明記し、限界を隠さない。
- （53）依頼要約: 「humanoid スロット基準のモーション retarget（回転 + hips スケール・プレビュー承認・保持チェック接続）を実装してください。指・表情・足滑り補正は対象外と明記してください。」

#### 3D-STAGE3-GATE 第三段階終了 Gate

- デモ手順: 1. draco.glb 読み込み → 2. 一括検品 → 3. big-texture を 2048 化 + Meshopt 出力 → 4. Compare で保持チェック → 5. 派生を選んで決定的 ZIP を 2 回出し hash 一致確認 → 6. 中断とメモリ不足復旧の実演 → 7. character-anim.glb の humanoid 対応付け →「右手に anchor」→ 8. texture の背景透過編集 → Compare → 書き出し → **9. unrigged-character.glb（静的）に骨格フィット → 自動ウェイト → rigged derived 生成（hand anchor の再バインド確認）→ walk テンプレ適用 → 別モデルのクリップを retarget → 書き出し ZIP のクリップ再生確認**。
...
| 項目 | 第四段階での内容 |
|---|---|
| 対象アセット / 入力 | 完成条件の 10 種デモモデル（静止 / animation / 複数 material / 外部 texture / 圧縮 / AI 生成申告 / 破損 / 過大 / モバイル向け / PC 向け） |
| 出力 | 変更なし（verified 昇格による表示区分の変化のみ） |
| 画面 | 原則新画面なし（アクセシビリティ・文言の完成が主）。条件付きで WebXR preview（`-09`。承認時のみ） |
| データ形式 / 保存 | migration リハーサル（本番形式は変更しない）・旧形式読み込み証拠 |
| 検品 | 変更なし（総点検のみ） |
| 性能 / 端末 | 全予算項目の確定実測・低性能 / WebGL 制限端末の記録 |
| アクセシビリティ | キーボード完結・SR 代替・非色依存の監査完了 |
| セキュリティ | 敵対 fixture 総点検・オフライン動作・静的検査 |
| ライセンス | 台帳最終化・NOTICE / attribution の表記完成 |
| 未対応範囲 | `3D_COMPLETE_PRODUCT_SPEC.md` 5 章の非目標すべて（完成の妨げにしない） |
| テスト証拠 | エンジン実行検証（自動）+ 手動証拠、性能確定報告、完成条件 12 項目の証拠リンク集 |
| 次段階へ進む条件 | なし（STAGE4-GATE = 最終完成判定） |

### 4.4 work package

作業順: (01・03・04・05・06 並行可) → 07 → 08 → GATE。02（外部接続）は `3D-DEC-EXTGEN-01` で承認された場合のみ 01 の後に実施し、承認されなければ省略する（06 以降は 02 の有無に関わらず進む）。09（WebXR preview）も条件付きで、`3D-OPEN-23` 承認時のみ 01〜06 と並行して実施する。
...
- （7〜9）範囲: 共通 adapter 仕様の確定（プロトコル案は interop 仕様 8.2。入力 = 画像等 / 出力 = GLB+付帯情報 / 失敗時のデータ保全 / 記録形式 / job 状態遷移）、**2D→3D ブリッジ**（生成用入力として既存 2D プロジェクトのアセット画像を読み取り専用で選べる入口。interop 仕様 8.1）、手動持ち込みフローの仕上げ（provenance 入力 UI・external-generator 申告・生成物向け検品ガイド文）、provider 候補（TripoSR / Stable Fast 3D / SPAR3D / TRELLIS / Hunyuan3D 系）ごとのライセンス確認**記録表の枠**作成（確認自体は接続採用時。2026-07-20 時点の確認状況は interop 仕様 8.3）。非範囲 = 実接続（-02）。
- （7〜9）参照資料: [Three.js Object Sculptor](https://github.com/vinhhien112/Three.js-Object-Sculptor-Codex-Plugin/tree/543da1fc0e45a703b0ac037fb040ce082c79a1c2)。画像から、設計データ `ObjectSculptSpec` とコードで組み立てる Three.js model factory を生成し、部品分解、pivots / sockets / animation anchors、段階的な品質 gate、元画像と描画結果の比較証拠を扱う。参照する理由は、Chameleon の docs-first、階層・anchor / collider、inspection / evidence、ブラウザゲーム向けという設計に近く、外部 3D 生成を「一回の mesh 生成」以外へ広げられるため。標準出力は GLB ではないため、直接の実装候補ではなく `3D-STAGE4-01` の設計参照とする。評価する場合も外部の隔離処理で `Three.js code + ObjectSculptSpec → GLB + sidecar + evidence` へ変換し、Chameleon 本体は生成コードを実行せず、plugin / dependency も追加しない。詳細は interop 仕様 8.4。
...
- （7〜9）範囲: Three.js / Babylon.js の実行検証（export ZIP を実プロジェクトで読み込む fixture テストの自動化。**rigged + motion 付き fixture の skin 描画・クリップ再生を検証対象に含める**。2026-07-20 第2改訂）、Godot / Unity は手動取り込み証拠（`3D_TEST_EVIDENCE_AND_RELEASE_SPEC.md` 4 章形式）、表示区分の昇格（import notes only → verified）。証拠が揃わない対象は昇格しない。⚠️ `3D-RISK-11`（エンジン別の joint 数・頂点あたり影響数の上限は notes と書き出し検査の両方で扱う）。
...
- （10〜13）依存: `3D-STAGE3-GATE`（保存・migration 機構が固まっていること）。**`3D-STAGE4-02`（外部接続）には依存しない**（migration/rollback は外部接続の有無と独立。STAGE4-02 が不実施でも本 WP は実施する）。後続: -07。
...
#### 3D-STAGE4-09 WebXR preview（条件付き）

- （1〜3）: `3D-STAGE4-09` / VR での実寸確認 / 第四段階（**条件付き**。2026-07-20 改訂で追加）
- （4）価値: VR 向け素材を実寸・実環境で最終確認できる（1.8m のキャラクターを実際に 1.8m で見る）。
- （5〜6）根拠: interop 仕様 6.2。`3D-OPEN-23` の承認時のみ実施（既定: 不採用でも完成）。
- （7〜9）範囲: WebXR 対応環境の feature detection、閲覧専用の immersive preview（スナップ移動のみ・編集なし・motion sickness 配慮）、非対応環境でのボタン非表示。非範囲 = VR 内編集、iPhone/iPad 対応（WebXR 非対応のため対象外と明記）、プラットフォーム（VRChat 等）固有対応。
- （10〜13）依存: `3D-STAGE3-GATE`。後続: 採用時のみ -07 の監査対象に含める。並行: -01/-03〜-06。
- （31〜33）性能: vr プリセット合格 fixture で 72fps 目標（暫定）。session 終了時の GPU リソース解放を dispose 契約に含める。
- （40〜42）受け入れ: 前提: WebXR 対応環境 + vr プリセット合格 fixture。操作: preview 開始 → モデル周囲を移動 → 終了。期待結果: 実寸表示され、終了後に Object URL / GPU リソースが解放されている（カウンタ assert）。非対応環境ではボタン自体が表示されない。
- （48）人間確認: **必須**（`3D-OPEN-23` の採否 + 対応 HMD の確定）。（49〜52）: M / 高（環境依存）/ 低 / 対処: 不採用でも完成条件に影響しない設計を維持。
- （53）依頼要約: 「承認された場合のみ、閲覧専用 WebXR preview（feature detection・スナップ移動・解放保証）を実装してください。iPhone/iPad は対象外と明記してください。」

#### 3D-STAGE4-GATE 最終完成判定

- 範囲: `3D_COMPLETE_PRODUCT_SPEC.md` 8 章の完成条件 12 項目を 1 項目ずつ証拠に紐づけて確認し、人間が「3D 機能の完成」を宣言する。
- デモ手順（最終）: 静止モデル / animation キャラ / 複数 material / 外部 texture glTF / 圧縮済み / AI 生成（申告つき）/ 壊れた / 過大 / モバイル向け / PC 向けの 10 種のモデルを、取込→検品→設定→（必要なら最適化・比較）→書き出し→エンジン読み込みまで通す。**AI 生成（静的）モデルについては、骨格フィット→自動ウェイト→モーションテンプレ→エンジンでのクリップ再生まで**を必ず含める（interop 仕様 8.0 の動線が行き止まりなく完了する証拠。2026-07-20 第2改訂）。
...
flowchart TB
  ...
  subgraph S2["第二段階"]
    ...
    S205 --> S207
    S206 --> S208["08 import notes(6対象)"]
    S207 --> S208
    S203 --> S209["09 thumbnail"]
    S202 --> S211["11 VRM/skeleton検査"]
    S205 --> S211
    S208 --> S210["10 モバイル/復旧"]
    S209 --> S210
    S211 --> S2G["STAGE2-GATE"]
    S210 --> S2G
  end
  subgraph S3["第三段階"]
    ...
    S306 --> S307
    S310 --> S312["12 texture編集"]
    S312 --> S307
    S311["11 humanoid対応付け"]
    S311 --> S313["13 骨格フィット(R2)"]
    S313 --> S314["14 自動ウェイト/rigged(R3-R4)"]
    S310 --> S314
    S314 --> S315["15 モーションテンプレ(M1)"]
    S314 --> S316["16 retarget(M2)"]
    S311 --> S316
  ...
  subgraph S4["第四段階"]
    S401["01 adapter仕様"] -.承認時のみ.-> S402["02 外部接続(条件付)"]
    S403["03 engine verified"]
    S404["04 アクセシビリティ"]
    S405["05 性能証拠"]
    S406["06 migration証拠"]
    S409["09 WebXR preview(条件付)"]
...
（STAGE1-00 の fixture 生成基盤は品質レーンで STAGE1-01 と並行に進むため critical path には現れないが、STAGE1-04 の前に完了している必要がある。STAGE4-06 は STAGE3-GATE 直後から STAGE4-07 までの間に実施する。2026-07-20 改訂で追加した STAGE2-11 / STAGE3-11 / STAGE4-09 は critical path 外の並行 WP、STAGE3-12 は Compare（STAGE3-07）の前までに完了すればよい。）
...
- 想定 PR 総数: Gate 6（記録中心）+ Stage1 約 13（00〜11 + dependency 分割）+ Stage2 約 11（+VRM/skeleton）+ Stage3 約 20（+humanoid、texture 編集 2 PR、骨格フィット、**自動ウェイト = 3 PR 分割**、モーションテンプレ、retarget）+ Stage4 約 10（+条件付き WebXR） = **約 60 PR**（±20% の幅を見込む。条件付き WP の不採用でこの数は減る。カレンダー日程は約束しない）。
...
## 付録 A: work package 53 項目書式

各 WP は次の 53 項目を書式とする（A 節の共通既定で全 WP 共通の項目を定め、各 WP には差分だけを書く）。この番号が本書中の（1)〜(53)の意味の正本である。
...
各 WP では紙面の都合上、連続する項目を範囲でまとめて記載する（例: 「（7〜9）目的 / 範囲 / 非範囲」は 7・8・9 を、「（49〜52）」は複雑度・不確実性・危険度・対処をまとめる）。共通既定に含まれ差分の無い項目は各 WP で省略する。
