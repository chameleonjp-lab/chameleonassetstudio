...
最終更新日: 2026-07-20（第2改訂: 静的モデル→リグ→モーションの動線を完成形・完成条件に追加）
...
6. **ボーンが無い静的モデル（画像→3D 生成物の標準状態）には、humanoid テンプレート骨格をフィットし、自動ウェイトで rigged モデル（派生）を作り、モーションテンプレートや外部モーションの retarget で動きを付ける**（2026-07-20 第2改訂で追加。ボーン付きモデルは humanoid 対応付けで同じ動線に乗る）。
7. Game Data で anchor（手・足元・発射位置など）と collider（box / sphere / capsule）を付ける（骨追従含む）。
8. 必要なら Optimize で軽量化した派生モデルを作り、Compare で見た目・サイズ・animation 保持を比較して選ぶ。
9. Export で ZIP（モデル + metadata + 検査報告 + README + import notes + manifest/hash）を書き出す。
10. Three.js / Babylon.js には verified の手順とサンプルで、Godot / Unity には import notes で持ち込む。
11. `.cas3dproj` を保存すれば、別の日・別の端末でも作業を再開できる。

（画像 1 枚から動くキャラクターまでの標準動線図は `3D_INTEROP_VRM_VR_AND_CREATION_SPEC.md` 8.0 を参照。）
...
| **静的キャラクター（ボーン無し）** | 画像→3D 生成物の標準状態 | **骨格フィット + 自動ウェイト + モーション付与で「動くキャラクター」へ引き上げ可能**（2026-07-20 第2改訂） |
| animation 付きキャラクター | skin + クリップ複数 | 完全対応（再生確認・保持確認・retarget の元としても利用可） |
...
| VRM（人型アバター） | VRoid Studio 等の出力 | 検出・meta 利用条件表示・humanoid 検査まで対応（2026-07-20 改訂。`3D_INTEROP_VRM_VR_AND_CREATION_SPEC.md` 5 章）。VRM の作成・編集はしない |
| VR 向けモデル | VR ゲーム用素材 | vr 検品プリセットで対応。VR での実寸確認（WebXR）は条件付き |
...
- 総合モデリング（sculpt / retopology / UV 編集 / texture painting）
- **本格的な** rig 作成（頂点ウェイトペイント・自由骨格編集・物理ボーン）と**本格的な**アニメーション制作（カーブエディタ・IK・レイヤー合成）— テンプレート骨格 + 自動ウェイト + テンプレモーション + retarget という**限定形は 2026-07-20 第2改訂で目標に含めた**（`3D_INTEROP_VRM_VR_AND_CREATION_SPEC.md` 4 章）
...
限定的な作成・修正機能のうち、**テクスチャ編集（baseColor 限定）と humanoid ボーン対応付けは 2026-07-20 改訂で採用**した（`3D-STAGE3-11/-12`。設計は `3D_INTEROP_VRM_VR_AND_CREATION_SPEC.md`）。それ以外（primitive 追加、node 整理、material factor 調整、variant 作成など）は引き続き open 項目とし、採用しなくても完成と判定できる。VRM アバターの作成（VRoid の代替）・spring bone / expression の編集も非目標である。
...
- 入力: GLB、glTF + bin + textures（bundle）、VRM（glTF 拡張として読み込み。拡張データは不変保持）、`.cas3dproj`
...
1. 3 章の一続きの体験が、PC / iPad / iPhone の対象端末で実際に完了できる（実機確認報告あり）。**「画像→（外部生成）→静的モデル→骨格フィット→自動ウェイト→モーション付与→書き出し→エンジンでクリップ再生」の動線（2026-07-20 第2改訂で追加）を含む**。
