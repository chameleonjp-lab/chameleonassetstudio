...
文書種別: Phase 17 後の旧実装計画
現在の正本: `2D_COMPLETE_PRODUCT_SPEC.md`, `2D_COMPLETION_ROADMAP.md`
旧計画・関連文書: `POST_PHASE17_REQUIREMENTS.md`, `THREE_D_ASSET_PREPARATION_REQUIREMENTS.md`

---

> **2026-07-10 の優先関係:** 本文書は Phase 18〜21 の実装済み範囲と旧計画を残す。今後の着手順と完成判定は `2D_COMPLETION_ROADMAP.md` を優先する。特に、本文 8〜14 の旧 Phase 22〜28 は **2D Pro Gate 待ちの凍結計画** であり、3D の library 評価、dependency 追加、実装開始の根拠にしてはいけない。
...
| 22〜28 | 3D の旧計画（凍結） | 2D Pro Gate 承認後に `3D-0`〜`3D-6` として再開する。詳細は `2D_COMPLETION_ROADMAP.md`。 |
...
- 反転コピー（アセット全体→新規アセット生成）は実装済み（`flipCopyAsset` / 「アセット」欄の「独立左右反転コピーを作成」ボタン / Unit・E2E）。メタデータ反転・role 入れ替え・id 付け替え・画像 Blob 複製を含む。`asset.json` schema / version 変更なし。
...
- 実装前に `docs/future/EXPORT_QUALITY_DESIGN.md` を読む。
- 推奨スコープは、padding / scale / includeHelpers を**デフォルト無効（現行出力と同一）の export 関数オプション**として別 PR にスライスすること。
- いずれのスライスでも `atlas.json` の format / version、`asset.json` / `.casproj` / export-presets の schema / version、export ZIP のディレクトリ構成は変更しない。
- `atlas.json` へのメタ情報追加、`ExportPreset` の新フィールド、extrude、パッキングアルゴリズム変更は人間確認を通してから別 PR で扱う。
...
## 14. [旧計画・凍結] Phase 28: 外部 3D 生成連携の設計
...
まずは Phase 18 の整合確認を土台に、`2D_COMPLETION_ROADMAP.md` の順番で 2D 制作体験を完成させる。3D 対応は、2D Pro Gate の人間承認後に、生成 AI ではなく GLB / glTF の読み込みと検品から始める。
