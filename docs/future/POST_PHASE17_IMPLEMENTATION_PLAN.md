...
- 実装済み: 既存 rect / circle のキャンバス選択、hover / selected 強調、rect 移動・リサイズ、circle 移動・radius 変更、purpose 凡例、sensor 破線表示、GameDataPanel との選択同期を追加した。
- 実装済み範囲は UI 操作に限定し、polygon、frame-specific collider、schema、asset.json version、`.casproj`、export ZIP は変更しない。
