...
新規Asset入口だけがSVG / GIF / APNGを通常画像とのmixed batchで受け、最大16 fileをSlice Cと同じmemory staging、共通preview、原子保存、History 1 entryへ通す。既存Assetへのlayer追加、連番、sheet、tileset、atlasのMIME gateは変更しない。
...
active / external SVG、17frame超、unsupported形式は正本を変更せず理由付き拒否し、quarantineへ保存しない。署名不一致、decode失敗、寸法超過だけは既存quarantine境界を使う。本Sliceはschema / version / migration、IndexedDB構造、`.casproj`配置、product exportを変更しない。
