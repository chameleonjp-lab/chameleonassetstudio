...
### 5.1 AssetFamily / Variant（2D-2-VARIANT Slice A）
...
linked variantの`fingerprint`は最後に同期したbase / variantの決定的hashと`syncedAt`（date-time）を持つ。Project / Asset ID、表示名、`updatedAt`はhash対象にせず、`.casproj` importでIDを付け替えても値を保持する。具体的なhash方式、stale判定、refresh処理はSlice Cで実装する。base保存だけでvariantをbackground更新しない。

recipeの`idMap`は`textures` / `layers` / `parts` / `anchors` / `colliders` / `frames` / `animations`ごとに、base内部IDからvariant内部IDへの対応を持つ。Asset IDと名前空間を混ぜない。`writeSet`は同じ要素種別のvariant側ID配列と`blobPaths`（TextureRef.path相当の相対path）を持ち、配列全体を手動調整保護の1単位として扱う。source Blob、Assetの`id` / `name` / `displayName` / `createdAt`、Family membership、対象外fieldはwrite-setに含めない。
