...
Group 12のR1 / ADR-0022では、現行bakeのLayer中心計算とaccepted座標契約の不一致を製品実装前に修正し、rigを先にflipしてbakeした結果と、先にbakeしてFrameをflipした結果の同値をfixtureで固定する。bakeは割当前に有限値、参照、循環、生成Frame / LayerState / serialized bytes / sheet pixelをpreflightし、上限超過を原子的に拒否する。上限値はH3のbrowser・iPhone Safari測定と人間承認待ちである。
