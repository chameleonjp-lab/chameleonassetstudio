...
Group 12のP1 / ADR-0023は、初回part replaceを既存Partの`layerIds`だけを既存Layer ID集合へ置き換える静的操作に限定する。その他のPart field、Layer / Texture / Blob、Frame / Animation / RigAnimation、既存bake済みFrameは変えない。空集合と1 Layerの複数Part所属はH2の人間判断待ちであり、製品実装は未着手である。
