...
- Import provenance: source Blob原本bytesのSHA-256、strict source recordと既存open recordのschema互換、旧0.1.0無変換、複製 / flip copyのtexture参照規則、dangling参照のread-only検査、canonical `asset.json`保持とengine向け派生出力への非流出
...
- 確認観点: 主要画面の表示、取り込み → 編集 → 書き出し一連、WebP 書き出しの可否表示、ダウンロード動作（Blob URL）、タッチ操作、iOS Filesからのatlas.json + spritesheet.png選択、native dialog focus、safe-area、画面キーボード、大きなAtlasのmemory
