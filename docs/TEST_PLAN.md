...
- schemaと意味検証: rect / circleの完全geometry、`visible`だけの上書き、field単位fallback、空配列、未知field保持、重複Asset collider ID、Frame内重複参照、dangling参照、shape不一致、非有限値、0以下の寸法を扱う。無効dataを黙って修復・fallbackしない。
- 編集: Frame選択・停止中だけのscope、rect / circle編集とpreview、`visible`のinherit / show / hide、geometry resetと全reset、Enter / blurの最大1 History、Escape取消、no-op抑止、`visible: false`の再編集を扱う。
...
- 保存・復旧: History / autosave / IndexedDB、reload、旧dataのfield不在維持、新dataの`asset.json` / `.casproj` exact roundtrip、容量・保存失敗時rollbackを扱う。
