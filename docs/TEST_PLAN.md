...
### 3.2 Group 12 Timeline / Rig実装Gate（未着手）

正本は`docs/future/2D_3_TIMELINE_RIG_PLAN.md`。T1 / R1 / P1の中核契約はacceptedだが、H1〜H3の人間判断が揃うまで製品実装と実装test追加を開始しない。

| Slice | Unit / contract | Chromium E2E・保存 | 実機 |
| --- | --- | --- | --- |
| T1 | `1000/fps` fallback、Frame duration override、反復Frame、loop / event、安全payload、duplicate / delete / flip | mock clock順序、Undo / Redo、reload、IndexedDB、`.casproj`、対応不能exportのH1挙動 | 長いtimeline、keyboard、入力zoom、44px、縦横 |
| R1 | 完全ID graph、鏡映式、source不変、double flip、親子 / pivot / bind / limit / scale | 全Frameのbake parity、Undo / Redo、reload、上限超過の原子的拒否 | 採用上限でbake、操作応答、Safari reload / crashなし |
| P1 | `Part.layerIds`だけのexact write-set、missing / duplicate / empty / order / ownership、他field不変 | 既存bake不変、次回bakeだけ反映、1 History、Undo / Redo、reload、`.casproj` | touch選択、長いLayer一覧、keyboard後の確定 / 取消 |

R1 parity fixtureは左右Part、親子3段以上、非zero pivot、bind pose、rotation limit、複数keyframe、負scale、非等方scaleを含める。ID・timestampを正規化し、transform数値と最終pixelを比較する。

bake性能はNode純関数、実browser、iPhone Safariを分けて測る。Frame数、LayerState数、serialized bytes、click-to-complete、Long Task、autosave、Undo / Redo、reload、Safari reload / crashを記録し、Node結果だけでiPhone Gateを通過させない。
