...
### 3.2 Group 12 Timeline / Rig計測・実装Gate（製品実装は未着手）

正本は`docs/future/2D_3_TIMELINE_RIG_PLAN.md`。PR #146 merge `cb21ea4`後にH1=E1、H2=L1、H3=M1を人間承認した。製品実装は別の明示指示まで開始せず、H3数値budgetは`docs/future/2D_3_H3_MEASUREMENT_PROTOCOL.md`の実測と後続人間承認まで未決定とする。
...
PR CIと公開workflowでは、専用Playwright設定でbuild済みH3ページをChromiumから開き、baselineを3 warm-up / 10 sampleで完了し、download対象と同じJSONが`result.schema.json`を通ることを確認する。結果値の閾値assertion、raw JSONのartifact保存、PlaywrightによるSafari実機Gate代替は行わない。
