...
### 3.2 Group 12 Timeline / Rig計測・実装Gate（T1 Slice A / P1 Slice C実装済み）

正本は`docs/future/2D_3_TIMELINE_RIG_PLAN.md`。PR #146 merge `cb21ea4`後にH1=E1、H2=L1、H3=M1を人間承認した。T1 Slice AはPR #153で可変時間、event、共通scheduler、検査、保存roundtrip、E1拒否を実装済みである。P1 Slice CはPR #154で静的Part構成Layer差し替え、H2=L1拒否、read-only inspection、1 History、保存roundtripを実装済みである。R1、bake共通preflight、H3数値budgetは先取りせず、H3数値は`docs/future/2D_3_H3_MEASUREMENT_PROTOCOL.md`の実測と後続人間承認まで未決定とする。
