...
プロパティパネルの「アセット」欄にある「新規アセットを作成」フォームで、画像を取り込まなくても透明なキャンバスのアセットを作れます。

- 「新規アセット名」「新規アセットの種別」「新規アセットのサイズ」「新規アセットのテンプレート」を指定して「新規アセットを作成」を押します。
- サイズは正方形・横長・縦長のプリセット、または幅・高さ 1〜4096px の自由入力から選べます。範囲外の値は自動調整せず、画像を作る前に理由を表示して止めます。
- 「空白」テンプレートは透明画像1枚だけを作ります。種別に対応するstarterを選ぶと、当たり判定・tag・背景・tile・gimmick・effectなど、そのテンプレートで示された設定も作成結果へ追加します。
- character基本テンプレートでは、mainレイヤーを参照するbody Partを作るかどうかをチェックボックスで選べます。テンプレートIDそのものは保存せず、作成された通常のデータだけを `.casproj` / asset.json へ保存します。
...
- **パーツ**: レイヤーにチェックを付けて「パーツを作成」。種別（head / body / …）、pivot、親パーツ、バインドポーズ、可動域を設定できます。既存パーツは「構成レイヤーを変更」から1件以上のLayerを選び、「構成レイヤーを確定」で静的に差し替えられます。別のパーツで使用中のLayerは暗黙に移動せず選択を拒否し、取消では保存・履歴を変更しません。空・重複・参照切れ・複数Part共有が残る場合、素材検査とリグ焼き込みに理由が表示されます。
...
Frame別の当たり判定上書きが1件以上あるAssetでは、PNG / WebP / `asset.json` / `.casproj`は利用できます。現在のSprite Sheet / Atlas / ZIPはFrame別上書きを表現できないため、処理開始前に対象Frameと判定、失われる情報、代替形式を示して拒否します。空の上書き配列だけでは拒否しません。

## 7. スマホ / iPad で使う

- スマホでは下部ナビの「ホーム / 編集 / プロパティ / タイムライン / 書き出し」で作業場所を切り替えます。
- 「編集」ではキャンバス上に横スクロールできる編集ツール一覧が表示されます。ツールを選ぶと、その下に「できること / 操作 / 変化と戻し方」が表示されます。
...
トリミング・消しゴム・色調整・パレット置換・輪郭線などの画像編集では、適用前の状態を「復旧点」として保存する処理を試みます。復旧点の保存に失敗しても画像編集自体は続くため、確実なバックアップとしては扱わないでください。
...
ブラウザの保存容量が足りずに保存へ失敗すると、画面上部に「保存容量が不足しています。ごみ箱を空にするか、不要なプロジェクトを削除して空き容量を確保してください。」と表示されます。ごみ箱（8.1）を空にする、または使わなくなったプロジェクトを完全に削除してから、もう一度操作をやり直してください。Frame別当たり判定の変更に失敗した場合も未確定の画面・Asset・Project参照・履歴・保存データは変更前へ戻り、エラー表示は原因確認のため残ります。画像Blobはこのmetadata操作では変更されません。
