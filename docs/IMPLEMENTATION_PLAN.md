...
| `2D-2` | 新規作成・取り込み・修正 | 複数 Asset、空キャンバス、template、画像 import、基本描画、非破壊修正、派生 variant、一括変更、任意形式と AI の境界を完成させる。独立Asset管理C-sliceはPR #93、#94で完了した。 | Codex | 現在の段階。`2D-3-TYPE-PROFILES + 2D-3-INSPECT`完了後、残るCREATEの矩形・自由size、明示template、初期Part、図形・文字境界を契約監査する。 |
...
- 現在の正式work packageは`2D-2-PROJECT + 2D-2-CREATE`の後続slice契約監査である。矩形・自由size、型別template、characterの初期Part、図形・文字をRASTERへ送る境界を判断する。正本は`docs/future/2D_2_CREATE_REMAINING_PLAN.md`とし、判断前にproduct code、schema、version、migration、保存形式、export形式を変更しない。
