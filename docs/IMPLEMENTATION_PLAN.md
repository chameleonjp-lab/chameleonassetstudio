...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理、残るCREATE A+B+X slice、RASTER Slice 1、REPAIR Slice 2は完了した。 | Codex | 現在の段階。複数Layer align / distributeはPR #113で実装・CI・mergeまで完了し、次のAsset canvas resize / game data追従契約を監査中。 |
...
- PR #113ではschema、version、migration、`.casproj`、export ZIP、dependenciesを変更していない。次の正式作業はAsset canvas resizeとgame data追従の契約監査である。現状、6判断と推奨`B1+P1+G1+O1+V1+H1`を`docs/future/2D_2_CANVAS_RESIZE_PLAN.md`へ提示し、人間判断を待つ。accept前にproduct codeへ着手しない。
