...
- 現在の正式work packageは`2D-2-PROJECT + 2D-2-CREATE`。2026-07-16に`C`（現行0.1.0の独立Asset管理を先行し、Family / Variantを別契約へ保留）をacceptedとした。契約監査PR #93はmerge commit `359cb9c9d0918df95d1fc52db6d472639f0f3703`でmainへmerge済みである。実装Draft PR #94でProject要約同期、安全な独立copy、複数Assetのreload / `.casproj`退避を実装中である。契約は`docs/future/2D_2_PROJECT_CREATE_PLAN.md`、実装証拠は`docs/future/2D_2_PROJECT_CREATE_REPORT.md`を正本とする。
