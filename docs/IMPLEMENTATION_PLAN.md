...
- 完了したwork package: `2D-1B-RECOVERY`。PR #80、#81により、snapshot所有境界、source不変性、復元前Blob必須化、復元とUndo / Redoの原子的整合、trash失敗時の原状態維持、Project / Asset ID衝突時の復元拒否、autosave確定後の競合検出、復元→Undo→reloadの利用者E2Eをmainへ反映した。PR #81のCI Run #207は成功済み。
- 次の実行チェックポイント: `2D-1B-STORAGE-CROSS-REVIEW`。`2D-1B-REVISION`、`2D-1B-LAYERS`、`2D-1B-RECOVERY`を横断し、所有境界、source不変性、原子性、競合拒否、失敗経路、test coverage、文書整合を確認する。これは新しい永続化機能を追加するwork packageではない。
- 横断レビューでBLOCKERがなければ、次の実装work packageは`2D-1B-CAPACITY`。BLOCKERが見つかった場合は、CAPACITYへ進む前に同じ保存基盤範囲の補修PRを作る。
- PR #53（`2D-1B-STORAGE`）と PR #55（`2D-2-CREATE-01`）はrevertせず、provisionalとして保持する。PR #53全体のprovisional状態は、未完了の`2D-1B-CAPACITY`、`2D-1B-CASPROJ`、`2D-1B-INPUT-SAFETY`、`2D-1B-GATE`に残る。PR #55は`2D-2-CREATE`のpartialであり、`2D-2-PROJECT`は未完了。
