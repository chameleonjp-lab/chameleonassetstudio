...
| `2D-3` | 動き・ゲーム用情報 | timeline、可変時間、event、簡易 rig、origin / anchor / collider、素材種別 profile、ゲーム風 preview、理由付き検査、変更影響を完成させる。 | Codex。新しい仕様判断はユーザーとChatGPTへ戻す。 | 現在の段階。`2D-3-TYPE-PROFILES + 2D-3-INSPECT`はPR #96、#97で完了。group 12契約監査PR #146、計測準備PR #147、配信基盤PR #148、root / H3分離PR #149はmainへ反映済み。2026-07-23の人間判断により、正式B0の4端末収集は保留し、H3を問題発生時の診断手段として残す。サービス本体を継続公開しながら、数値budgetへ依存しないT1 Slice Aを本実装PRで検証する。H3の数値budget、warning、hard capは未決定のまま維持する。正本は`docs/future/2D_3_TIMELINE_RIG_PLAN.md`と`2D_3_H3_MEASUREMENT_PROTOCOL.md`。 |
...
- group 12 `2D-3-TIMELINE + 2D-3-RIG`は、PR #145 merge `236571c`後のmainを固定し、仕様、実装・データ安全、テスト・端末の3担当によるread-only監査を実施した。契約監査PR #146 merge `cb21ea4`に続き、H1=E1 / H2=L1 / H3=M1の記録と計測準備PR #147 final head `1ba671f7`も、CI Run #460成功と3担当の最終`BLOCKER 0 / MUST 0`を経てmerge `24a089c`としてmainへ反映された。24時間配信基盤PR #148 merge `fbdeb357`と、サービス本体をroot・H3を`/h3/`へ分けたPR #149 merge `536318f`もmainへ反映済みである。移設前commitのiPhone 17 Pro baseline結果2件はハーネスの参考動作確認として保持するが、正式B0や数値budgetには使わない。2026-07-23の人間判断により、4端末B0はサービス利用で必要性が見えた時に再開する。T1 Slice Aは本実装PRで可変時間、event、時間consumer統一、H1=E1拒否を検証中で、R1 / P1とH3数値budgetは後続に残す。
