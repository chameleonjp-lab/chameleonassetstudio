...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理と残るCREATE A+B+X sliceは完了した。 | Codex | 現在の段階。`2D-2-RASTER + 2D-2-REPAIR`のshape/text表現、selection、resize座標、frameずれ時期を契約監査する。 |
...
- 現在の正式work packageは`2D-2-RASTER + 2D-2-REPAIR`の契約監査である。shape / textの保存表現、selectionの最初の範囲、trim / padding / resizeと座標の意味、frameずれ修正の時期を判断する。正本は`docs/future/2D_2_RASTER_REPAIR_PLAN.md`とする。
- 判断前にproduct code、schema、version、migration、`.casproj`、export ZIP、dependenciesを変更しない。
