...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理、残るCREATE A+B+X slice、RASTER Slice 1、REPAIR Slice 2は完了した。 | Codex | 現在の段階。複数Layer align / distributeはPR #113で完了した。Asset canvas resize / game data追従はPR #114の契約監査をmergeし、`B1+P1+G1+O1+V1+H1` acceptedとして別Draft PRで実装中。 |
...
- Asset canvas resize / game data追従の契約監査PR #114はmainへmerge済み（merge commit `99a00d250532010c0bbafed82a33bff290aebf7e`）。2026-07-17の人間による後続作業開始指示で`B1+P1+G1+O1+V1+H1`をacceptedとし、product code、unit、Desktop / touch E2E、docsを別Draft PRで実装中。schema、version、migration、`.casproj`、export ZIP構成、dependenciesは変更しない。
