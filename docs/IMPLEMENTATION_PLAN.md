...
| `2D-2` | 新規作成・取り込み・修正 | 複数 Asset、空キャンバス、template、画像 import、基本描画、非破壊修正、派生 variant、一括変更、任意形式と AI の境界を完成させる。独立Asset管理C-sliceはPR #93、#94で完了した。 | Codex | 現在の段階。残るCREATEは`A+B+X` accepted。矩形・自由size、明示template、任意character初期Partを実装し、shape / textはRASTERへ保留する。 |
...
- `2D-1B-GATE`: 正式完了。Gate実装PR #91はmainへmerge済みで、merge commitは`71c568d6c38846d5795b5e70fdea476336596e57`、最終headは`51a5a2baf6171e80a52fa5823df25fb5d33f95d8`、CI Run #269はlint、format、build、unit test、E2Eを全成功した。2026-07-16にユーザーからOpus 4.8 review完了・問題なしが報告され、少なくとも`BLOCKER 0 / MUST 0`、その他の指摘報告なしとして人間確認された。review証拠待ちを記録したPR #92もmainへmerge済みで、merge commitは`f1fcdf1fbd05f33810206ee0ebfbfd49cba784f0`、CI Run #271はsuccessである。契約は`docs/future/2D_1B_GATE_PLAN.md`、証拠は`docs/future/2D_1B_GATE_REPORT.md`、review記録は`docs/future/2D_1B_GATE_REVIEW_REQUEST.md`を正本とする。
- PR #53（`2D-1B-STORAGE`）のprovisional状態は`2D-1B-GATE`正式完了により解消した。PR #55（`2D-2-CREATE-01`）は`2D-2-CREATE`全体完了ではないためpartial / provisionalとして保持する。
- `2D-2-PROJECT + 2D-2-CREATE`のaccepted `C` sliceは完了した。契約監査PR #93に続き、実装PR #94はmerge commit `5b0d16478d0b0140e6e56db63b5b89c52fd0f608`でmainへmerge済みである。最終head `c44246cbcf9f038c8089cbba4f79528165d3b553`のCI Run #277はlint、format、build、unit test 401件、E2E 84件を全成功した。2026-07-16にOpus reviewと人間reviewの完了が報告された。Family / Variantと残るCREATE項目があるためwork package全体はpartialを維持する。契約は`docs/future/2D_2_PROJECT_CREATE_PLAN.md`、実装証拠は`docs/future/2D_2_PROJECT_CREATE_REPORT.md`を正本とする。
- `2D-3-TYPE-PROFILES + 2D-3-INSPECT`は正式完了した。契約PR #96と実装PR #97はmainへmerge済みで、PR #97のmerge commitは`500397ac7d04b23ac88cd17a6e79843c8405a557`、最終head `3e237e3186317095c73cdf411aa13d39e4ac8e6c`のCI Run #293はlint、format、build、unit test、E2Eを全成功した。2026-07-16にOpus reviewと人間確認の完了・問題なしが報告された。正本は`docs/future/2D_3_TYPE_PROFILES_INSPECT_PLAN.md`と`docs/future/2D_3_TYPE_PROFILES_INSPECT_REPORT.md`である。
- 現在の正式work packageは`2D-2-PROJECT + 2D-2-CREATE`の後続sliceである。2026-07-16に`A+B+X`（4096上限の矩形・自由size、明示template selector、任意character初期Part / shape-text保留）をacceptedとした。契約監査PR #98はmerge commit `0135cfe8859ea1d18af46d387e7089797512b9ed`でmainへmerge済みである。正本は`docs/future/2D_2_CREATE_REMAINING_PLAN.md`とし、次はaccepted範囲のproduct code Draft PRを開始する。
