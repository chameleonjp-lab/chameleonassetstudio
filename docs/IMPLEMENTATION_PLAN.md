...
| `2D-2` | 新規作成・取り込み・修正 | 複数 Asset、空キャンバス、template、画像 import、基本描画、非破壊修正、派生 variant、一括変更、任意形式と AI の境界を完成させる。現行0.1.0の独立Asset管理C-sliceはPR #93、#94で完了したが、Family / Variantと残るCREATE項目はpartialのまま保留する。 | Codex | `2D-3-TYPE-PROFILES + 2D-3-INSPECT`と契約が重ならない後続準備だけ条件付き並行可。 |
| `2D-3` | 動き・ゲーム用情報 | timeline、可変時間、event、簡易 rig、origin / anchor / collider、素材種別 profile、ゲーム風 preview、理由付き検査、変更影響を完成させる。 | Codex。新しい仕様判断はユーザーとChatGPTへ戻す。 | 現在の段階。`2D-3-TYPE-PROFILES + 2D-3-INSPECT`は`A+B+X` accepted。現行6種のread-only inspector実装を先行し、UI / icon、保存・export強制、preflightは別契約へ保留する。 |
...
- 現在の正式work packageは`2D-3-TYPE-PROFILES + 2D-3-INSPECT`である。2026-07-16に`A+B+X`（現行6種先行、balanced profile、read-only inspector）をacceptedとした。UI / iconは専用schema契約まで保留し、inspector issueによって保存、autosave、`.casproj`、exportを新たに停止しない。正本は`docs/future/2D_3_TYPE_PROFILES_INSPECT_PLAN.md`とする。
