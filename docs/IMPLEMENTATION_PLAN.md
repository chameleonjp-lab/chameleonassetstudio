...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理、残るCREATE A+B+X slice、RASTER Slice 1、REPAIR Slice 2は完了した。 | Codex | 現在の段階。複数Layer align / distributeはPR #113で完了した。Asset canvas resize / game data追従は`B1+P1+G1+O1+V1+H1` acceptedとしてDraft PR #115で実装し、実装commitのCI Run #383は全成功、review待ち。 |
...
- `2D-2-PROJECT + 2D-2-CREATE`の独立Asset管理C-sliceはPR #93、#94で完了した。Family / Variantは別契約までpartialを維持する。
...
- 現在の正式work packageは引き続き`2D-2-RASTER + 2D-2-REPAIR`である。複数Layerのalign / distributeはPR #112の契約監査後、`S1+R2+W1+D1+H1`が2026-07-17にacceptedとなり、PR #113で実装・検証・mainへのmergeまで完了した。最終head `a43d13a8fc82262495b2bfb8ab37eedefb1f4176`のCI Run #379は全成功、merge commitは`c6810487fd7dcd9e182f70c71fe7047c47b0ba0f`である。align / distributeの正本は`docs/future/2D_2_LAYER_ALIGN_PLAN.md`とし、work package全体の正本は引き続き`docs/future/2D_2_RASTER_REPAIR_PLAN.md`とする。
- Asset canvas resize / game data追従の契約監査PR #114はmainへmerge済み（merge commit `99a00d250532010c0bbafed82a33bff290aebf7e`）。2026-07-17の人間による後続作業開始指示で`B1+P1+G1+O1+V1+H1`をacceptedとし、product code、unit、Desktop / touch E2E、docsをDraft PR #115で実装した。実装commit `22a0d8c644f4db6c88448ae41c0a7f3ec95d9d97`のCI Run #383は全成功し、Draftのままreview待ち。schema、version、migration、`.casproj`、export ZIP構成、dependenciesは変更していない。
