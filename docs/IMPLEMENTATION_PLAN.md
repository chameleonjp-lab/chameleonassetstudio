...
| `2D-3` | 動き・ゲーム用情報 | timeline、可変時間、event、簡易 rig、origin / anchor / collider、素材種別 profile、ゲーム風 preview、理由付き検査、変更影響を完成させる。 | Codex。新しい仕様判断はユーザーとChatGPTへ戻す。 | 現在の段階。`2D-3-TYPE-PROFILES + 2D-3-INSPECT`はPR #96、#97で完了。group 12契約監査PR #146、計測準備PR #147、配信基盤PR #148、root / H3分離PR #149はmainへ反映済み。T1 Slice AはPR #153、merge `e8fac95`でmainへ反映済み。2026-07-23の人間判断により、正式B0の4端末収集は保留し、H3を問題発生時の診断手段として残す。現在は数値budgetへ依存しないP1 Slice Cの静的Part構成レイヤー差し替えを本実装PRで検証する。H3の数値budget、warning、hard capは未決定のまま維持する。正本は`docs/future/2D_3_TIMELINE_RIG_PLAN.md`と`2D_3_H3_MEASUREMENT_PROTOCOL.md`。 |
...
- group 12 `2D-3-TIMELINE + 2D-3-RIG`は、PR #145 merge `236571c`後のmainを固定し、仕様、実装・データ安全、テスト・端末の3担当によるread-only監査を実施した。契約監査PR #146 merge `cb21ea4`に続き、H1=E1 / H2=L1 / H3=M1の記録と計測準備PR #147 final head `1ba671f7`も、CI Run #460成功と3担当の最終`BLOCKER 0 / MUST 0`を経てmerge `24a089c`としてmainへ反映された。24時間配信基盤PR #148 merge `fbdeb357`と、サービス本体をroot・H3を`/h3/`へ分けたPR #149 merge `536318f`もmainへ反映済みである。T1 Slice AはPR #153 final head `7f684a7`、merge `e8fac95`としてmainへ反映済みで、CI Run #489は全成功した。移設前commitのiPhone 17 Pro baseline結果2件はハーネスの参考動作確認として保持するが、正式B0や数値budgetには使わない。2026-07-23の人間判断により、4端末B0はサービス利用で必要性が見えた時に再開する。現在はP1 Slice Cで静的Part構成レイヤー差し替え、H2=L1拒否、read-only inspection、狭いL1 bake refusal、History・保存roundtripを検証し、有限値・座標・資源上限を含むbake共通preflight、R1、H3数値budgetは後続に残す。
