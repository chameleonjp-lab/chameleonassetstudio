...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理、残るCREATE A+B+X slice、RASTER Slice 1、REPAIR Slice 2、Variant / Batch group 10、Import group 11の現在契約範囲まで完了した。 | Codex | group 11はPR #138とpost-merge補修PR #144（merge `616d225`）までmainへ反映した。Opus 4.8 unavailableを記録し、3担当の固定head代替reviewで`BLOCKER 0 / MUST 0`を確認後、人間がmergeを判断したため完了とする。frameずれ修正はGroup 12 D4としてPR #209で実装・検証・main反映まで完了した。 |
| `2D-3` | 動き・ゲーム用情報 | timeline、可変時間、event、簡易 rig、origin / anchor / collider、素材種別 profile、ゲーム風 preview、理由付き検査、変更影響を完成させる。 | Codex。新しい仕様判断はユーザーとChatGPTへ戻す。 | Group 12 `2D-3-TIMELINE + 2D-3-RIG`はcompleted。B2は未測定・未実装の性能課題として`2D-6-PERFORMANCE`へ延期した。Group 13は`G1+O1+P1`をacceptedし、PR #215〜#221の契約・実装・補修・検証をmainへ反映してcompletedとした。P1によりpolygonは2D Pro Gateまで`unsupported`を維持する。Group 14 `2D-3-PREVIEW + 2D-3-IMPACT`は`G14-P1 + G14-I1 + G14-U1`をacceptedし、初期実装PR #225、verification hardening PR #226、最終補修PR #227をmainへ反映した。PR #227 final head `95c1e9de81a7c445cbae3e8846a7582c55dd7ded`、merge `217c695f22ee494f0ee2166f44d496de09fa3e8e`、CI Run #689の全job成功と3方向read-only reviewを完了し、Group 14をcompleted、進捗を16/27とする。Group 15契約監査PR #229をmainへ反映し、2026-08-11に人間が`G15-C1 A + G15-C2 A + G15-C3 A`を採用した。契約状態はaccepted、製品実装状態は`implemented: 2D-4-CORE / CI-passed / independently-verified / merged`、実装handoffはcompleteとする。PR #233（merge `4ae44f647d66c184c55febfbe8577e682be2877f`）のCI Run #707（workflow ID `31478935075`）は全job成功、CORE証拠artifact ID `9096417804`（digest `sha256:d6868ce878edbc03ad5c9a5986c08c1812691a6c0738df79275ada3529a5b1c7`）とPlaywright artifact ID `9096547560`（digest `sha256:dd88d6496a683190d4264d17a0649f4b7dd7f1bc383c7ef83b406e5b56248077`）を記録した。現行Atlas `0.1.0`、既存ZIP、現行の理由付き拒否は維持する。G15-H1〜H3 Aの固定値を前提に、SHEETはPR #236で実装・CI・独立確認・mainへのmergeまで完了した。最終head `d3a178b5614a00f72f1bd0e706b95e28d4c5189b`、merge `4d55e898d78729c92fd4c9c55188edaa64eba8e5`、CI Run #716（workflow ID `31510296044`）は全job成功し、SHEET・CORE・Playwright artifactを記録した。採用済みG15-H3 Aに基づく`2D-4-SCALE`はPR #238でmainへmerge済み（final head `c2dc87ee1506f0a68b283eb3accf09d78921011a`、merge `a6f15d0b8bff90323d363827be62804959a0e7b0`）。CI Run #724（workflow ID `31518786336`）はbuild-and-test / E2Eを含む全job成功し、SCALE evidence artifact `9112190870`（digest `sha256:00b3fb7b2b266c8ff4eb306d64d1a88b9f0ebfc1733054ea8cff0e0d3573a2e4`）を記録した。親の固定head read-only確認でもBLOCKER / MUSTはなく、Group 15をcompleted、進捗を17/27とする。次は`2D-4-PACKAGE + 2D-4-PREFLIGHT + 2D-4-GENERIC-WEB`であり、distribution UI、375×667 product-path、物理iPhone Safariは後続Gateへ残す。 Group 15契約監査の正本は`docs/future/2D_4_CORE_SHEET_SCALE_PLAN.md`である。Group 12の正本は`docs/future/2D_3_TIMELINE_RIG_PLAN.md`、Group 13の正本は`docs/future/2D_3_GAME_DATA_PLAN.md`、Group 14の正本は`docs/future/2D_3_PREVIEW_IMPACT_PLAN.md`。 |
...
ADR-2026-07-28-029 / 030 / 031に残る「docs-only決定PRのmerge後に別Draft PRで開始」はD2〜D4の実装前Gateの履歴である。D2はPR #204、D3はPR #206、D4はPR #209で`implemented / CI-passed / independently-verified / merged`として完了した。
...
- `2D-1b`: 完了。`2D-1B-GATE`まで正式closeout済み。
...
- 個別完了: `2D-1B-CAPACITY`。PR #87はmainへmerge済み。merge commitは`66ba2c4096dabc297f402a9176b8c60de9c584f9`、最終head `d7b965af333821d4e73937873ed039972a6f5f04`のCI Run #255は全成功した。
- 個別完了: `2D-1B-CASPROJ`。PR #88はmainへmerge済み。merge commitは`a9a1e27a6f69544c379fb9fcefc90a13e0928859`、最終head `15e77f4bd59c7e03dc5877060fbc9195567db177`のCI Run #260は全成功した。
- 個別完了: `2D-1B-INPUT-SAFETY`。`B+X` accepted。契約PR #89と実装PR #90はmerge済みで、PR #90最終head `eef04042b9e82583e9efd0666b4d1fc4fb091ad9`のCI Run #266は全成功した。
- `2D-1B-GATE`: 正式完了。PR #91 merge commit `71c568d6c38846d5795b5e70fdea476336596e57`、最終head `51a5a2baf6171e80a52fa5823df25fb5d33f95d8`、CI Run #269は全成功。Opus 4.8 review問題なし、少なくとも`BLOCKER 0 / MUST 0`として人間確認済み。
- PR #53（`2D-1B-STORAGE`）のprovisional状態は`2D-1B-GATE`正式完了により解消した。
- `2D-2-PROJECT + 2D-2-CREATE`の独立Asset管理C-sliceはPR #93、#94で完了した。Family / Variantのoptional永続契約はPR #117、管理UI・linked recipe・明示refreshはPR #121（merge `2216f0c`）でmainへmerge済みである。複数target batchのSlice DはPR #122（merge `e98bcf0`）でmainへmerge済みであり、遡及Opus review完了により全体partialを解消した。
- `2D-3-TYPE-PROFILES + 2D-3-INSPECT`はPR #96、#97で正式完了した。PR #97 merge commit `500397ac7d04b23ac88cd17a6e79843c8405a557`、CI Run #293は全成功し、Opus reviewと人間確認も完了した。
...
- `2D-2-RASTER + 2D-2-REPAIR`の現在実装可能なaccepted範囲は完了した。複数Layerのalign / distributeはPR #113で、Asset canvas resize / game data追従はPR #115でmainへmerge済み。frameずれ修正はADR-2026-07-29-031のD4としてPR #209で実装・検証・main反映まで完了した。
...
- `2D-2-VARIANT + 2D-2-BATCH`は工程完了した。契約監査はPR #116、Slice AはPR #117（merge `015064c`）、Slice BはPR #118（merge `61a9b50`）、遡及review補修はPR #120（merge `378d611`）、Slice CはPR #121（merge `2216f0c`）、Slice DはPR #122（merge `e98bcf0`、CI Run #398全成功）でmainへ反映済み。Slice Dはlinked refresh / 明示Asset・layer palette / canvas resize、target別preview・除外・warning、直列progress・取消、16件上限、原子commit・1 History entryを実装し、Slice C reviewのSHOULD-A（非表示・非選択Familyのfingerprint全件再計算）とNOTE-3（mobile手動上書き確認の44px E2E）、Slice B reviewのNOTE-3（History往復による復旧点evict）も同じ境界で補修した。Slice D遡及Opus reviewは`BLOCKER 0 / MUST 0 / SHOULD 2 / NOTE 5`で、SHOULD-1（縮小canvas batchのwarning経路E2E）とSHOULD-2（docs statusのmerge後同期）はgroup 10 closeout補修で対応した。
- `2D-2-IMPORT-GATE + 2D-2-IMPORT-OPTIONAL + 2D-2-AI-BOUNDARY`は`G1+L1+Q1+P1+F1+A1+W1+S1` accepted。契約PR #124とSlice A PR #125（merge `7018984`、CI Run #404成功）でADR-0016 / ADR-0017を確定し、Slice B PR #126（merge `5a1663b`、CI Run #407成功）でprovenance基盤を実装した。Slice CはPR #127（merge `eaeb110`）とpreview guard repair PR #128（merge `be887a1`、CI Run #413全成功）で連番・手動格子sheet・common preview・quarantineをmainへ反映した。Slice Dは人間承認とADR-0018に従いPR #129（final head `5b98b24`、merge `33ebad4`）でTileset + Chameleon Atlas意味上roundtripを実装し、CI Run #415は全成功した。独立read-only reviewのSHOULD 2件はPR #133（merge `55750d2`）でcloseoutした。Slice Eのsource保存契約は人間承認A / ADR-0019 / PR #135（merge `f5bb322`）でmainへ反映済み。製品挙動はPR #138（merge `c188e17`）、独立レビュー補修はPR #144 final head `1980ae6`（merge `616d225`）でmainへ反映した。CI Run #450はunit 676件とChromium E2E 142件を含め全成功し、固定headの独立read-only reviewは`BLOCKER 0 / MUST 0 / SHOULD 2 / NOTE 1`だった。Opus review済みとは扱わず、SHOULD 2件は取り込みE2Eのtest debt、NOTE 1件はiPhone Safari実機Gateへ残したうえでgroup 11を完了とする。
- group 12 `2D-3-TIMELINE + 2D-3-RIG`は、PR #145 merge `236571c`後のmainを固定し、仕様、実装・データ安全、テスト・端末の3担当によるread-only監査を実施した。契約監査PR #146 merge `cb21ea4`に続き、H1=E1 / H2=L1 / H3=M1の記録と計測準備PR #147 final head `1ba671f7`も、CI Run #460成功と3担当の最終`BLOCKER 0 / MUST 0`を経てmerge `24a089c`としてmainへ反映された。24時間配信基盤PR #148 merge `fbdeb357`と、サービス本体をroot・H3を`/h3/`へ分けたPR #149 merge `536318f`もmainへ反映済みである。T1 Slice AはPR #153 final head `7f684a7`、merge `e8fac95`としてmainへ反映済みで、CI Run #489は全成功した。P1 Slice CはPR #154 final head `fdf75f0`、merge `1c700e7`としてmainへ反映済みで、CI Run #492はunit 732件、Chromium 159件、H3 1件、Pages公開・閉鎖各1件を含め全成功し、固定head独立reviewは`BLOCKER 0 / MUST 0 / SHOULD 0`だった。移設前commitのiPhone 17 Pro baseline結果2件はハーネスの参考動作確認として保持するが、正式B0や数値budgetには使わない。2026-07-23の人間判断により、4端末B0はサービス利用で必要性が見えた時に再開する。ADR-2026-07-24-027でSlice Bを分割し、PR #157 final head `834cc38`、merge `bf13cac`、CI Run #501全成功、非GitHub・非Opusの固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`としてB1を完了した。B2の資源上限、warning、hard cap、採用上限での実機測定とH3数値budgetは後続の明示判断まで保留する。ADR-2026-07-27-028でA1+B1を承認し、Slice D1はPR #201 final head `25cd332`、merge `c1d08e3`、CI Run #596全成功、固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として完了した。ADR-2026-07-28-029でA1+B1+C1を承認したD2は、PR #204 final head `8ebeb279`、merge `eeaea395`、CI Run #603全成功、固定head独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として完了した。
- ADR-2026-07-29-030でP1+A1+B1+C1を承認したD3は、PR #206 final head `ac84b8c2d6141f6353c3e07dbb2dbfae9a2f5c98`、merge `3081495a979d10176a05eb2907e7cede55cc8c9a`、CI Run #610全成功、固定headの3方向独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として完了した。
- ADR-2026-07-29-031でA1+B1+C1を承認し、D4は基準Frameと対象Frameを重ね、対象Frame全体の全LayerStateへ同じ手動deltaを適用する契約を確定した。PR #209 final head `16f5ae2a62928c92f039710e935f3c66c113b0c1`、merge `5e25d0d4e1a4c6680afdd5f5d05ef51d0f8bdea8`で専用model、UI、描画、編集防護、History / autosave接続、Unit / storage / `.casproj` / Chromium E2Eをmainへ反映した。CI Run #623はunit 799件、Chromium E2E 176件、H3 1件、Pages open / closed各1件を全成功し、failed / flaky / skippedは0件、固定headの3方向独立reviewは`BLOCKER 0 / MUST 0 / SHOULD 0`だった。D1〜D4の物理Safari確認は完了済みである。2026-08-02の人間判断によりT1 / P1 / B1の物理Safari Gateも完了扱いとし、B2は`2D-6-PERFORMANCE`へ延期した。B2の数値、warning、hard cap、採用上限でのproduct-path実測は未完了であり、性能合格の証拠には使わない。Group 12をcloseoutし、Group 13の契約監査を次の正式作業とする。schema、保存形式、export、dependencyは変更していない。
- Group 13 `2D-3-GAME-DATA + 2D-3-COLLIDER-OVERRIDE + 2D-3-POLYGON`は、docs-only契約監査PR #215と`G1+O1+P1`採用記録PR #216をmainへ反映した。G1 Slice AはPR #217 final head `e28e4de43f6825fdd5f0d206983e0760359f0503`、merge `bc48487ef47de113f96e80cf625b56b0e245efce`、CI Run #650全成功、固定headの3方向独立review `BLOCKER 0 / MUST 0 / SHOULD 0`として完了した。O1正式契約Slice BはPR #218 / merge `bbe9df960170942ddac67cad737b77fcb93d7e8d`、製品実装はPR #219 / merge `202f2e2f97fa4441e515d85a41bfb73234daf64e`、補修はPR #220 / merge `8e81c29c3d0141e76dfe6b0cdea611db1a1f72d0`、E2E待機安定化はPR #221 final head `45a41a19153334017801fd0354ffd0f678d9a30b`、merge `65df697e36f53ee20464d7bb74940f8713317d65`でmainへ反映した。CI Run #669はUnit 842件、Chromium E2E 184件、H3 1件、Pages公開・閉鎖各1件を全成功し、failed / flaky / skipped / retryは0件だった。固定headの3方向独立reviewも`BLOCKER 0 / MUST 0 / SHOULD 0`である。Group 13をcompleted、進捗を15/27とし、Asset `0.2.0`、migrationなし、polygon `unsupported`、現行Atlas系の事前拒否を維持する。その後、Group 14 one-sheetをacceptedとし、PR #225 / merge `cd3c3ffa`で製品実装、PR #226 / merge `280ab99d39a4857bb6bd7acd0d8f5479d3650766`でverification hardening、PR #227 final head `95c1e9de81a7c445cbae3e8846a7582c55dd7ded` / merge `217c695f22ee494f0ee2166f44d496de09fa3e8e`で最終補修をmainへ反映した。CI Run #689と3方向read-only reviewを完了し、Group 14をcompleted、進捗を16/27とする。次は17工程目のGroup 15で、契約監査PR #229と採用記録PR #231をmainへ反映済み。2026-08-11に`G15-C1 A + G15-C2 A + G15-C3 A`をacceptedとして採用した。G15-H1〜H3 Aの実装handoffは具体値、対象ファイル、受入証拠まで固定済みである。製品実装はPR #233でCORE、PR #236でSHEET、PR #238でSCALEをmainへ反映した。SCALEはCI Run #724、SCALE evidence artifact、親の固定head read-only確認を完了し、次は`2D-4-PACKAGE + 2D-4-PREFLIGHT + 2D-4-GENERIC-WEB`とする。進捗は17/27。Group 13の正本は`docs/future/2D_3_GAME_DATA_PLAN.md`、Group 14の正本は`docs/future/2D_3_PREVIEW_IMPACT_PLAN.md`。
- Group 16 `2D-4-PACKAGE + 2D-4-PREFLIGHT + 2D-4-GENERIC-WEB`は、docs-only契約監査PR #240 / merge `b5401529a552c38147d308d7209ad8483ffd85c4`、ユーザー採用`G16-C1 A + G16-C2 A + G16-C3 A`、実装PR #242 final head `6616ad30fdae7a05f98e0a0146ce69555bab1bfa`、merge `711bcec268d6e732a24c0c787c6054b41e415c27`でmainへ反映済み。CI Run #765は全job成功し、固定headの独立read-only確認は`BLOCKER 0 / MUST 0`だった。Group 16を`implemented / CI-passed / independently-verified / merged`、進捗を17/27とする。正本は`docs/future/2D_4_PACKAGE_PREFLIGHT_GENERIC_WEB_PLAN.md`。
- Group 17 `2D-4-PIXIJS + 2D-4-PHASER + 2D-4-DOCS`はPR #243 / #244 / #245でGate Aを完了し、PR #246（final head `9380494f7a662b9211f341d87a15f62d4b82986f`、merge `b79327857b2d9f9bfbc9a4c0a4a4566d1000a69a`、CI Run #785全job成功）で専用fixture、engine別E2E、証拠artifact、docsの実装・独立確認・main反映まで完了した。`G17-C1 A + G17-C2 A + G17-C3 A`の契約は`accepted / implemented / CI-passed / independently-verified / merged`であり、verifiedはPixiJS 8.12.0 / Phaser 4.2.0の専用fixtureに限る。Group 18（`2D-5-EVIDENCE + 2D-5-LABELS`）は`G18-C1 A + G18-C2 A + G18-C3 A`をacceptedし、PR #248（merge `3ab844d28d155a438dc8f10f8f9b22099a40093a`、CI Run #793）で実装・CI・独立確認・main反映まで完了した。進捗は18/27。次はGroup 19 / 20のtarget-specific検証である。
...
Group 19の契約とcandidate実装は維持し、runtime検証の未完了状態も維持する。PR #257（merge fb8272aaf21545dd8031bcb6fc761de6f2d379ab）でサポート終了したUnity activation workflowを削除し、main CI [Run #837](https://github.com/chameleonjp-lab/chameleonassetstudio/actions/runs/31962261321)の全job成功を確認した。

Godot 4.7.1-stableの既存Run #9は成功済みである。一方、Unity runtimeの手動Run #1 / #2は認証確認で停止し、Unity editorの実行結果・runtime artifactは未取得である。PCが使用できないという人間判断によりUnity runtimeはスキップするが、Unity 6000.3.21f1とGroup 19全体をverifiedへ昇格せず、進捗は18/27のままとする。Group 20はdocs-only handoffを開始し、runtime実行・artifact取得・verified昇格は保留する。

## Group 20 start decision (2026-08-26)
...
Group 21Bのマージ後、Group 21C（`2D-6-PERFORMANCE` + `2D-6-A11Y` + `2D-6-SECURITY`）を開始する。正本は[`2D_6_QUALITY_CONTRACT.md`](future/2D_6_QUALITY_CONTRACT.md)とする。
...
現在の後続Draft PR #274は、PR #273のマージ後に証拠台帳のCI記録を最新化するdocs＋Gate test変更である。head `49769a14490e2300e66a238251d9aa7da5ad52cc`に対するRun #897（Actions ID `33348582105`）はclassify / build-and-test success、unit 87 files / 916 tests success、E2Eは変更分類によりskipである。Group 22 artifact `9742835911`を取得した。
