...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理、残るCREATE A+B+X slice、RASTER Slice 1、REPAIR Slice 2は完了した。 | Codex | 現在の段階。Family / Variant契約のSlice AはPR #117、原子revision / recovery基盤のSlice BはPR #118と補修PR #120でmainへ反映済み。Slice Cの管理UI・linked recipe・明示refreshを現在の実装branchで検証中。Slice Dの複数target batch体験はC merge後。 |
...
- `2D-2-PROJECT + 2D-2-CREATE`の独立Asset管理C-sliceはPR #93、#94で完了した。Family / Variantのoptional永続契約はPR #117でmainへmerge済みで、管理UI・linked recipe・明示refreshは現在のSlice C実装branchへ反映した。merge / CI / review完了までは全体partialを維持する。
...
- 現在の正式work packageは`2D-2-VARIANT + 2D-2-BATCH`である。契約監査はPR #116、Slice AはPR #117（merge `015064c`）、Slice BはPR #118（merge `61a9b50`）、遡及review補修はPR #120（merge `378d611`）でmainへ反映済み。推奨`F1+C1+V1+T1+B1+O1+H1+L1`の直列分割に従い、現在はSlice CのFamily管理、linked mirror / palette、fingerprint、stale / manual-adjusted、preview、明示refresh、削除 / 隔離導線を実装・検証中である。複数targetの除外 / progress / cancelを含むSlice DはC merge後まで開始しない。
