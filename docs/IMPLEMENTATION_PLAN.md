...
| `2D-3` | 動き・ゲーム用情報 | timeline、可変時間、event、簡易 rig、origin / anchor / collider、素材種別 profile、ゲーム風 preview、理由付き検査、変更影響を完成させる。 | Codex。新しい仕様判断はユーザーとChatGPTへ戻す。 | 現在の段階。`2D-3-TYPE-PROFILES + 2D-3-INSPECT`はPR #96、#97で完了。group 12のdocs-only契約監査を`main@236571c`から開始し、T1 / R1 / P1の中核をADR-0021〜0023へ固定した。H1（派生export）、H2（Layer所属・空集合）、H3（bake上限）は人間判断待ちで、製品実装は未着手。正本は`docs/future/2D_3_TIMELINE_RIG_PLAN.md`。 |
...
- group 12 `2D-3-TIMELINE + 2D-3-RIG`は、PR #145 merge `236571c`後のmainを固定し、仕様、実装・データ安全、テスト・端末の3担当によるread-only監査を実施した。T1 / R1 / P1の中核契約はacceptedだが、H1〜H3が未決定のためproduct code、schema、保存、export、dependencyは変更せず、製品実装を開始しない。
