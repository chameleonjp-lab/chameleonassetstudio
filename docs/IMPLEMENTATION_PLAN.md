...
- `2D-2-PROJECT + 2D-2-CREATE`の独立Asset管理C-sliceはPR #93、#94で完了した。Family / Variantのoptional永続契約はPR #117、管理UI・linked recipe・明示refreshはPR #121（merge `2216f0c`）でmainへmerge済みである。複数target batchのSlice Dは現在の実装branchへ反映し、merge / CI / review完了までは全体partialを維持する。
...
- 現在の正式work packageは`2D-2-VARIANT + 2D-2-BATCH`である。契約監査はPR #116、Slice AはPR #117（merge `015064c`）、Slice BはPR #118（merge `61a9b50`）、遡及review補修はPR #120（merge `378d611`）、Slice CはPR #121（merge `2216f0c`）でmainへ反映済み。推奨`F1+C1+V1+T1+B1+O1+H1+L1`の直列分割に従い、現在はSlice Dのlinked refresh / 明示Asset・layer palette / canvas resize、target別preview・除外・warning、直列progress・取消、16件上限、原子commit・1 History entryを実装・検証中である。Slice C reviewのSHOULD-A（非表示・非選択Familyのfingerprint全件再計算）とNOTE-3（mobile手動上書き確認の44px E2E）、Slice B reviewのNOTE-3（History往復による復旧点evict）も同じ境界で補修した。
