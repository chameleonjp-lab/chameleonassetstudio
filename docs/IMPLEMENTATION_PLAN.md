...
| `2D-1b` | 保存・migration・復旧実装 | 改訂単位の整合保存、履歴・backup・trash・rollback、容量警告、安全な staged `.casproj` import と旧形式互換を実装する。PR #53 は詳細契約 Gate より先に merge 済みの provisional 実装であり、`2D-1B-REVISION` はPR #70、#71、#72で完了、`2D-1B-GATE` は未完了。 | ユーザーとChatGPTが範囲を確定し、Codexが実装する。Opus 4.8は利用可能な場合にCI成功後レビュー。 | 現在の段階。`2D-1B-REVISION`と`2D-1B-LAYERS`は完了。PR #76、#77、#78で保存境界、source不変性、TextureRefとBlobの双方向整合、export edit境界をmainへ反映済み。次は`2D-1B-RECOVERY`。 |
...
- 完了したwork package: `2D-1B-LAYERS`。PR #76、#77、#78により、source / edit / cache / export artifactの保存境界、source不変性、TextureRefとBlobの双方向整合、TextureRef IDとBlob keyの一意性、metadata-onlyの`saveAsset`境界、export入力のedit限定をmainへ反映した。PR #78のCI Run #182は成功済み。
- 次のwork package: `2D-1B-RECOVERY`。snapshot、trash、rollback、復元の既存provisional実装をaccepted済み契約と照合し、原子性、所有境界、source不変性、復元後Undo / Redoを再監査・補修する。
- PR #53（`2D-1B-STORAGE`）と PR #55（`2D-2-CREATE-01`）はrevertせず、provisionalとして保持する。PR #53全体のprovisional状態は、未完了の`2D-1B-RECOVERY`、`2D-1B-CAPACITY`、`2D-1B-CASPROJ`、`2D-1B-INPUT-SAFETY`、`2D-1B-GATE`に残る。PR #55は`2D-2-CREATE`のpartialであり、`2D-2-PROJECT`は未完了。
...
2. Codexが、確定済みwork packageのcode、tests、docsを1つのDraft PRへ実装し、CI失敗も同じPRで修正する。
3. CI成功後、Opus 4.8が利用可能な場合はreview-onlyで確認する。
4. Opus 4.8が利用できない場合は、未実施のレビュー範囲を記録し、ユーザーとChatGPTが差分とリスクを確認する。
5. `BLOCKER` / `MUST`相当の問題はCodexが同じPRで修正する。
