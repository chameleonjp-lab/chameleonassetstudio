...
| `2D-1a` | データ・座標・migration・復旧設計 | 5つのデータ層、Family / Variant、ID、座標、trim / flip / scale、animation / collider、target 拡張、検証、migration 契約を固定する。`2D-1A-LAYERS` と `2D-1A-COORD` は PR #52 accepted。現在の契約上の次段階は `2D-1A-MOTION`。 | Fable5、Codex が ADR / fixture 化 | `2D-6-PERFORMANCE` / `2D-6-A11Y` の現状基準計測だけ準備可。 |
| `2D-1b` | 保存・migration・復旧実装 | 改訂単位の整合保存、履歴・backup・trash・rollback、容量警告、安全な staged `.casproj` import と旧形式互換を実装する。PR #53 は詳細契約 Gate より先に merge 済みの provisional 実装であり、`2D-1B-GATE` は未完了。 | Codex、Opus 4.8 必須 review | 残る 2D-1A 契約後に PR #53 を再監査する。追加の 2D-1B 本実装は停止。 |
| `2D-2` | 新規作成・取り込み・修正 | 複数 Asset、空キャンバス、template、画像 import、基本描画、非破壊修正、派生 variant、一括変更、任意形式と AI の境界を完成させる。PR #55 は `2D-2-CREATE` の partial / provisional 実装であり、`2D-2-PROJECT` は未完了。 | Codex | PR #53 / #55 は revert しないが、再監査まで追加の 2D-2 本実装は停止。 |
...
- `2D-1A-LAYERS` と `2D-1A-COORD`: PR #52 accepted。
- 現在の契約上の次段階: `2D-1A-MOTION`。これは Fable5 または人間判断が必要な契約固定作業であり、Codexだけで新規決定しない。
- PR #53（`2D-1B-STORAGE`）と PR #55（`2D-2-CREATE-01`）は main へ先行マージ済み。revert せず provisional として保持し、残る 2D-1A 契約後に再監査する。`2D-1B-GATE` は未完了、`2D-2-CREATE` は partial。
- Codexだけで並行可能な準備作業: `2D-6-PERFORMANCE` / `2D-6-A11Y` の現状基準計測。これらは実装済み扱いにしない。
