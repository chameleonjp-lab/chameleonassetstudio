...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理、残るCREATE A+B+X slice、RASTER Slice 1、REPAIR Slice 2は完了した。 | Codex | 現在の段階。PR #117でSlice Aのoptional Family / Variant契約をmainへ反映済み。`2D-2-VARIANT + 2D-2-BATCH`の直列Slice Bとして、複数Assetの原子revision / recovery基盤を実装中。 |
...
- `2D-2-PROJECT + 2D-2-CREATE`の独立Asset管理C-sliceはPR #93、#94で完了した。Family / Variantのoptional永続契約はPR #117でmainへmerge済みで、管理UIはSlice Cまで未実装のため全体partialを維持する。
...
- 現在の正式work packageは`2D-2-VARIANT + 2D-2-BATCH`である。契約監査はPR #116、Slice AはPR #117（merge commit `015064c6ae6b9e2a0f28e84c9ac447b9f9e0a8d1`）でmainへmerge済み。推奨`F1+C1+V1+T1+B1+O1+H1+L1`の直列分割に従い、現在はSlice Bの原子revision / recovery基盤を実装中で、C / DはSlice B mergeまで開始しない。
