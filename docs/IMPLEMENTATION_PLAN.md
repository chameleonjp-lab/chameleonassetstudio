...
| `2D-2` | 新規作成・取り込み・修正 | 複数Asset、空キャンバス、template、画像import、基本描画、非破壊修正、variant、一括変更、任意形式とAI境界を完成させる。独立Asset管理、残るCREATE A+B+X slice、RASTER Slice 1、REPAIR Slice 2は完了した。 | Codex | 現在の段階。Asset canvas resize / game data追従はPR #115でmainへmerge済み。次の正式作業は`2D-2-VARIANT + 2D-2-BATCH`のdocs-only契約監査で、推奨`F1+C1+V1+T1+B1+O1+H1+L1`は人間判断待ち。 |
...
- 現在の正式work packageは`2D-2-VARIANT + 2D-2-BATCH`の契約監査である。正本`docs/future/2D_2_VARIANT_BATCH_PLAN.md`に推奨`F1+C1+V1+T1+B1+O1+H1+L1`、危険契約PRの直列分割、受け入れ条件を記録し、product implementationは人間判断待ちとする。
