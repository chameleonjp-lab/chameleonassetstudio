...
- flip copy 時の `events[].frameId` の新 ID への張り替えは、ADR-0002 のID参照規則とADR-0022のrig flip完全mapを参照する（events実装PRのチェックリストに含めること）。ADR-0010のcollider override / polygonはGroup 13の契約であり、Group 12へ混ぜない。
