...
ステータス: accepted（2026-07-22 人間承認、T1 + H1=E1）
上位文書: `docs/future/2D_3_TIMELINE_RIG_PLAN.md`（§3、§6 H1）、`docs/future/2D_ASSET_DATA_CONTRACT.md`（§8.2）
関連 fixture: T1 Slice A（PR #153、merge commit `e8fac954bf43e2589f5ac6e9917b0d7731a0fec8`）で追加
...
- H1=E1として、上記lossを持つ場合は固定fpsの`atlas.json`、それを含むproduct ZIP、helpers、examplesを理由付きで拒否する。PNG / WebP、単体`asset.json`、`.casproj`は許可する。明示確認後に均一化するE2は採用しない。
...
- 実装済み（T1 Slice A / PR #153）: Frame型・schema、再生scheduler、Frame複製、event、`assetInspection`、linked Family対応field、保存roundtrip、export preflight。
- fixture: fps fallback、duration override、同一Frame反復、loop / event、Frame単体複製のevent不変、Asset複製 / flipのevent ID一意性・frameId張替え・発火回数、削除、旧0.2.0 roundtrip、`.casproj`、E1対象exportの理由付き拒否と許可対象を固定する。
- 影響なし: Project / export-presets / atlas version、IndexedDB layout、`.casproj`配置、既存GIF / APNG importのuniform fps写像。
- 本ADRのdocs-only決定時点では型、schema、製品UI、exportを変更せず、後続のPR #153で上記範囲を実装した。

## 再検討条件

派生exportの初期挙動はH1=E1で決定済みである。Animation出現単位のduration、Atlasへのduration / event追加、resample、有限repeat保存、またはloss確認付き均一化へ変更する場合は、別ADR、形式version監査、独立review、人間承認を必要とする。T1製品実装には本ADRとは別の明示指示と実装PRを必要とし、その条件はPR #153で満たした。
