...
派生exportの初期挙動はH1=E1で決定済みである。Animation出現単位のduration、Atlasへのduration / event追加、resample、有限repeat保存、またはloss確認付き均一化へ変更する場合は、別ADR、形式version監査、独立review、人間承認を必要とする。T1製品実装は本ADRだけでは開始せず、別の明示指示と実装PRを必要とする。
