...
- Frame複製は`durationMs`をコピーし、新規captureはoverrideを持たない。旧GIF / APNG importで失った時間を遡及復元しない。
- eventは対象Frameの表示開始時に発火し、同じFrameの各出現とloopの各周回で発火する。複製・flipでは`frameId`を張り替え、dangling参照は意味検証で検出する。
- 再生、effect時間検査、関連consumerは同じ実効時間関数を使う。
- 可変時間またはeventを、固定fpsしか表現できない派生exportへ黙って均一化・削除しない。
...
- fixture: fps fallback、duration override、同一Frame反復、loop / event、複製・削除・flip、旧0.2.0 roundtrip、`.casproj`、対応不能exportの拒否またはloss確認を固定する。
