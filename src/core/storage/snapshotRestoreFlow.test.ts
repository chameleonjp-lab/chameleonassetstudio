 ...
 * ストレージ層だけで再現し、「復元 → Undo」を経ても edit texture の size と
 * 実際に保存されている edit Blob の中身が食い違わないことを固定する。
 * source TextureRef / Blob は復元対象に含めず、全手順で不変とする。
 *
...
describe('復旧点の復元 -> Undo の整合性（2D-1B-STORAGE §C, Opus 4.8 レビュー修正）', () => {
  it('復元してから Undo すると、edit texture.size と Blob の実体が復元前の状態に一致する', async () => {
    ...
    // 「現在（編集後）」のアセットと edit Blob。source metadata / Blob は不変。
    const currentAsset: Asset = {
      ...baseAsset,
      id: 'asset_restore_flow',
      textures: baseAsset.textures.map((tex) =>
        tex.kind === 'edit' ? { ...tex, size: newSize } : { ...tex },
      ),
    };
    ...
    // 「復旧点（編集前）」として、editだけが小さいアセットとBlobを保存しておく。
    const oldAsset: Asset = {
      ...baseAsset,
      id: 'asset_restore_flow',
      textures: baseAsset.textures.map((tex) =>
        tex.kind === 'edit' ? { ...tex, size: oldSize } : { ...tex },
      ),
    };
    ...
    // 上書きする前に、復元前（現在）の edit Blob を退避する
    ...
    // 復元直後: edit Asset metadata・Blob ともに「旧（復旧点）」で揃う。sourceは不変。
    {
      const { asset } = await loadAsset(currentAsset.id);
      expect(asset.textures.find((texture) => texture.kind === 'edit')?.size).toEqual(oldSize);
      expect(asset.textures.find((texture) => texture.kind === 'source')).toEqual(
        currentAsset.textures.find((texture) => texture.kind === 'source'),
      );
      const blob = await loadBlob(key);
      expect(new Uint8Array(await blob!.arrayBuffer())).toEqual(oldBytes);
      const sourceBlob = await loadBlob(`${currentAsset.id}/source/original.png`);
      expect(new Uint8Array(await sourceBlob!.arrayBuffer())).toEqual(sourceBytes);
    }

    // --- Undo（修正後の handleRestoreSnapshot と同じ手順: edit Blob も書き戻す） ---
    await saveAssetRevision({
      projectId,
    ...
    // Undo 後: edit Asset metadata・Blob ともに「新（復元前）」で揃い、sourceは不変。
    {
      const { asset } = await loadAsset(currentAsset.id);
      expect(asset.textures.find((texture) => texture.kind === 'edit')?.size).toEqual(newSize);
      expect(asset.textures.find((texture) => texture.kind === 'source')).toEqual(
        currentAsset.textures.find((texture) => texture.kind === 'source'),
      );
      const blob = await loadBlob(key);
