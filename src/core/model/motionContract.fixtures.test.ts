...
describe('ADR-0011: 追加フィールドは additive（durationMs の有無、events / colliderOverrides のような未知フィールドを許容）', () => {
  ...
  it('animation の未知フィールド events、frame の未知フィールド colliderOverrides を持つデータが現行 validator を通る', () => {
    ...
    const animationWithEvents = {
      ...
      events: [{ id: 'evt_1', name: 'attack_start', frameId: 'frame_with_overrides' }],
    };
    const asset = {
      ...baseAsset,
