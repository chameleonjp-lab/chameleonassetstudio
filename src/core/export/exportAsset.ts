...
function assertValidAsset(asset: Asset): void {
  ...
  if (!result.valid) {
    throw new ExportError(
      `アセットの内容が不正です: ${result.errors.join(" / ")}`,
    );
  }
}
...
async function loadAssetBitmaps(
  asset: Asset,
): Promise<Map<string, DecodedImageSource>> {
...
export async function exportSpriteSheet(
  asset: Asset,
): Promise<{ sheet: Blob; atlas: AtlasJson }> {
  assertValidAsset(asset);
  assertFixedFpsAnimationExportSafe(asset);
  assertColliderOverrideExportSafe(asset);
  const frames = asset.frames ?? [];
  const frameIds =
    frames.length > 0 ? frames.map((frame) => frame.id) : ["default"];
  const layout = computeSheetLayout(
    frameIds,
    asset.canvasSize.width,
...
async function sha256Hex(text: string): Promise<string> {
  ...
  if (!subtle) {
    throw new ExportError(
      "この環境ではdistribution manifestのSHA-256検査に対応していません。",
    );
  }
...
export async function exportDistributionZip(
  ...
  if (options.profile && options.profile !== "fixed-grid") {
    throw new ExportError(
      `未対応のdistribution profileです: ${options.profile}`,
    );
  }

  const legacyZip = await exportZip(asset);
  const legacyEntries = unzipSync(
    new Uint8Array(await legacyZip.arrayBuffer()),
  );
  ...
  entries["README.md"] = strToU8(
    buildExportReadme(asset, { distribution: true }),
  );
