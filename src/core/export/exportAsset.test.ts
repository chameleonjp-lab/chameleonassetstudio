...
describe("exportAsset texture kind boundary", () => {
  beforeEach(() => {
    ...
    loadBlobMock.mockResolvedValue(
  ...
  function assetReferencing(kind: "edit" | "source" | "thumbnail"): Asset {
    ...
    const texture = asset.textures.find(
  ...
  it("edit 参照の layer は export できる", async () => {
    await expect(
      exportImage(assetReferencing("edit"), "image/png"),
  ...
  it("source 参照の layer は Blob を読まずに export を拒否する", async () => {
    await expect(
      exportImage(assetReferencing("source"), "image/png"),
    ).rejects.toThrow(/edit テクスチャ/);
    expect(loadBlobMock).not.toHaveBeenCalled();
  });

  it("thumbnail 参照の layer は Blob を読まずに export を拒否する", async () => {
    await expect(
      exportImage(assetReferencing("thumbnail"), "image/png"),
    ).rejects.toThrow(/edit テクスチャ/);
    expect(loadBlobMock).not.toHaveBeenCalled();
  });

  it("export ZIP 生成後も保存系 API は呼ばれない", async () => {
    await expect(exportZip(assetReferencing("edit"))).resolves.toBeInstanceOf(
      Blob,
    );
  ...
  it("Frame collider overrideはPNG/WebP/asset.jsonを許可しSprite Sheet/ZIPをBlob読込前に拒否する", async () => {
    ...
    await expect(exportSpriteSheet(asset)).rejects.toThrow(
      /frame_idle_0.*col_body/s,
    );
    ...
    await expect(exportImage(asset, "image/png")).resolves.toBeInstanceOf(Blob);
    await expect(exportImage(asset, "image/webp")).resolves.toBeInstanceOf(
      Blob,
    );
    expect(loadBlobMock).toHaveBeenCalled();
  });
  ...
  it("意味不正なFrame collider overrideは許可形式も画像処理前に拒否する", async () => {
    const asset = assetReferencing("edit");
    asset.frames![0].colliderOverrides = [
      { colliderId: "missing", visible: false },
    ];
    expect(() => exportAssetJson(asset)).toThrow(
  ...
  it("distribution ZIPをlegacy ZIPと分離し、manifest hashとentry順を固定する", async () => {
    ...
    const firstEntries = unzipSync(
      new Uint8Array(await firstBlob.arrayBuffer()),
    );
    const secondEntries = unzipSync(
      new Uint8Array(await secondBlob.arrayBuffer()),
    );
    const legacyEntries = unzipSync(
    ...
    expect(firstEntries["manifest.json"]).toEqual(
      secondEntries["manifest.json"],
    );

    const manifest = JSON.parse(
      new TextDecoder().decode(firstEntries["manifest.json"]),
    );
