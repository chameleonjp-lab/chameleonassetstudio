...
重要: docs 確認は作業を止めるためではなく、誤実装を避けるために行う。対象が小さい場合は、関係箇所だけを読んで進めてよい。
...
GitHub のルールは、開発を止めるための禁止事項ではなく、事故を減らすための原則である。ユーザーが明示した目的を通すことを優先する。

### 6.1 原則

- 原則として `main` へ直接変更せず、作業ブランチと PR を使う。
- 原則として 1 PR 1 目的で進める。同じ目的を完成させる実装、テスト、docs、CI 安定化は同じ PR に含めてよい。
- 原則として、開始前に open PR を確認し、同じ目的の PR を重複作成しない。
- 既存仕様を壊さないようにする。
- 変更対象ファイルは、分かる範囲で先に列挙する。
- 既存テストは、失敗を隠す目的で削除、skip、期待値緩和しない。
- 仕様・UI が意図して変わった場合、またはテストの準備・待機・読み取り方法に欠陥がある場合は、理由と代替検証を記録してテストを修正または置き換えてよい。

### 6.2 例外として許可されること

次は、必要なら進めてよい。

- ユーザーが明示した場合の PR close、branch 作成、PR 作成、docs 修正。
- 小さな docs / instruction / README 修正を、過度に細かく分割せず 1 PR にまとめること。
- 既に失敗した PR と同じ目的でも、古い PR を close したうえで main から作り直すこと。
- 仕様に影響しない文言整理や運用ルール修正を、テスト未実行で docs-only として出すこと。
- 目的を達成するために必要な範囲で、関連 docs を同じ PR に含めること。
- 不安定なテストを一時的に skip する場合、原因、復帰条件、未検証範囲を PR に記録すること。

### 6.3 止めるべき場合

次は、作業を止めて人間確認に戻す。

- `asset.json` / `.casproj` / export ZIP / JSON Schema / version の互換性を壊す可能性がある。
- 既存 `.casproj` を読めなくする可能性がある。
- dependency を追加するが、ライセンスや商用利用条件が未確認。
- 3D 生成 AI、外部モデル重み、WebGPU 必須化を入れようとしている。
- SaaS / アカウント / クラウド / 課金を入れようとしている。
- ユーザーの明示目的と実装内容が食い違っている。
...
PR のきれいさを理由に、ユーザーが通したい内容を止めてはいけない。

### 6.5 CI の基準

- Markdown 文書だけの変更では、変更分類だけを実行し、build / unit / E2E は省略してよい。
- Markdown の文章は、コード用の自動整形に合わないことだけを理由に失敗させない。
- コードまたは設定を変更した場合は、lint / format / build / unit を実行する。
- `src/`、`e2e/`、ブラウザ表示、依存関係、Playwright、Vite、CI workflow に触れる変更では E2E も実行する。
- CI が失敗した場合は、実装不具合、テスト不具合、環境不具合に分類する。テスト自体が誤っている可能性を除外しない。
- 最終承認のための設計レビューは CI 成功後に行う。ただし、失敗原因を調べるための Opus 4.8 レビューは CI 失敗中でも行ってよい。
...
Codex Fallback Mode へ切り替える場合だけ、次の形式で明示する。
...
- E2E は変更禁止の仕様書ではない。仕様変更またはテスト欠陥を確認した場合は、理由と代替検証を明記して修正する。
