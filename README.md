...
AI エージェント向けの作業指示は、Claude Code 用の [`CLAUDE.md`](CLAUDE.md) と Codex 用の [`AGENTS.md`](AGENTS.md) に分けています。実装者は使用する環境に応じて該当する指示書を先に読んでください。
...
Claude Code で進める場合は `CLAUDE.md` を、Codex で進める場合は `AGENTS.md` を先に読んでください。Fable5 / `claude-fable-5` が一時的に使える場合でも、全体方針・仕様変更・複雑な優先順位判断だけに限定し、実装・探索・検証は Sonnet / Opus / Haiku / Codex へ委譲します。Fable5 が使えない場合は Claude Code の Opus / Sonnet / Haiku 運用と人間確認で進めます。
