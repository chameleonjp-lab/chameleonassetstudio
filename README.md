...
Phase 18 以降の作業、または `asset.json` / `.casproj` / export ZIP / 座標系 / 原点 / アンカー / 当たり判定 / リグ / 3D 関連に触れる作業では、`docs/future/FABLELESS_DEVELOPMENT_GUIDE.md` と `docs/future/POST_PHASE17_IMPLEMENTATION_PLAN.md` も必ず読んでください。開発モデルは `claude-opus-4-8`（設計レビュー・高難度判断）と `claude-sonnet-5`（実装）を中心とし、`claude-fable-5` は使いません。判断が割れる場合は人間確認に戻します。
