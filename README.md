...
Phase 18 以降の作業、または `asset.json` / `.casproj` / export ZIP / 座標系 / 原点 / アンカー / 当たり判定 / リグ / 3D 関連に触れる作業では、`docs/future/FABLELESS_DEVELOPMENT_GUIDE.md` と `docs/future/POST_PHASE17_IMPLEMENTATION_PLAN.md` も必ず読んでください。通常の主実装は Codex が担当し、設計レビュー・高難度判断は Claude Code Opus 4.8（`claude-opus-4-8`）で行います。必要時の補助として Claude Sonnet 系（例: `claude-sonnet-5`）を実装レビューや既存設計に沿う中程度の実装に使えますが、`claude-fable-5` は使いません。判断が割れる場合は人間確認に戻します。
