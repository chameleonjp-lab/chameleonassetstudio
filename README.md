...
初めて使う人は、[図で分かる初心者向けガイド](https://chameleonjp-lab.github.io/chameleonassetstudio/guide/)から、画像1枚を取り込んで書き出すまでの流れを確認できる。詳しい条件は[ユーザーガイド](docs/USER_GUIDE.md)に記載する。
...
AI エージェント向けの上位運用は [`docs/DEVELOPMENT_MODES.md`](docs/DEVELOPMENT_MODES.md) に集約しています。2D 完成計画では、Fable5 が段階開始時の判断、Codex が実装、Opus 4.8 が CI 成功後のレビューを担当する Hybrid Roadmap Mode を既定にします。Claude Code だけで完結する Primary Mode と、Fable5 制限時の Codex Fallback Mode も選べます。
...
2D 完成ロードマップでは Hybrid Roadmap Mode を使い、Fable5 は段階開始時の方針判断に限定し、Codex が実装、Opus 4.8 が CI 成功後の review-only を担当します。Claude Code 内で完結させる場合は Fable5（方針）、Sonnet5（実装）、Opus 4.8（レビュー）、Haiku（探索）の Primary Mode を使います。詳しい使い分けは `docs/DEVELOPMENT_MODES.md` を参照してください。
