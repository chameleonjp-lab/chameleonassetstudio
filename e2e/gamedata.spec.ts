...
test('矩形判定をパネルの移動ボタンでスナップ移動できる', async ({ page }) => {
  ...
  const before = (await readStoredAsset(page)).colliders[0].rect!;
  await page.getByRole('button', { name: '判定「body」を右へ移動' }).click();

  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[0].rect?.x)
    .not.toBe(before.x);
  const after = (await readStoredAsset(page)).colliders[0].rect!;
  expect(after.x % 16).toBe(0);
  expect(after.width).toBe(before.width);
  expect(after.height).toBe(before.height);

  await page.getByRole('button', { name: '元に戻す' }).click();
  await expect.poll(async () => (await readStoredAsset(page)).colliders[0].rect?.x).toBe(before.x);
});

test('円判定をパネルの移動ボタンで移動しても半径は変わらない', async ({ page }) => {
  await setupProjectWithImage(page, '円判定移動テスト');

  await page.getByRole('button', { name: '円判定を追加' }).click();
  await page.getByRole('button', { name: '判定「body」を選択' }).click();

  const before = (await readStoredAsset(page)).colliders[0].circle!;
  await page.getByRole('button', { name: '判定「body」を右へ移動' }).click();

  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[0].circle?.x)
    .toBe(before.x + 1);
  const after = (await readStoredAsset(page)).colliders[0].circle!;
