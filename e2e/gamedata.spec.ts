...
test('判定ツールでキャンバス上から矩形判定を選択・スナップ移動でき、Undo できる', async ({
  page,
}) => {
  const canvas = await setupProjectWithImage(page, '判定ツール矩形テスト');

  // 既定の矩形判定（64x64 キャンバスなので x:16, y:16, w:32, h:32）
  await page.getByRole('button', { name: '矩形判定を追加' }).click();

  // 判定ツールへ切り替え、座標計算を決定的にするため 100% ズームにする
  await page
    .getByRole('navigation', { name: 'ツール' })
    .getByRole('button', { name: '判定' })
    .click();
  ...
  // fit 表示の中心は world(32,32)。100% 表示ではキャンバス中心がビューポート中央に来る
  await page.mouse.click(centerX, centerY);
  const rectSelect = page.getByRole('button', { name: '判定「body」を選択' });
  await expect(rectSelect).toHaveAttribute('aria-pressed', 'true');

  // スナップ ON（グリッドサイズは既定の 16px）
  await page.getByLabel('スナップ').check();

  // 自動保存（デバウンス）完了を待ってから初期値を確認する
  const beforeX = 16;
  await expect.poll(async () => (await readStoredAsset(page)).colliders[0]?.rect?.x).toBe(beforeX);

  // 本体をドラッグして移動する
  await page.mouse.move(centerX, centerY);
  await page.mouse.down();
  await page.mouse.move(centerX + 37, centerY + 21, { steps: 4 });
  await page.mouse.up();

  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[0]?.rect?.x)
    .not.toBe(beforeX);
  const moved = await readStoredAsset(page);
  expect(moved.colliders[0].rect!.x % 16).toBe(0);
  expect(moved.colliders[0].rect!.y % 16).toBe(0);
  expect(moved.colliders[0].rect!.width).toBe(32);
  expect(moved.colliders[0].rect!.height).toBe(32);
...
test('判定ツールで円判定の半径ハンドルと中心移動を操作できる', async ({ page }) => {
  const canvas = await setupProjectWithImage(page, '判定ツール円テスト');

  // 既定の円判定（64x64 キャンバスなので中心 (32,32)、半径 16）
  await page.getByRole('button', { name: '円判定を追加' }).click();
  ...
  // 中心をクリックして選択する
  await page.mouse.click(centerX, centerY);
  await expect(page.getByRole('button', { name: '判定「body」を選択' })).toHaveAttribute(
    'aria-pressed',
    'true',
  );

  // 半径ハンドルは中心の右 (x + radius, y) = boxCenter + (16, 0)
  await page.mouse.move(centerX + 16, centerY);
  await page.mouse.down();
  await page.mouse.move(centerX + 28, centerY, { steps: 4 });
  await page.mouse.up();

  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[0]?.circle?.radius)
    .toBe(28);
  const afterResize = await readStoredAsset(page);
  // スナップ OFF のままなので中心は変わらない
  expect(afterResize.colliders[0].circle!.x).toBe(32);
  expect(afterResize.colliders[0].circle!.y).toBe(32);

  // スナップ ON にして中心をドラッグ移動する
  await page.getByLabel('スナップ').check();
  await page.mouse.move(centerX, centerY);
  await page.mouse.down();
  await page.mouse.move(centerX + 19, centerY + 13, { steps: 4 });
  await page.mouse.up();

  // 自動保存（デバウンス）完了を待ってから確認する
  await expect.poll(async () => (await readStoredAsset(page)).colliders[0]?.circle?.x).not.toBe(32);
  const moved = await readStoredAsset(page);
  expect(moved.colliders[0].circle!.x % 16).toBe(0);
  expect(moved.colliders[0].circle!.y % 16).toBe(0);
  expect(moved.colliders[0].circle!.radius).toBe(28);
});
