...
test('スナップ ON で矩形判定をキャンバス上で移動すると保存座標だけが更新される', async ({
  page,
}) => {
  const canvas = await setupProjectWithImage(page, '判定移動テスト');
  ...
  const beforeRect = before.colliders[0].rect!;
  const box = (await canvas.boundingBox())!;
  await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
  await page.mouse.down();
  await page.mouse.move(box.x + box.width / 2 + 80, box.y + box.height / 2 + 48, { steps: 4 });
  await page.mouse.up();

  await expect
    .poll(async () => {
      const rect = (await readStoredAsset(page)).colliders[0].rect!;
      return rect.x !== beforeRect.x || rect.y !== beforeRect.y;
    })
    .toBe(true);

  const afterRect = (await readStoredAsset(page)).colliders[0].rect!;
  expect(afterRect.x % 16).toBe(0);
  expect(afterRect.y % 16).toBe(0);
