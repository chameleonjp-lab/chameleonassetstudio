...
test('キャンバス上で rect / circle 判定を選択・ドラッグでき、スナップに連動する', async ({
  page,
}) => {
  const canvas = await setupProjectWithImage(page, '判定キャンバス編集テスト');
  const box = (await canvas.boundingBox())!;
  const cx = box.x + box.width / 2;
  const cy = box.y + box.height / 2;

  await page.getByRole('button', { name: '矩形判定を追加' }).click();
  const rectList = page.getByRole('list', { name: '当たり判定一覧' });
  const rectRow = rectList.getByRole('listitem').first();

  await page.mouse.click(cx, cy);
  await expect(rectRow.getByRole('button', { name: '判定「body」を選択' })).toHaveAttribute(
    'aria-pressed',
    'true',
  );

  const beforeRect = (await readStoredAsset(page)).colliders[0].rect!;
  await page.getByLabel('スナップ').check();
  await page.mouse.move(cx, cy);
  await page.mouse.down();
  await page.mouse.move(cx + 23, cy + 9, { steps: 4 });
  await page.mouse.up();
  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[0].rect?.x)
    .not.toBe(beforeRect.x);
  const snappedRect = (await readStoredAsset(page)).colliders[0].rect!;
  expect(snappedRect.x % 16).toBe(0);
  expect(snappedRect.y % 16).toBe(0);

  await page.getByRole('button', { name: '円判定を追加' }).click();
  const circleRow = rectList.getByRole('listitem').nth(1);
  await page.mouse.click(cx, cy);
  await expect(circleRow.getByRole('button', { name: '判定「body」を選択' })).toHaveAttribute(
    'aria-pressed',
    'true',
  );

  const beforeCircle = (await readStoredAsset(page)).colliders[1].circle!;
  await page.mouse.move(cx, cy);
  await page.mouse.down();
  await page.mouse.move(cx + 32, cy, { steps: 4 });
  await page.mouse.up();
  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[1].circle?.x)
    .not.toBe(beforeCircle.x);
});

test('sensor 判定は凡例テキストとスワッチで破線区別が分かる', async ({ page }) => {
  await setupProjectWithImage(page, 'センサー判定表示テスト');
  const legend = page.getByRole('list', { name: '判定用途の色凡例' });
  await expect(legend).toContainText('センサー（破線）');

  await page.getByRole('button', { name: '円判定を追加' }).click();
  const row = page.getByRole('list', { name: '当たり判定一覧' }).getByRole('listitem').first();
  await row.getByLabel('用途').selectOption('sensor');
  await expect(row.locator('.gamedata-collider-swatch.sensor')).toBeVisible();
});
