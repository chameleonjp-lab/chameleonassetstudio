...
test('スナップ ON で選択中の矩形判定を一覧ボタンから右へ移動できる', async ({ page }) => {
  await setupProjectWithImage(page, '矩形判定移動テスト');

  await page.getByRole('button', { name: '矩形判定を追加' }).click();
  await page.getByRole('button', { name: '判定「body」を選択' }).click();
  await page.getByLabel('スナップ').check();

  const before = await readStoredAsset(page);
  const rectBefore = before.colliders[0].rect!;
  await page.getByRole('button', { name: '判定「body」を右へ移動' }).click();

  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[0].rect?.x)
    .toBe(rectBefore.x + 16);
  const rectAfter = (await readStoredAsset(page)).colliders[0].rect!;
  expect(rectAfter.x % 16).toBe(0);
...
test('スナップ OFF で選択中の円判定を一覧ボタンから右へ移動できる', async ({ page }) => {
  await setupProjectWithImage(page, '円判定移動テスト');

  await page.getByRole('button', { name: '円判定を追加' }).click();
  await page.getByRole('button', { name: '判定「body」を選択' }).click();

  const before = await readStoredAsset(page);
  const circleBefore = before.colliders[0].circle!;
  await page.getByRole('button', { name: '判定「body」を右へ移動' }).click();

  await expect
    .poll(async () => (await readStoredAsset(page)).colliders[0].circle?.x)
    .toBe(circleBefore.x + 1);
  const circleAfter = (await readStoredAsset(page)).colliders[0].circle!;
  expect(circleAfter.y).toBe(circleBefore.y);
  expect(circleAfter.radius).toBe(circleBefore.radius);
});
