...
function readAssetJson(bytes: Uint8Array): CasprojAssetSummary {
  const entries = unzipSync(bytes);
  const path = Object.keys(entries).find((candidate) =>
    /^assets\/[^/]+\/asset\.json$/.test(candidate),
  );
  if (!path) {
    throw new Error('asset.json not found in casproj fixture');
  }
  return JSON.parse(strFromU8(entries[path])) as CasprojAssetSummary;
}

test('.casproj を書き出し、削除後に読み込むと画像ごと復元される', async ({ page }) => {
  const sourceBytes = await setupProjectWithImage(page, 'casproj-rt', 512, 256);
  ...
  const firstAsset = readAssetJson(bytes);
  const sourceTexture = firstAsset.textures.find((texture) => texture.kind === 'source')!;
  const thumbnailTexture = firstAsset.textures.find((texture) => texture.kind === 'thumbnail')!;
  const firstProvenance = firstAsset.provenance?.[0];
  expect(thumbnailTexture.size).toEqual({ width: 256, height: 128 });
  expect(firstProvenance).toMatchObject({
    sourceFileName: 'base.png',
    mimeType: 'image/png',
    byteLength: sourceBytes.byteLength,
    hash: `sha256:${createHash('sha256').update(sourceBytes).digest('hex')}`,
    textureId: sourceTexture.id,
  });
  expect(new Date(String(firstProvenance?.importedAt)).toISOString()).toBe(
    firstProvenance?.importedAt,
  );
  ...
  const reexportedAsset = readAssetJson(reexportBytes);
  expect(reexportedAsset.provenance).toEqual(firstAsset.provenance);
  expect(
    reexportedAsset.textures.some(
...
test('不正families付き.casprojは正本へ保存せず、理由とquarantineを表示する', async ({ page }) => {
  const [projectText, assetJson, imageBase64] = await Promise.all([
    readFile('src/core/storage/__fixtures__/v0.1.0-project.json', 'utf-8'),
    readFile('src/core/storage/__fixtures__/v0.1.0-asset.json', 'utf-8'),
  ...
  project.families = [
    {
      id: 'family_invalid_import',
      name: 'broken',
      baseAssetId: 'asset_missing_import',
      variants: [],
    },
  ];
  const zipped = zipSync({
  ...
  await expect(page.getByRole('alert')).toContainText(/Family|families/);
  await expect(page.getByRole('alert')).toContainText(
    '既存の保存済みプロジェクトは変更されていません',
  );
  await expect(page.getByRole('button', { name: '「旧形式フィクスチャ」を開く' })).toHaveCount(0);
  await expect(
    page
      ...
      .getByText('invalid-families.casproj'),
  ).toBeVisible();
});
