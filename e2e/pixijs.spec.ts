...
test('PixiJS 8.12.0 fixtureはHTTP経由で2ページとanimationを確認できる', async ({ page, browser }) => {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      await page.goto('/engine-fixtures/pixijs-v8/index.html');
      await expect(page.getByRole('heading', { name: 'PixiJS 8.12.0 fixture' })).toBeVisible();
      await expect(page.locator('#status')).toContainText('PixiJS 8.12.0 ready', {
        timeout: 15000,
      });

      const response = await page.request.get('/engine-fixtures/pixijs-v8/manifest.json');
      ...
      expect(runtime).toEqual({ engine: 'PixiJS', version: '8.12.0', pageCount: 2 });
    ...
    await writeFile(
      'test-results/group17-pixijs-evidence.json',
      JSON.stringify(
        {
          workPackage: '2D-4-PIXIJS',
          engine: 'PixiJS',
          engineVersion: '8.12.0',
          cdnUrl: 'https://cdn.jsdelivr.net/npm/pixi.js@8.12.0/dist/pixi.min.js',
          status,
