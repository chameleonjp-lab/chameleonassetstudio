...
test('画像レイヤーを追加し、名前変更・並べ替え・表示切り替えができる', async ({ page }) => {
  ...
  const expectedOverlayHash = `sha256:${createHash('sha256').update(overlay).digest('hex')}`;
  await expect
    .poll(async () => {
      ...
      return {
        provenanceCount: imported.provenance?.length,
        hash: overlayProvenance?.hash,
        textureKind: imported.textures.find(
          (texture) => texture.id === overlayProvenance?.textureId,
        )?.kind,
      };
    })
    .toEqual({
      provenanceCount: 2,
      hash: expectedOverlayHash,
      textureKind: 'source',
    });
