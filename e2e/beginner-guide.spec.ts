...
test("初心者向けビジュアルガイドを開ける", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: /まずは画像1枚.*5ステップ.*完成/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "最初の1作品を作る" }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "今すぐサービスを開く" }),
  ).toHaveAttribute("href", "../");
  await expect(
    page.getByRole("link", { name: "詳しいガイドを読む" }),
...
test("初心者向けビジュアルガイドはスマホ幅で横スクロールしない", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "画面下の4つだけ、先に覚える" }),
  ).toBeVisible();
